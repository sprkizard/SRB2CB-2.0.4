...
void R_DrawColumn_8(void)
{
	...
	count = dc_yh - dc_yl; // This 'count' is the total num of vertical pixels to draw

	if (count < 0) // Zero length, column does not exceed a pixel.
		return;

#ifdef RANGECHECK
	if ((unsigned int)dc_x >= (unsigned int)vid.width || dc_yl < 0 || dc_yh >= vid.height)
		return;
#endif
	...
	// This is as fast as it gets.
	{
		...
		register unsigned int heightmask = dc_texheight-1;

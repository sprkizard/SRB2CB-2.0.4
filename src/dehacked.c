...
#if 0 // dehacked searchfloatvalue
static float searchfloatvalue(const char *s)
{
	while (s[0] != '=' && s[0])
		s++;
	if (s[0] == '=')
	...
	do
	{
		if (myfgets(s, sizeof (s), f))
		{
			...
			*(tmp-1) = '\0';
...
#ifdef FREEFLY
			else if (!strcmp(word, "FREEFLY"))
			{
				DEH_WriteUndoline(word, va("%d", mapheaderinfo[num-1].freefly), UNDO_NONE);
				mapheaderinfo[num-1].freefly = i;
			}
#endif
			...
			else if (!strcmp(word, "LEVELFOGCOLOR"))
			{
				DEH_WriteUndoline(word, mapheaderinfo[num-1].levelfogcolor, UNDO_NONE);
				strncpy(mapheaderinfo[num-1].levelfogcolor, word2, 33);
			}
			else if (!strcmp(word, "LEVELFOG"))
			{
				DEH_WriteUndoline(word, va("%d", mapheaderinfo[num-1].levelfog), UNDO_NONE);
				mapheaderinfo[num-1].levelfog = i;
			}
			else if (!strcmp(word, "LEVELFOGDENSITY"))
			{
				DEH_WriteUndoline(word, va("%d", mapheaderinfo[num-1].levelfogdensity), UNDO_NONE);
				mapheaderinfo[num-1].levelfogdensity = (unsigned int)i;
			}
			else
...
static actionpointer_t actionpointers[] =
{
	...
	{{A_LightningShield},      "A_LIGHTNINGSHIELD"},
	{{A_FlameShield},          "A_FLAMESHIELD"},
#endif
#ifdef PARTICLES
	{{A_EmitParticle},         "A_EMITPARTICLE"},
    {{A_EmitLightParticle},    "A_EMITLIGHTPARTICLE"},
	{{A_EmitStarParticle},     "A_EMITSTARPARTICLE"},
#endif
	{{A_SetSkyBoxMobj},        "A_SETSKYBOXMOBJ"},
	{{A_SetBalloonColor},           "A_SETBALLOONCOLOR"},
...
static void DEH_LoadDehackedFile(MYFILE *f)
{
	...
	while (!myfeof(f))
	{
		...
		// SRB2CBTODO: Causes errors? Is this fine commented?
...
/*				else if (!strcmp(word, "SPRITE"))
				...
				else if (!strcmp(word, "SRB2"))
				{
					...
					// Only display an error message if this is not 2.0 or higher
					if (ver <= 200)
					{
...
void DEH_UnloadDehackedWad(USHORT wad) // SRB2CBTODO: autoreset everything, soc isn't really needed
{

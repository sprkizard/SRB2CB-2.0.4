...
// Aligns an entity to a vector, returns a vector with pitch yaw and roll for the object
// Pitch Roll and Yaw make the current orientation of the object
// v is what to align the object to
...
v3float_t *M_VectorAlignTo(float Pitch, float Yaw, float Roll, v3float_t v, byte AngleAxis, float Rate)
{
	...
	if (AngleAxis == 1)
	{
		float DestYaw   = (atan2(v.z,v.x));
		float DestRoll  = (atan2(v.y,v.x));
	...
	{
		float DestPitch = (atan2(v.z,v.y));
		float DestRoll  = (-atan2(v.x,v.y));
	...
	{
		float DestPitch = (-atan2(v.y,v.z));
		float DestYaw   = (-atan2(v.x,v.z));

		Pitch = Pitch+(DestPitch-Pitch)*Rate;
		Yaw   = Yaw+(DestYaw-Yaw)*Rate;
	}

	v3float_t *returnangles = Z_Malloc(sizeof(v3float_t), PU_LEVEL, NULL);
	memset(returnangles, 0, sizeof(*returnangles));
	returnangles->x = Yaw; // * 180 / M_PI
	returnangles->y = Pitch;
	returnangles->z = Roll;
...
void FV_Rotate(v3float_t *rotVec, const v3float_t *axisVec, const angle_t angle)
{
	...
	fixed_t ux = (axisVec->x* rotVec->x);
	fixed_t uy = (axisVec->x* rotVec->y);
	fixed_t uz = (axisVec->x* rotVec->z);
	fixed_t vx = (axisVec->y* rotVec->x);
	fixed_t vy = (axisVec->y* rotVec->y);
	fixed_t vz = (axisVec->y* rotVec->z);
	fixed_t wx = (axisVec->z* rotVec->x);
	fixed_t wy = (axisVec->z* rotVec->y);
	fixed_t wz = (axisVec->z* rotVec->z);
	fixed_t sa = sin(angle);
	fixed_t ca = cos(angle);
	fixed_t ua = ux+vy+wz;
	fixed_t ax = (axisVec->x*ua);
	fixed_t ay = (axisVec->y*ua);
	fixed_t az = (axisVec->z*ua);
	fixed_t xs = (axisVec->x*axisVec->x);
	fixed_t ys = (axisVec->y*axisVec->y);
	fixed_t zs = (axisVec->z*axisVec->z);
	fixed_t bx = (rotVec->x*ys+zs);
	fixed_t by = (rotVec->y*xs+zs);
	fixed_t bz = (rotVec->z*xs+ys);
	fixed_t cx = (axisVec->x*vy+wz);
	fixed_t cy = (axisVec->y*ux+wz);
	fixed_t cz = (axisVec->z*ux+vy);
	fixed_t dx = (bx-cx* ca);
	fixed_t dy = (by-cy* ca);
	fixed_t dz = (bz-cz* ca);
	fixed_t ex = (vz-wy* sa);
...
{
    vec[VX] = FIXED_TO_FLOAT(x);
    vec[VY] = FIXED_TO_FLOAT(y);
}

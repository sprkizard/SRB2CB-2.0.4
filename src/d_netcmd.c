...
static void Command_ResetCamera2_f(void);
...
#ifdef JTEBOTS
static CV_PossibleValue_t botai_cons_t[] = {{0, "Newbie"}, {1, "Normal"}, {2, "Experienced"}, {3, "Ruthless"}, {0, NULL}};
#endif

// Console variables used solely in the menu system.
// todo: // SRB2CBTODO: add a way to use non-console variables in the menu
...
// SRB2CBTODO: Inactive window options, support Windows and other OS's too!
consvar_t cv_pauseresume = {"pauseresume", "On", CV_SAVE, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL};
consvar_t cv_inactivepause = {"inactivepause", "On", CV_SAVE, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL};
consvar_t cv_inactivemusic = {"inactivemusic", "On", CV_SAVE, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL};
...
// XSrb2: color cvar is a netvar in case the server disallows colors (default on)
consvar_t cv_allowcolors = {"allowcolors", "Yes", CV_NETVAR, CV_YesNo, NULL, 0, NULL, NULL, 0, 0, NULL};

consvar_t cv_soundtest = {"soundtest", "0", CV_CALL, NULL, SoundTest_OnChange, 0, NULL, NULL, 0, 0, NULL};
consvar_t cv_ringcolor = {"ringcolor", "Off", CV_SAVE, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL};
...
// JTEBOT stuff for servers
#ifdef JTEBOTS
boolean jb_cmdwait = false;

CV_PossibleValue_t maxbots_cons_t[] = {{0, "MIN"}, {MAXPLAYERS-1, "MAX"}, {0, NULL}};
...
consvar_t cv_botai = {"botai", "Normal", CV_NETVAR, botai_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL};

// JTEBOTS On_Change functions
static void JB_MaxBots_OnChange(void)
{
	// Max of MAXBOTS bots...
	if (server && !stricmp(cv_maxbots.string, "MAX"))
	{
		cv_maxbots.value = MAXPLAYERS-1;
		CV_StealthSetValue(&cv_maxbots, (int)(cv_maxbots.value));
	}
}
...
static void JB_Command_BotAdd(void)
{
	...
	if (jb_cmdwait)
	{
		CONS_Printf("Error: You broke the bot system!, restart level to add more...\n"); // SRB2CBTODO: MAKE this WORK
		return;
	}
	...
	// Bots can changed in single player, but it invalidates your modfied-ness :P
	if (!multiplayer && !netgame)
	{
		G_ModifyGame();
	}
	...
	{
		CONS_Printf("Usage: addbot [character] [color or ctfteam] [nickname]\n");
		return;
	}
	...
	if (COM_Argc() < 2)
	{
		...
		strncpy(&buf[2],va("%s-bot",skins[(int)buf[0]].name),MAXPLAYERNAME);
	}
	else
	{
		// Skin name
		...
		if (skin == numskins)
		{
			CONS_Printf("Character %s not found.\n", skinname);
		...
		if (COM_Argc() > 3)
			...
			strncpy(&buf[2],va("%s-bot",skins[skin].name),MAXPLAYERNAME);
	}

	SendNetXCmd(XD_ADDBOT, &buf, 2+strlen(&buf[2])+1);
	jb_cmdwait = true;

	CONS_Printf("Added bot %s with the skin %s ", &buf[2], skins[(int)buf[0]].name);
	if (gametype == GT_CTF)
		CONS_Printf("on the %s team.\n", (buf[1] == 2) ? "blue" : "red");
	else
		CONS_Printf("colored %s.\n", Color_Names[(int)buf[1]]);
}
...
void D_RegisterServerCommands(void)
{
	...
	COM_AddCommand("add", Command_Addfile); // Shorthand for me, yay!
	...
	// SRB2CBTODO: Sometimes a bit can be missed and the command never recieved!
	RegisterNetXCmd(XD_ADDBOT, JB_Got_BotAdd);

	CV_RegisterVar(&cv_botai);
	CV_RegisterVar(&cv_maxbots);
	CV_RegisterVar(&cv_botcanflyme);
#endif

	// XSrb2: chat colors!
	CV_RegisterVar(&cv_allowcolors);
...
void D_RegisterClientCommands(void)
{
	...
	for (i = 0; i < MAXSKINCOLORS; i++)
		Color_cons_t[i] = (CV_PossibleValue_t){i,Color_Names[i]};
	Color_cons_t[MAXSKINCOLORS] = (CV_PossibleValue_t){0,NULL};
	...
	COM_AddCommand("resetcamera2", Command_ResetCamera2_f);
	...
	CV_RegisterVar(&cv_inactivemusic);
	CV_RegisterVar(&cv_inactivepause);
	CV_RegisterVar(&cv_pauseresume);

	// add devmode and testing commands
	COM_AddCommand("noclip", Command_CheatNoClip_f);
	COM_AddCommand("god", Command_CheatGhost_f);
	COM_AddCommand("ghost", Command_CheatGhost_f);
	...
	COM_AddCommand("shield", Command_Sheild_f);
	COM_AddCommand("gravflip", Command_Gravflip_f);
	COM_AddCommand("hurt", Command_Hurt_f);
...
#if 0 // Keep this for refrence purposes. -Jazz
...
static void Got_DummyCommand(byte **cp, int playernum)
{
	...
	if (NetPacket.packet.verification)
	{
		...
		if (NetPacket.packet.playernum == (unsigned int)playernum)
...
static void SendNameAndColor(void)
{
	...
	// SRB2CB: WHY? I like customizations

	// be either red or blue in capture the flag
	// SRB2CBTODO: Allow dark red, dark blue, cyan, and pink in CTF too?
...
#if 0 // SRB2CBTODO: Causes issues with skins?
...
static void SendNameAndColor2(void)
{
	...
	if (!netgame || (server && secondplaya == consoleplayer))
	{
		...
		if (cv_mute.value) // The server doesn't want name changes.
		{
...
static void Got_NameAndColor(byte **cp, int playernum)
{
	...
	if (netgame && !server && !addedtogame)
	{
		...
		// FIXME: What a mess. // SRB2CBTODO: FIX

		// Skip the message, ignoring it. The server will send
		// another later.
		extrainfo = READBYTE(*cp); // SRB2CBFIX: never read
		SKIPSTRING(*cp); // name
		SKIPSTRING(*cp); // skin
		return;
	}
	...
	{
		I_Error("Network Error: consoleplayer color received as %d, cv_playercolor.value is %d",
	...
	if (splitscreen && playernum == secondarydisplayplayer
		...
		&& !chmappending)
	{
		I_Error("Network Error: secondarydisplayplayer color received as %d, cv_playercolor2.value is %d",
...
// Reset the cameras of either player
static void Command_ResetCamera_f(void)
{
...
#define MAPNAME &buf[5]
	...
	if (delay != 2)
	{
		...
		I_Assert(W_CheckNumForName(mapname) != LUMPERROR); // SRB2CBTODO: More detailed assert stuff
...
static void Command_Map_f(void)
{
	...
	// The map command loads an internal wad lump always,
	// the map command doesn't support external files as in doom legacy // SRB2CBTODO: Support external maps too?
	...
	if (!multiplayer && !netgame)
	{
		if (!(mapheaderinfo[newmapnum-1].typeoflevel & TOL_SP) && !cv_devmode)
		{
...
static void Got_Mapcmd(byte **cp, int playernum)
{
	...
	if (!skipprecutscene)
	{
		...
		CONS_Printf(text[STSTR_CLEV], stringy);
	}
...
static void Got_RandomSeed(byte **cp, int playernum)
{
	...
	seed = READBYTE(*cp);
	if (playernum != serverplayer) // it's not from the server, why?
		return;
...
static void Command_Teamchange_f(void)
{
	...
	{
		if (NetPacket.packet.newteam == (unsigned int)players[consoleplayer].ctfteam ||
...
static void Command_Teamchange2_f(void)
{
	...
	{
		if (NetPacket.packet.newteam == (unsigned int)players[secondarydisplayplayer].ctfteam ||
...
static void Command_ServerTeamChange_f(void)
{
	...
	{
		if (NetPacket.packet.newteam == (unsigned int)players[NetPacket.packet.playernum].ctfteam ||
			(players[NetPacket.packet.playernum].spectator && !NetPacket.packet.newteam))
			error = true;
	}
	else
...
static void Got_Teamchange(byte **cp, int playernum)
{
	...
	{
		if ((NetPacket.packet.newteam && (NetPacket.packet.newteam == (unsigned int)players[playernum].ctfteam)) ||
	...
	//Safety first!
	//Mega hack. P_DamageMobj needs cleaning, badly. // SRB2CBTODO:!
...
// Got_Consistency
//
// If you consfailed, this tries // SRB2CBTODO: THIS IS STUPID, CHECK AND FIX
...
//	affectedplayer = READBYTE(*cp);
	...
	for (i = 0; i < numplayers; i++) // SRB2CBTODO: Consistency seems stupidly done here
	{
		j = READBYTE(*cp);
		if (!playeringame[j] || !players[j].mo)
			continue; // ...huh?! // SRB2CBTODO: Causes issues?
...
static void Command_Addfile(void)
{
	...
	// A file any file can be added when not in a netgame,
	// but if the player is in a network game and he is not the server,
	// he can only add non-game changing files
...
void JB_Got_BotAdd(UINT8 **p, int playernum)
{
	UINT8 skin, color;
	char name[MAXPLAYERNAME];

	// Read the data from the p lump...
	skin = READUINT8(*p);
	color = READUINT8(*p);
	READSTRING(*p,name);

	// Add the bot and announce it.
	// JB_BotAdd must be called first in the if statement
	// or it might not add the bot for some players!!
	// JB_BotAdd returns false when adding the bot has failed,
	// so it doesn't announce when nothing happened.
	if (JB_BotAdd(skin, playernum, color, name)
		&& consoleplayer != playernum)
	{
		CONS_Printf("%s the BOT has joined the game", name);
		if (gametype == GT_CTF)
			CONS_Printf(" on the %s team", (color == 2) ? "blue" : "red");
		CONS_Printf(".\n");
	}
...
boolean JB_BotAdd(byte skin, int playernum, byte color, char *name)
{
	...
	bots[i].ownernum = (byte)playernum;
	...
	if (i+1 > doomcom->numslots)
		doomcom->numslots = (short)(i+1);

	// Occupy the node...
	nodeingame[i] = true;
	playernode[i] = (byte)i;
	nodetoplayer[i] = (signed char)i;
	playerpernode[i] = 1;
...
static void Command_Playintro_f(void)
{
	if (netgame)
	{
		CONS_Printf("This can't be done in a netgame.\n"); // SRB2CBTODO: make a string list
		return;
	}
...
void ObjectPlace_OnChange(void) // SRB2CBTODO: Don't only change data for player [0]!
{
	...
	if ((netgame
		 ...
		 || multiplayer
#endif
		) && cv_objectplace.value) // You spoon!
	{
...
#ifdef JTEBOTS
			if (players[i].bot) // don't do this if you're not a real player
				continue;
#endif
	...
	if (cv_objectplace.value)
	{
		...
		// Ceco is annoying here...

		if ((players[0].pflags & PF_NIGHTSMODE))
			return;

#ifdef JTEBOTS
		if (players[0].bot) // don't do this if you're not a real player
			return;
#endif
...
void D_GameTypeChanged(int lastgametype)
{
	...
	if (gametype == GT_CTF || gametype == GT_MATCH)
	{
		...
		// If an unscrupulous player(cheater :P) deletes the above code block,
...
static void Ringslinger_OnChange(void)
{
	...
	if ((grade&7) < 3 && !netgame && cv_ringslinger.value && !cv_devmode)
	{
...
static void Setrings_OnChange(void)
{
	if ((grade&7) < 5 && !netgame && cv_setrings.value && !cv_devmode)
	{
		CONS_Printf("%s", text[NOTEARNED]);
...
static void Setlives_OnChange(void)
{
	if ((grade&7) < 4 && !netgame && cv_setlives.value && !cv_devmode)
	{
		CONS_Printf("%s", text[NOTEARNED]);
...
static void Setcontinues_OnChange(void)
{
	if ((grade&7) < 4 && !netgame && cv_setcontinues.value && !cv_devmode)
	{
		CONS_Printf("%s", text[NOTEARNED]);
...
static void Gravity_OnChange(void)
{
	if ((grade&7) < 2 && !netgame && !(cv_devmode || devparm)
...
static void Command_ExitLevel_f(void)
{
	if (!(netgame || (multiplayer && gametype != GT_COOP)) && !cv_devmode)
	{
		CONS_Printf("%s",text[CANTUSESINGLEPLAYER]);
		return;
	}
...
static void Command_Skynum_f(void)
{
	// Yeah, this is considered modifying the game,
	// a level could use sky color as part of the level design or something
	G_ModifyGame();
...
static void Fishcake_OnChange(void)
{
	...
	else if (cv_devmode != cv_fishcake.value)
		CV_SetValue(&cv_fishcake, cv_devmode);
}
#endif
...
static void ForceSkin_OnChange(void)
{
	...
	if (cv_forceskin.value >= 0 && (netgame || multiplayer)) // Don't cv_forceskin in singleplayer
	{
...
static void Color_OnChange(void)
{
	//if (!P_PlayerMoving(consoleplayer))
	{
		...
		if (menuactive)
		{
			// Don't allow a player to have a "None" color
			...
			// Don't allow yellow in game modes where the it could be mistaken for a player that went super
			if (gametype == GT_MATCH || gametype == GT_CTF) // SRB2CBTODO: Color error?
			{
				if (cv_playercolor.value == 15)
				{
					if (players[consoleplayer].skincolor == 16)

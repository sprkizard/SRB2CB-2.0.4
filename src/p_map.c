...
static void P_DoSpring(mobj_t *spring, mobj_t *object)
{
	...
	{
		// SRB2CBTODO: // Someone want to make these work like bumpers?
		return;
	}
	...
	{
		// Don't reset momx or momy with these kinds of springs
		if (!(spring->type == MT_YELLOWSIDE) || !(spring->type == MT_REDSIDE)
			|| !(spring->type == MT_CBUMPER))
			object->momx = object->momy = 0;

		P_UnsetThingPosition(object);
		// Teleport the player to the middle of the spring,
		// so there's no issues with the angle he approaches at
		object->x = spring->x;
		object->y = spring->y;
		P_SetThingPosition(object);
	}

	// Normal springs can be used as vertically flipped springs,
	// but ceiling spings stay ceiling springs
	...
	if ((spring->eflags & MFE_VERTICALFLIP) && (spring->info->speed > 0))
		P_SetObjectAbsMomZ(object, -FIXEDSCALE(spring->info->speed, spring->scale), false);
	else
		P_SetObjectAbsMomZ(object, FIXEDSCALE(spring->info->speed, spring->scale), false);
	...
	{
#ifdef JTEBOTS
		if (object->player && object->player->bot)
			object->player->bot->springmove = true;
#endif
		// SRB2CBTODO: Evenin2d may not be needed
		P_InstaThrustEvenIn2D(object, spring->angle, FIXEDSCALE(spring->info->damage, spring->scale));
	}
#ifdef JTEBOTS
	else if (object->player && object->player->bot)
		object->player->bot->springmove = false;
#endif
	...
	if (object->player)
	{
		if (spring->info->damage && !(object->player->cmd.forwardmove != 0 || object->player->cmd.sidemove != 0)
			&& !(spring->type == MT_CBUMPER) && !(spring->type == MT_BALLOONR) && !(spring->type == MT_BALLOON))
		{
		...
		if (spring->type == MT_BALLOONR || spring->type == MT_BALLOON)
		{
			object->player->pflags &= ~PF_SPINNING;
			object->player->pflags &= ~PF_GLIDING;
			object->player->glidetime = 0;
			object->player->climbing = 0;
			object->player->powers[pw_tailsfly] = 0;
			object->player->pflags &= ~PF_THOKKED;
			object->player->onconveyor = 0;
			object->player->pflags &= ~PF_CARRIED;
			object->player->pflags &= ~PF_SLIDING;
			object->player->jumping = false; // SRB2CBTODO: What?
		}
		else
			P_ResetPlayer(object->player);

		// Don't change states for sideways springs, bumpers or balloons
		if (!(spring->type == MT_YELLOWSIDE || spring->type == MT_REDSIDE
			  || spring->type == MT_CBUMPER || spring->type == MT_BALLOON || spring->type == MT_BALLOONR))
		{
			if (spring->info->speed > 0)
...
// Acceleration for MF2_SLIDEPUSH items // SRB2CBTODO: Make this better
#define PUSHACCEL (2*FRACUNIT)
...
static boolean PIT_CheckThing(mobj_t *thing)
{
	...
	// Make sure that objects are here before continuing
...
#ifdef SEENAMES // SRB2CBTODO: Causes problems?
	if (tmthing->type == MT_NAMECHECK)
	{
		if (thing->player && tmthing->target && tmthing->target->player && thing->player != tmthing->target->player)
		{
			const char *stringy = player_names[thing->player-players];
			sprintf(tmthing->target->player->seename, "%s", stringy);
		}
	...
	// Missiles can hit other things and hang gliders can also touch players
	if (tmthing->flags & MF_MISSILE || tmthing->type == MT_SHELL || tmthing->type == MT_FIREBALL
		|| tmthing->type == MT_HANGGLIDER || tmthing->type == MT_SKATEBOARD)
	{
		...
		else if ((tmthing->type == MT_BLACKEGGMAN_MISSILE || tmthing->type == MT_HANGGLIDER)
			&& thing->player
			...
			&& tmthing->target != thing)
		{
			...
			// Give the object no issues if it contacts objects,
			// without this flag it would stay stuck on object contact
			// The player is overshadowed by the flying object's properties and
			// so the object must be adjusted to be more flexible
			tmthing->flags |= MF_NOCLIPTHING;
		...
		else if ((tmthing->type == MT_SKATEBOARD)
				 && thing->player
				 && thing->tracer != tmthing
				 && !(thing->player->pflags & PF_MINECART)
				 && tmthing->target != thing) // SRB2CBTODO:
		{
			// Control the skateboard!
			thing->player->pflags |= PF_MINECART;
			thing->player->pflags &= ~PF_JUMPED;
			thing->player->pflags &= ~PF_SPINNING;
			thing->player->pflags &= ~PF_THOKKED;
			thing->player->pflags &= ~PF_SLIDING;
			thing->player->pflags &= ~PF_JUMPDOWN;
			P_SetTarget(&thing->tracer, tmthing);
			P_SetTarget(&tmthing->tracer, NULL); // Disable homing-ness
			P_SetTarget(&tmthing->target, thing); // Set owner to the player
			tmthing->momz = 0;
			...
			// Give the object no issues if it contacts objects,
			// without this flag it would stay stuck on object contact
			// The player is overshadowed by the skateboard's properties and
			// so the skateboard must be adjusted to be more flexible
			tmthing->flags |= MF_NOCLIPTHING;

			tmthing->height = thing->height;
		...
		else if ((tmthing->type == MT_BLACKEGGMAN_MISSILE || tmthing->type == MT_HANGGLIDER) && thing->player
				 && ((thing->player->pflags & PF_ITEMHANG) || (thing->player->pflags & PF_JUMPED)))
		{
			// Ignore this check, don't attach to the player if the player already attactched to a mobj like this
		}
		else if ((tmthing->type == MT_SKATEBOARD) && thing->player
				 && ((thing->player->pflags & PF_MINECART)))
		{
			// Ignore this check, don't attach to the player if he already attactched to a mobj like this
		}
		...
		// Don't let hang-gliders or other specific things hurt the player!
		else if (!(tmthing->type == MT_HANGGLIDER || tmthing->type == MT_SKATEBOARD))
		...
		&& (tmthing->z + tmthing->height) >= thing->z)
	{
		...
		{
			// SRB2CBTODO: Make springs work like bumpers in nights mode
		...
		&& !(thing->z + thing->height < tmthing->z || thing->z > tmthing->z + tmthing->height))
	{
		if ((gametype == GT_MATCH && (!cv_matchtype.value))
			|| ((gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value))
	...
	{
		// Thing is the player being picked up
		// tmthing is the flyer
		if (tmthing->player && thing->player)
		{
			// If the thing you're trying to carry is a lot bigger than you,
			// just drop it
			if (thing->scale > tmthing->scale+50)
				thing->player->pflags &= ~PF_CARRIED;

			// Bots cannot pick another random player up in match mode
			if ((tmthing->player->bot && (thing->player != &players[tmthing->player->bot->ownernum])) && gametype == GT_MATCH)
				thing->player->pflags &= ~PF_CARRIED;
...
#ifdef JTEBOTS
						|| (thing->player->bot && (players[thing->player->bot->ownernum].pflags & PF_CARRIED)) // bots don't latch on to flying people if their owner is flying
#endif
						)
						thing->player->pflags &= ~PF_CARRIED;
					else
	...
	if (thing->player)
	{
		if (thing->eflags & MFE_VERTICALFLIP && tmthing->eflags & MFE_VERTICALFLIP)
		{
			...
			// Stuff where the player doesn't move
			if (thing->z + thing->height <= tmthing->z + tmthing->height && !(thing->state == &states[thing->info->painstate]))
			{
				switch (tmthing->type)
				{
					case MT_FAN: // fan
						if (thing->z + thing->height >= tmthing->z + tmthing->height - (tmthing->health << FRACBITS)
							&& thing->momz > -tmthing->info->speed
							&& !(thing->player->climbing || (thing->player->pflags & PF_GLIDING)))
						{
					...
					case MT_STEAM: // Steam
						// Only when it bursts
						if (tmthing->state == &states[S_STEAM1]
							&& thing->z + thing->height >= tmthing->z + tmthing->height - 16*FRACUNIT)
						{
							thing->momz = -tmthing->info->speed;
							P_ResetPlayer(thing->player);
							if (!(thing->state == &states[S_PLAY_FALL1] || thing->state == &states[S_PLAY_FALL2]))
		...
		else
		{
			...
			// Stuff where the player doesn't move
			if (thing->z >= tmthing->z && !(thing->state == &states[thing->info->painstate]))
			{
				switch (tmthing->type)
				{
					case MT_FAN: // fan
						if (thing->z <= tmthing->z + (tmthing->health << FRACBITS) && thing->momz < tmthing->info->speed
							&& !(thing->player->climbing || (thing->player->pflags & PF_GLIDING)))
						{
							P_SetObjectAbsMomZ(tmthing, tmthing->info->speed/4, true);

							if (thing->momz > tmthing->info->speed/NEWTICRATERATIO)
								thing->momz = tmthing->info->speed/NEWTICRATERATIO;
	...
	if (tmthing->player) // Is the moving/interacting object the player?
	{
		if (tmthing->z >= thing->z && !(tmthing->state == &states[tmthing->info->painstate]))
		{
			...
			{
				case MT_FAN: // fan
					if (tmthing->z <= thing->z + (thing->health << FRACBITS) && tmthing->momz < thing->info->speed
						&& !(tmthing->player->climbing || (tmthing->player->pflags & PF_GLIDING)))
					{
						P_SetObjectAbsMomZ(tmthing, thing->info->speed/4, true);

						if (tmthing->momz > thing->info->speed/NEWTICRATERATIO)
							tmthing->momz = thing->info->speed/NEWTICRATERATIO;
				...
				|| ((tmthing->eflags & MFE_VERTICALFLIP) && tmthing->z + tmthing->height == thing->z - FRACUNIT)))
		{
			if (thing->flags & MF_SPRING)
			{
				...
				&& (tmthing->scale > thing->scale/2)) // The thing trying to break the monitor must be greater than at least half its scale
			{
			...
			// If a lot larger than the monitor, break it just by touching it
			else if (thing->flags & MF_MONITOR && (tmthing->scale*FRACUNIT > (FixedMul(thing->scale*FRACUNIT, (17*FRACUNIT)/10))))
				P_DamageMobj(thing, tmthing, tmthing, 1); // break the monitor
		...
		!(thing->flags & MF_NOCLIP) && !(tmthing->flags & MF_NOCLIP))
	{
		if (tmthing->eflags & MFE_VERTICALFLIP)
		{
			...
			// (don't climb max. 24 FRACUNITS while already in air)
...
static boolean PIT_CheckLine(line_t *ld)
{
	...
	{
		if (((tmthing->flags & MF_ENEMY) || (tmthing->flags & MF_BOSS)
			 || (tmthing->type == MT_EGGGUARD)) && ld->flags & ML_BLOCKMONSTERS)
...
//   (enemies won't move to a dropoff)
//  speciallines[]
//  numspeciallines
//
...
boolean P_CheckPosition(mobj_t *thing, fixed_t x, fixed_t y)
{
	...
	{
		tmfloorz = tmsectorfloorz = P_GetZAt(newsubsec->sector->f_slope, thing->x, thing->y);
		tmdropoffz = P_GetZAt(newsubsec->sector->f_slope, thing->x, thing->y);
	}
	else
#endif
	...
	if (newsubsec->sector->c_slope)
		tmceilingz = tmsectorceilingz = P_GetZAt(newsubsec->sector->c_slope, thing->x, thing->y);
	else
#endif
	...
	if (newsubsec->sector->ffloors)
	{
		...
		for (rover = newsubsec->sector->ffloors; rover; rover = rover->next)
		{
			...
			if (thing->player && (rover->flags & FF_SWIMMABLE)
				&& GETSECSPECIAL(rover->master->frontsector->special, 1) == 3
				&& !(rover->master->flags & ML_BLOCKMONSTERS) && ((rover->master->flags & ML_EFFECT3)
				|| thing->z-thing->momz > topheight - 16*FRACUNIT))
				;
			else if (!((((rover->flags & FF_BLOCKPLAYER) && thing->player)
				|| ((rover->flags & FF_BLOCKOTHERS) && !thing->player)) || rover->flags & FF_QUICKSAND))
			{
				if (!(thing->player && !(thing->player->pflags & PF_NIGHTSMODE)
					  && (((thing->player->charability == CA_SWIM) || thing->player->powers[pw_super])
						  && thing->ceilingz-topheight >= thing->height)
					&& !(thing->player->pflags & PF_SPINNING) && thing->player->speed > thing->player->runspeed
					&& thing->z < topheight + 30*FRACUNIT
					&& thing->z > topheight - 30*FRACUNIT
			...
			if (bottomheight < tmceilingz && abs(delta1) >= abs(delta2)
				&& (/*thing->z + thing->height <= bottomheight
					|| */!(rover->flags & FF_PLATFORM))
				&& !(thing->type == MT_SKIM	&& (rover->flags & FF_SWIMMABLE)))
			{
				tmceilingz = bottomheight;
			}
		}
	}

#ifdef POLYOBJECTS // Collision (position check)
	...
	// check lines
	xl = (unsigned int)(tmbbox[BOXLEFT] - bmaporgx - MAXMOVE)>>MAPBLOCKSHIFT;
	xh = (unsigned int)(tmbbox[BOXRIGHT] - bmaporgx + MAXMOVE)>>MAPBLOCKSHIFT;
	yl = (unsigned int)(tmbbox[BOXBOTTOM] - bmaporgy - MAXMOVE)>>MAPBLOCKSHIFT;
	yh = (unsigned int)(tmbbox[BOXTOP] - bmaporgy + MAXMOVE)>>MAPBLOCKSHIFT;
...
boolean P_CheckCameraPosition(fixed_t x, fixed_t y, camera_t *thiscam)
{
	...
	if (newsubsec->sector->f_slope)
	{
		tmfloorz = tmsectorfloorz = P_GetZAt(newsubsec->sector->f_slope, thiscam->x, thiscam->y);
		tmdropoffz = P_GetZAt(newsubsec->sector->f_slope, thiscam->x, thiscam->y);
	}
	else
#endif
	tmfloorz = tmsectorfloorz = tmdropoffz = newsubsec->sector->floorheight;
...
#ifdef ESLOPE
	if (newsubsec->sector->c_slope)
		tmceilingz = tmsectorceilingz = P_GetZAt(newsubsec->sector->c_slope, thiscam->x, thiscam->y);
	else
#endif
	tmceilingz = tmsectorceilingz = newsubsec->sector->ceilingheight;
	...
	if (newsubsec->sector->ffloors)
	{
		ffloor_t *rover;
		fixed_t delta1, delta2;
		fixed_t thingtop = thiscam->z + thiscam->height;
		...
		for (rover = newsubsec->sector->ffloors; rover; rover = rover->next)
		{
			...
			// Make sure platform FOFs don't restrict the camera, if the player
			// goes to the top of the platform, allow the camera to go through
			if (rover->flags & FF_PLATFORM)
			{
				if ((thiscam->z > bottomheight - 1) &&
					(thiscam->z < topheight))
				{
					if (thiscam == &camera && players[displayplayer].mo)
					{
						if (players[displayplayer].mo->z > bottomheight - 1)
							continue;
					}
					else if (thiscam == &camera2 && players[secondarydisplayplayer].mo)
					{
						if (players[secondarydisplayplayer].mo->z > bottomheight - 1)
							continue;
					}
				}
			}
			...
			{
				if ((thiscam->z < topheight + 1) &&
					(thiscam->z > bottomheight))
				{
					if (thiscam == &camera && players[displayplayer].mo)
					{
						if (players[displayplayer].mo->z < topheight + 1)
							continue;
					}
					else if (thiscam == &camera2 && players[secondarydisplayplayer].mo)
					{
						if (players[secondarydisplayplayer].mo->z < topheight + 1)
							continue;
					}
				}
			}
...
#ifdef POLYOBJECTS // Collision (camera position)
	// Check polyobjects and see if tmfloorz/tmceilingz need to be altered
	{
		validcount++;
...
int P_TryCameraMove(fixed_t x, fixed_t y, camera_t *thiscam)
{
	...
	if (twodlevel
		|| (thiscam == &camera && players[displayplayer].mo
			&& (players[displayplayer].mo->flags2 & MF2_TWOD))
		|| (thiscam == &camera2 && players[secondarydisplayplayer].mo
			&& (players[secondarydisplayplayer].mo->flags2 & MF2_TWOD)))
			itsatwodlevel = true;
	...
	if (!itsatwodlevel)
	{
		...
		// When the player doesn't clip, the camera shouldn't either
		if ((thiscam == &camera
			 && (players[displayplayer].pflags & PF_NOCLIP))
			|| (thiscam == &camera2
				&& (players[secondarydisplayplayer].pflags & PF_NOCLIP)))
		{
			cameranoclip = true;
		}
		else
...
boolean PIT_PushableMoved(mobj_t *thing) // TODO: Fix for slopes VPHYSICS ESLOPE
{
	fixed_t blockdist;
...
//
// P_TryMove
// Attempt to move to a new position, called pretty much whenever something moves
//
// This checks moves along the XY axis
boolean P_TryMove(mobj_t *thing, fixed_t x, fixed_t y, boolean allowdropoff) // SRB2CBTODO: Check when something is outside the map!
{
	fixed_t oldx, oldy;
	...
	if (!(thing->flags & MF_NOCLIP))
	{
		...
		if (thing->eflags & MFE_VERTICALFLIP)  // SRB2CBCHECK // SRB2CBTODO:
		{
			if (thing->z < tmfloorz)
			{
				CheckMissileImpact(thing);
			...
			&& GETSECSPECIAL(R_PointInSubsector(x, y)->sector->special, 1) != 14)
		{
			...
			if (thing->eflags & MFE_VERTICALFLIP)
			{
				...
				{
					fixed_t slopestep = maxstep * thing->subsector->sector->c_slope->zangle;

					if (thing->z + thing->height == thing->ceilingz && tmceilingz > thing->z + thing->height
						&& tmceilingz - thing->z + thing->height <= slopestep) // ESLOPE: Handle drop off differently for slopes?
					{
						thing->z = tmceilingz - thing->height;
						thing->eflags |= MFE_JUSTSTEPPEDDOWN;
					}
				}
				else if (thing->z + thing->height == thing->ceilingz && tmceilingz > thing->z + thing->height
						 && tmceilingz - thing->z + thing->height <= maxstep) // ESLOPE: Handle drop off differently for slopes?
				{
					thing->z = tmceilingz - thing->height;
					thing->eflags |= MFE_JUSTSTEPPEDDOWN;
				}
			...
			else
			{
				...
				{
					fixed_t slopestep = maxstep * (thing->subsector->sector->f_slope->zangle);
					if (thing->subsector->sector->f_slope->zangle < 50)
						slopestep /= 5.4f;

					if (slopestep < maxstep)
						slopestep = maxstep;

					if (thing->z == thing->floorz && tmfloorz < thing->z && thing->z - tmfloorz <= slopestep)
					{
						thing->z = tmfloorz;
						thing->eflags |= MFE_JUSTSTEPPEDDOWN;
					}
	...
	if (thing->flags & MF_PUSHABLE)
	{
		...
		yh = (unsigned int)(thing->y + MAXRADIUS - bmaporgy)>>MAPBLOCKSHIFT;
		yl = (unsigned int)(thing->y - MAXRADIUS - bmaporgy)>>MAPBLOCKSHIFT;
		xh = (unsigned int)(thing->x + MAXRADIUS - bmaporgx)>>MAPBLOCKSHIFT;
		xl = (unsigned int)(thing->x - MAXRADIUS - bmaporgx)>>MAPBLOCKSHIFT;
...
#ifdef VPHYSICS // Catapult off a slope
#if 1
			...
			if (currentsector->f_slope)
				if (P_GetZAt(currentsector->f_slope, thing->x, thing->y)/FRACUNIT > (nextz/FRACUNIT)+(slopeang*3))
					...
					if (currentsector->f_slope->zangle > 9)
					{
						...
						predictz += (((thing->pitchangle/(ANG45/45))+90)/70.0f)+thingspeed/9;
	...
	if (P_IsObjectOnSlope(thing, false))
	{
		...
		if (nextsector != currentsector)
		{
			// Give a boost up from the slope you came if the next sector is lower than the first
			// If your next sector does not have a slope and you're comming off of one
			if (currentsector->f_slope && !nextsector->f_slope)
			{
				fixed_t currentz = P_GetZAt(currentsector->f_slope, thing->x, thing->y);
				fixed_t predictz = P_GetZAt(currentsector->f_slope, thing->x+thing->momx, thing->y+thing->momy);
	...
	// SRB2CBTODO: Make a function for floor/ceilingz auto check with slopes?
#ifdef ESLOPE
	if (thing->subsector->sector->f_slope)
	{
		// SRB2CBTODO: Support a mobj's gravity for this too
...
static boolean P_ThingHeightClip(mobj_t *thing)
{
	...
	boolean onfloor = P_IsObjectOnGround(thing);
	...
	if (thing->player)
	{
		if (thing->player->playerstate == PST_DEAD)
			return true;
	}
	...
	thing->floorz = tmfloorz;
	thing->ceilingz = tmceilingz; // SRB2CBTODO: ZMOVE
	...
	{
		if (thing->eflags & MFE_VERTICALFLIP) // Don't worry, it looks strange, but it works
		...
		if (thing->eflags & MFE_VERTICALFLIP)  // SRB2CBTODO: Check
	...
	if (thing->ceilingz - thing->floorz < thing->height && thing->z >= thing->floorz)
		// This code causes much trouble, but this also fixes
		// a lot of problems, this implementation is mainly for mobj stepping up
		// (walk on solid things without jumping or fake 3d bridge)
		// SRB2CBTODO: Is this even needed anymore?
		return false;
...
static boolean PTR_SlideCameraTraverse(intercept_t *in)
{
	...
	if (openbottom - mapcampointer->z > 0) // JTE: We don't want to make the camera step up.
	...
	if (glidesector->sector != player->mo->subsector->sector)
	{
		...
		if (glidesector->sector->ffloors)
		{
			...
			for (rover = glidesector->sector->ffloors; rover; rover = rover->next)
			{
				...
				if (player->mo->eflags & MFE_VERTICALFLIP)
				{
					...
					if (bottomheight > player->mo->z + player->mo->height - FIXEDSCALE(16*FRACUNIT, player->mo->scale))
					{
						floorclimb = false;
						thrust = true;
						boostup = true;
				...
				else
				{
					...
					if (topheight < player->mo->z + FIXEDSCALE(16*FRACUNIT, player->mo->scale))
					{
						floorclimb = false;
						thrust = true;
						boostup = true;
		...
		if (player->mo->eflags & MFE_VERTICALFLIP)
		{
			if ((floorz <= player->mo->z + player->mo->height)
				&& ((player->mo->z + player->mo->momz) <= floorz))
				floorclimb = true;

			if (!floorclimb && ceilingz > player->mo->z - FIXEDSCALE(16*FRACUNIT, player->mo->scale)
			...
			if ((player->mo->z + player->mo->height - FIXEDSCALE(16*FRACUNIT, player->mo->scale) > ceilingz)
		...
		else
		{
			...
			if (!floorclimb && floorz < player->mo->z + FIXEDSCALE(16*FRACUNIT, player->mo->scale)
			...
			if ((player->mo->z + FIXEDSCALE(16*FRACUNIT, player->mo->scale) < floorz)
...
static boolean PTR_SlideTraverse(intercept_t *in)
{
	...
	maxstep = FIXEDSCALE(MAXSTEPMOVE, slidemo->scale);
...
isblocking:
	...
	{
		if ((li->polyobj->lines[0]->backsector->flags & SF_TRIGGERSPECIAL_INSIDE)
	...
	if (slidemo->player) // ESLOPE VPHYSICS allow wall running
	{
		line_t *checkline = li;
		...
		if (checksector->ffloors)
		{
			for (rover = checksector->ffloors; rover; rover = rover->next)
			{
				...
				// Got this far, so I guess it's climbable. // SRB2CBTODO: Climbing check, also, better method to do this?
		...
		//if (!(checkline->flags & ML_NOCLIMB))
		{
			...
			if (((!slidemo->player->wallrunning
				  && abs(slidemo->angle - ANG90 - climbline) < ANG45) ||

				 (slidemo->player->wallrunning == 1
									 ...
									 + (ANG90 * (whichside ? -1 : 1))))
			{
				...
				slidemo->player->secondjump = false;

				if (slidemo->player->wallrunning > 1)
				...
				P_Thrust(slidemo, slidemo->angle, 5*FRACUNIT); // SRB2CBTODO: What does this do?
			}
		}
	}
...
stairstep:
	...
	// Move a bit to make sure it doesn't hit
	bestslidefrac -= 0x800;
	if (bestslidefrac > 0)
	{
...
void P_SlideMove(mobj_t *mo)
{
	...
	if (mo->player && mo->tracer && mo->tracer->type == MT_SKATEBOARD)
	{
		if (mo->player->itemspeed > 0)
			mo->player->itemspeed /= FRACUNIT;
	}
	else if (mo->target && mo->target->player && mo->type == MT_SKATEBOARD)
	{
		if (mo->target->player->itemspeed > 0)
			mo->target->player->itemspeed /= FRACUNIT;
	}
...
static boolean PTR_AimTraverse(intercept_t *in)
{
...
#ifdef ESLOPE
					if (rover->t_slope)
						topheight = P_GetZAt(rover->t_slope, th->x, th->y);
					if (rover->b_slope)
						bottomheight = P_GetZAt(rover->b_slope, th->x, th->y);
#endif

					highslope = FixedDiv(topheight - shootz, dist);
					lowslope = FixedDiv(bottomheight - shootz, dist);
					if ((aimslope >= lowslope && aimslope <= highslope))
						return false;

					if (lastz > topheight && dir == -1 && aimslope < highslope)
						frontflag |= 0x2;

					if (lastz < bottomheight && dir == 1 && aimslope > lowslope)
						frontflag |= 0x2;
				}
			}
	...
	if (!(th->flags & MF_SHOOTABLE))
		return true; // Non shootable object, don't aim at it

	// friends don't autoaim at friends
	if (aim_nofriends
		&& th->player
		&& (((gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value))
			 && th->player->ctfteam == aim_nofriends)
		|| aim_nofriends == -1))
		return true;
...
fixed_t P_AimLineAttack(mobj_t *t1, angle_t angle, fixed_t distance)
{
	...
	else
	{
		...
		// SRB2's base engine says that at a distance of 160,
...
void P_RadiusAttack(mobj_t *spot, mobj_t *source, int damage)
{
	...
	yh = (unsigned int)(spot->y + dist - bmaporgy)>>MAPBLOCKSHIFT;
	yl = (unsigned int)(spot->y - dist - bmaporgy)>>MAPBLOCKSHIFT;
	xh = (unsigned int)(spot->x + dist - bmaporgx)>>MAPBLOCKSHIFT;
	xl = (unsigned int)(spot->x - dist - bmaporgx)>>MAPBLOCKSHIFT;
	bombspot = spot;
...
// PIT_ChangeSector
//
static boolean PIT_ChangeSector(mobj_t *thing, boolean realcrush) // SRB2CBTODO: Sometimes a null thing is passed here?
{
...
// Delete an entire sector list for precip
...
// Create a list of sectors for precipitation
void P_CreatePrecipSecNodeList(precipmobj_t *thing, fixed_t x, fixed_t y)
{
	...
	xl = (ULONG)(preciptmbbox[BOXLEFT] - bmaporgx)>>MAPBLOCKSHIFT;
	xh = (ULONG)(preciptmbbox[BOXRIGHT] - bmaporgx)>>MAPBLOCKSHIFT;
	yl = (ULONG)(preciptmbbox[BOXBOTTOM] - bmaporgy)>>MAPBLOCKSHIFT;
	yh = (ULONG)(preciptmbbox[BOXTOP] - bmaporgy)>>MAPBLOCKSHIFT;
...
/* cphipps 2004/08/30 -
 * Must clear tmthing at tic end, as it might contain a pointer to a removed thinker,
 or the level might have ended/been ended and we clear the objects it was pointing too.
 Hopefully we don't need to carry this between tics for sync. */
...
// P_FloorzAtPos
// Returns the floorz of the XYZ position // SRB2CBTODO: Need ceilingpos function too
fixed_t P_FloorzAtPos(fixed_t x, fixed_t y, fixed_t z, fixed_t height)
{
	sector_t *sec = R_PointInSubsector(x, y)->sector;
	fixed_t floorz = sec->floorheight;

	// Intercept a 'fall through 3dfloors' bug
	if (sec->ffloors)
	{
		ffloor_t *rover;
...
//
static void P_FakeZMovement(mobj_t *mo) // SRB2CBTODO: Just fix this, start over
{
	fixed_t dist, delta;
	fixed_t zmove;
	...
	if (mo->subsector->sector->ffloors)
	{
		...
		for (rover = mo->subsector->sector->ffloors; rover; rover = rover->next)
		{
			...
			if ((!((((rover->flags & FF_BLOCKPLAYER) && mo->player)
				 ...
				 && !(mo->player && !(mo->player->pflags & PF_NIGHTSMODE)
					  && !mo->player->homing
					  && (((mo->player->charability == CA_SWIM) || mo->player->powers[pw_super])
						  && mo->ceilingz-topheight >= mo->height)
					  && (rover->flags & FF_SWIMMABLE)
					  && !(mo->player->pflags & PF_SPINNING) && mo->player->speed > mo->player->runspeed
			...
			{
				mo->ceilingz = bottomheight; // SRB2CBTODO: ZMOVE
			}
		}
	}
	...
	if (mo->flags & MF_FLOAT && mo->target && mo->type != MT_JETJAW && mo->type != MT_EGGMOBILE
		&& mo->type != MT_EGGMOBILE2 && mo->type != MT_RING && mo->type != MT_COIN)
	{
	...
	// clip movement // This code here handles floor & ceiling collosion,
	// it does not need to be changed for flipped gravity
	// -just saying
	if ((mo->z <= mo->floorz) && !(mo->flags & MF_NOCLIPHEIGHT))
	{
		// Hit the floor, correct the z if it's in the floor
		if (mo->z < mo->floorz)
			mo->z = mo->floorz;
	...
	if ((mo->z + mo->height >= mo->ceilingz) && !(mo->flags & MF_NOCLIPHEIGHT)) // hit the ceiling
	{
		...
		if (mo->flags2 & MF2_SKULLFLY) // the skull slammed into something // SRB2CBTODO: Rename, not skull
			mo->momz = -mo->momz;
	}
}
...
mobj_t *P_CheckOnmobj(mobj_t *thing)
{
	...
	// Exempt ghosts from momentum truncation. // SRB2CBTODO: Is this everywhere in code?

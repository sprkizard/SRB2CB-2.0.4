...
static boolean bigstate; //added : 24-01-98 : moved here, toggle between
...
static ULONG amclock; // SRB2CBTODO: What is this?
...
// based on level size // SRB2CBTODO: SUPPORT THIS
static long min_x;
static long min_y;
static long max_x;
static long max_y;

static long max_w; // max_x-min_x,
static long max_h; // max_y-min_y
...
static long min_scale_mtof; // used to tell when to stop zooming out
static long max_scale_mtof; // used to tell when to stop zooming in
...
/** Calculates the slope and slope according to the x-axis of a line
  ...
  */
static inline void AM_getIslope(const mline_t *ml, islope_t *is) // SRB2CBTODO: Could be used for lines too!
{
	int dx, dy;
	...
	dx = ml->b.x - ml->a.x;
	if (!dy)
		is->islp = (dx < 0 ? -MAXLONG : MAXLONG);
	else
		is->islp = FixedDiv(dx, dy);
	if (!dx)
		is->slp = (dy < 0 ? -MAXLONG : MAXLONG);
	else
...
/** Adds a marker at the current location.
  */
static inline void AM_addMark(void) // SRB2CBTODO:!! Support this
{
...
static void AM_findMinMaxBoundaries(void)
{
	...
	// Kalaron: Support large blockmaps!
	// Find map extents for the blockmap
	min_x = min_y = 2000*FRACUNIT;
	max_x = max_y = -2000*FRACUNIT;
	...
	// Automap has an integer limit, only up to 10,000*FRACUNIT range(that's 655,360,000!)
	// We're doing 4000 here because automap stops drawing after a cetain combination of automap's zoom and large coordinates
	// So with this setup, now automap can draw all maps even while the screen is in max coordinates of -29,000*FRACUNIT to 29,000*FRACUNIT (for x & y)
	if (min_x < -2000*FRACUNIT)
		min_x = -2000*FRACUNIT;
	if (min_y < -2000*FRACUNIT)
		min_y = -2000*FRACUNIT;
	if (max_x > 2000*FRACUNIT)
		max_x = 2000*FRACUNIT;
	if (max_y > 2000*FRACUNIT)
		max_y = 2000*FRACUNIT;
...
static void AM_initVariables(void)
{
	...
	// Find a player to center on initially
...
//
static void AM_LevelInit(void) // SRB2CBTODO: Can cause errors and not display?
{
...
boolean AM_Responder(event_t *ev)
{
	...
	if (devparm || cv_devmode) // SRB2CBTODO: Better automap in all modes
	{
		if (!automapactive)
		{
			if (ev->type == ev_keydown && ev->data1 == AM_STARTKEY)
			{
				...
				{
					bigstate = false; //added : 24-01-98 : toggle off large view
					AM_Start();
					rc = true;
				}
			}
...
static void AM_clearFB(int color)
{
	...
	{
		long dmapx, dmapy, i, y;
		static int mapxstart, mapystart;
		byte *dest = screens[0];
		const byte *src;
		// SRB2CBTODO: What is this?
#define MAPLUMPHEIGHT (BASEVIDHEIGHT - 42)
...
//
// SRB2CBTODO: Use flats and stuff for automap
// (and draw the background of automap as the level's sky and other nice stuff)
...
static inline void AM_drawPlayers(void)
{
	...
	if (!multiplayer)
	{
		...
		//
#define R ((5*FRACUNIT+PLAYERRADIUS*plr->mo->scale/100.0f))
...
#undef NUMPLYRLINES
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		//   starting from the middle.
		//
#define R ((8*PLAYERRADIUS*p->mo->scale/100.0f)/7)
		mline_t player_arrow[] = {
			{ { -R+R/8, 0 }, { R, 0 } }, // -----
			{ { R, 0 }, { R-R/2, R/4 } }, // ----->
			{ { R, 0 }, { R-R/2, -R/4 } },
...
#define R (FRACUNIT*t->scale/100.0f)

...
void Y_Ticker(void)
{
	...
	if (inttype == int_coop) // coop or single player, normal level
	{
		...
		if (data.coop.ringbonus || data.coop.timebonus || data.coop.perfbonus)
		{
			...
			{
				endtic = ((intertic > (5*TICRATE/2)) ? intertic : (5*TICRATE/2)) + 3*TICRATE; // 3 second pause after end of tally
...
static void Y_AwardCoopBonuses(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		// SRB2CBTODO: more conditions for perfect bonus
		if (sharedringtotal && sharedringtotal >= nummaprings
			&& (!mapheaderinfo[gamemap-1].noperfectbns))
		{
...
// Y_DrawScaledNum
//
// Display function for positive numbers.
...
// if you had a cutscene after the final level and things like that.
...
static void Y_FollowIntermission(void)
{
	if (timeattacking)
	{
		...
		I_CreateDirectory(va("%s"PATHSEP"replay", srb2home), 0755);
		I_CreateDirectory(va("%s"PATHSEP"replay"PATHSEP"%s", srb2home, timeattackfolder), 0755);

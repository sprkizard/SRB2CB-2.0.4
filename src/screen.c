...
void SCR_Startup(void)
{
	...
	if (!M_CheckParm("-NOCPUID") && RCpuInfo)
	{
		...
		if (RCpuInfo->CPUs > 1)
			R_ASM = false; // with more than 1 CPU, ASM goes BOOM!
	...
	if (!RCpuInfo || !RCpuInfo->CPUs) // bad CPU ID code?
	{
...
#ifdef CONSCALE
	vid.meddupx = (int)(vid.dupx >> 1) + 1;
	vid.meddupy = (int)(vid.dupy >> 1) + 1;
#ifdef HWRENDER
	vid.fmeddupx = (float)vid.meddupx;
	vid.fmeddupy = (float)vid.meddupy;
#endif
	
	vid.smalldupx = (byte)(vid.dupx / 3) + 1;
	vid.smalldupy = (byte)(vid.dupy / 3) + 1;
#ifdef HWRENDER

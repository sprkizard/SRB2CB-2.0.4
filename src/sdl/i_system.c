...
#include <direct.h>
// M_PI is not always defined,
// 3.14159265358979323846264338327950288 is more precise,
// and some compilers use that instead
#ifndef M_PI
...
// Locations for searching the main data file data.wad
#ifdef _arch_dreamcast
...
*/
#define WADKEYWORD1 "data.dta"
#define WADKEYWORD2 "data.wad"
...
boolean keyboard_started = false;
...
// bring the cursor back to beginning of line
...
// FIXME: // SRB2CBTODO: TTimo needs to position the cursor if needed??
...
//		CONS_Printf("Unable to use that joystick #(%s), non-number\n",fname);
		if (num_joy != 0)
		{
			CONS_Printf("The game couldn't find %d joysticks on this system\n", num_joy);
			for (i = 0; i < num_joy; i++)
				CONS_Printf("#: %d, Name: %s\n", i+1, SDL_JoystickName(i));
		}
		else
			CONS_Printf("The game couldn't find any joysticks on this system\n");
...
tic_t I_GetTime(void)
{
	...
	static Uint64 basetime = 0;
	       Uint64 ticks = timer_ms_gettime64(); //use timer_ms_gettime64 instead of SDL_GetTicks for the Dreamcast
#else
...
//
// I_GetScaledTime
// returns time in 1/TICRATE*scale second tics
// for stuff like display that needs to update more
//
tic_t I_GetScaledTime(float scale) // SRB2CBTODO: Need windows version
{
#ifdef _arch_dreamcast
	static Uint64 basetime = 0;
	Uint64 ticks = timer_ms_gettime64(); //use timer_ms_gettime64 instead of SDL_GetTicks for the Dreamcast
...
void I_Quit(void)
{
	...
	quiting = SDL_FALSE;
	I_ShutdownConsole();
	M_SaveConfig(NULL); // Save game config, console vars, etc.
#ifndef NONET
...
void I_Error(const char *error, ...) // SRB2CBTODO: Make this ALWAYS print out a message into the console log
{
	va_list argptr;
	...
	if (shutdowning)
	{
		...
		if (errorcount > 20)
		{
			...
			Mac_ShowAlert("Recursive Error", buffer, "Quit", NULL, NULL);
	...
	va_end(argptr);
#endif
	M_SaveConfig(NULL); // Save game config, console variables, etc..
#ifndef NONET
	D_SaveBan(); // save the ban list
#endif
	G_SaveGameData(); // Save any game sata

	// Start the game shut down
	if (demorecording)
		G_CheckDemoStatus();
	...
	Mac_ShowAlert("Critical Game Error", buffer, "Quit", NULL, NULL); // SRB2CBTODO: RENAME?
#endif
	W_Shutdown();
#if defined (PARANOIA) && defined (__CYGWIN__)
		*(int *)2 = 4; // Debug! // SRB2CBTODO: REMOVE, WHY IS IT HERE???
#endif
#ifdef GP2X
	chdir("/usr/gp2x");
	execl("/usr/gp2x/gp2xmenu", "/usr/gp2x/gp2xmenu", NULL);
...
int I_CreateDirectory(const char *dirname, int unixright)
{
...
/**	\brief	search for data.dta in the given path
...
/**	\brief go through all possible paths and look for data.dta

  \return path to data.dta if any
*/
...
const char *I_LocateWad(void)
{
	...
	if (waddir)
	{
		// change to the directory where we found data.dta
#if (defined (_WIN32) && !defined (_WIN32_WCE)) && !defined (_XBOX)

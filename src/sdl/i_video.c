...
// Mac OS X does not use an XPM image icon change
#if !defined (HAVE_IMAGE) && !(defined (DC) || defined (_WIN32_WCE) || defined (PSP) || defined(GP2X) || defined(__APPLE_CC__))
#define LOAD_XPM // Loads XPM files!
#include "IMG_xpm.c" // Custom small and simple XPM image code 
#define HAVE_IMAGE // SDL_Image for XPM is sort of obtained here
#endif
...
// Set the default render mode: OpenGL(render_opengl) or Software(render_soft)
...
boolean graphics_started = false; // Is used in console.c and screen.c
...
static int windowedModes[MAXWINMODES][2] = // SRB2CBTODO: What are these ratios?
{
#if !(defined (_WIN32_WCE) || defined (DC) || defined (PSP) || defined (GP2X))
#ifdef HDRES
	{MAXVIDWIDTH /*2560*/, MAXVIDHEIGHT/*1600*/}, // 1.60, 8.00
#else
...
static int SDLatekey(SDLKey sym)
{
	...
	switch (sym)
	{
		...
		case SDLK_KP_EQUALS: // Alam & Logan: What? Mac KB haves one! XD
		case SDLK_PAUSE:
			rc = KEY_PAUSE;
			break;
...
static void VID_Command_Info_f(void)
{
	...
	if (videoInfo)
	{
		...
		if (videoInfo->blit_fill)
			CONS_Printf(" Accelerated Color filling\n");
		//UnusedBits3  :16
...
void I_GetEvent(void) // SRB2CBTODO: Why is key input in the video file?!
{
	...
	while (SDL_PollEvent(&inputEvent))
	{
		...
		switch (inputEvent.type)
		{
			case SDL_ACTIVEEVENT:
				...
				{
					// Pause music when the player alt-tab's
					if (inputEvent.active.gain /*&& !paused */)
					{
						...
						if (cv_pauseresume.value)
						{
							if (cv_inactivepause.value == true)
							{
						...
						if (cv_inactivemusic.value == false)
						{
							if (gamestate == GS_LEVEL || gamestate == GS_DEMOSCREEN)
								if (!paused)
									I_ResumeSong(0); // Resume the music if it was paused
						}
					}
					else /*if (!paused)*/
					{
						...
						{
							if (gamestate == GS_LEVEL || gamestate == GS_DEMOSCREEN)
								I_PauseSong(0); // Pause the current song
						}
					}
				}
				if (MOUSE_MENU)
			...
			case SDL_MOUSEBUTTONUP:
				...
				if (USE_MOUSEINPUT)
				{
					...
					{
						if (inputEvent.type == SDL_MOUSEBUTTONUP)
							event.data1 = 0; // SRB2CBTODO: This could be a real button with some mice
						else
	...
	// Reset wheel like in win32 // SRB2CBTODO: How does this work?
	gamekeydown[KEY_MOUSEWHEELDOWN] = gamekeydown[KEY_MOUSEWHEELUP] = 0;
	
	// Caps lock support, it's important it's after the key events so that
	// SDL_GetModState can work
	int temp = SDL_GetModState(); 
	temp = (temp & KMOD_CAPS);
	
	if(temp == KMOD_CAPS) // SRB2CBTODO: Capslock for windows
		capslock = true;
	else
		capslock = false;
}
...
#ifdef HWRENDER
	if (rendermode == render_opengl) // So simple for OpenGL :P
		OglSdlFinishUpdate(cv_vidwait.value);
#endif
	else if (rendermode == render_soft && screens[0])
	{
		...
		if (f2xSurface) // Alam: filter!
		{
			...
			else if (vid.bpp == 1)
			{
				...
				if (!vidformat || vidSurface)
					I_Error("SDL vidformat error\n");
...
// I_UpdateNoVsync
//
void I_UpdateNoVsync(void) // SRB2CBTODO: Is this needed? A better manual method could be used
{
	boolean real_vidwait = cv_vidwait.value;
	cv_vidwait.value = false;
	I_FinishUpdate();
...
int VID_SetMode(int modeNum)
{
	...
	if (rendermode == render_soft)
	{
		...
		vid.direct = NULL; // Do not directly access memory in drawing
	...
	// The display is always stretched to fit
#endif
	I_StartupMouse();
...
void I_StartupGraphics(void)
{
	...
	// Allocate space for the video name
	char vd[100];
	CONS_Printf("Starting up with video driver : %s\n", SDL_VideoDriverName(vd,100));
	
	// Make sure that software mode can still be used if a parameter is called
	if (M_CheckParm("-software"))
		rendermode = render_soft;
	
	// Setup keyboard things
	SDL_EnableKeyRepeat(SDL_DEFAULT_REPEAT_DELAY>>1,SDL_DEFAULT_REPEAT_INTERVAL<<2);
	SDLESSet();
	
	// Setup window video parameters
	...
	// Setup the game's window title
	SDL_WM_SetCaption("SRB2CB: Starting up...", "SRB2CB");

	// Setup the game's window icon
#ifdef HAVE_IMAGE
	// SRB2CBTODO: Icon check on other OS's
	#ifndef __APPLE__ // Don't change the Mac OS X icon
		icoSurface = IMG_ReadXPMFromArray(SDL_icon_xpm);
		SDL_WM_SetIcon(icoSurface, NULL);
	#endif
#endif

	// Setup OpenGL video mode
#ifdef HWRENDER
	if (M_CheckParm("-opengl") || rendermode == render_opengl)
	{
		...
		// load up the OpenGL system, OpenGL is already compiled with the game itself,
		// so this is really a paranoid check to make sure the game does not crash
		// Setting up the OpenGL data can ONLY be done on program load
		// IMPORTANT: SRB2CBTODO: Sets up core stuff, for things like accumulation buffer and stuff
		if (GL_Init(I_Error))
		{
		/*
		 ...
		 */
			...
			// SRB2CBTODO: This greatly affects how well the engine can 
			// handle the detail on far away polygons,
			// we ideally want 32 for the game,
			// asking for more than a computer has is ok, it will just use the maximum on that machine
			// The higher this buffer is, the less the game must worry about
			// polygons visibly overlapping which are drawn far away,
			// but at the same time, the game needs to be more careful about any polygons that overlap
		...
		{
			I_OutputMsg("The OpenGL renderer failed to load\n The game will startup in software mode.");
			rendermode = render_soft; // Just in case of any errors at all
		}
	}
#else
	rendermode = render_soft; // Force software mode when there no HWRENDER code, the old render code
#endif
	if (rendermode == render_soft)
	{
...
{
	// Rendermode is about to be cleared, so save it just for this function

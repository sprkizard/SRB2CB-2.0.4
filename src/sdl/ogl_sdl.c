...
boolean OglSdlSurface(int w, int h, boolean isFullscreen)
{
...
#ifdef _WIN32
	// Disable an advanced feature that might not work on some hadware
	// Now works on G400 with bios 1.6 and certified drivers 6.04
	if (strstr(glrenderer, "810"))
		oglflags |= GLF_NOZBUFREAD;
#elif defined (unix) || defined (UNIXLIKE)
	// Disable advanced features that don't work on some hardware
	if (strstr(glrenderer, "G200"))
		oglflags |= GLF_NOTEXENV;
	if (strstr(glrenderer, "G400"))
		oglflags |= GLF_NOTEXENV;
#endif
	...
	granisotropicmode_cons_t[1].value = anisotropy;
...
void GL_SetPalette(RGBA_t *palette, RGBA_t *pgamma)
{
	int i = -1;
	byte redgamma = pgamma->s.red, greengamma = pgamma->s.green,
		bluegamma = pgamma->s.blue;

	if (i == 0) redgamma = greengamma = bluegamma = 0x7F;
	for (i = 0; i < 256; i++)
	{
...
#endif
	// When the palette is changed, all textures must be flushed to see changes
	GL_Flush(); // SRB2CBTODO: Palette doesn't always work (works on flats only?)
}

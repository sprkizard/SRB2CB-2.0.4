...
static void I_SetChannelParams(chan_t *c, int vol, int sep, int step)
{
	...
	else if (c->samplerate != (unsigned int)audio.freq)
	...
	// then there would be no "dynamic room" for other sounds :-/
...
FUNCINLINE static ATTRINLINE void I_UpdateStream8S(Uint8 *stream, int len)
{
	...
	while (len--)
	{
		...
		// Love the L2 cache - made this a loop.
...
static boolean LoadSong(void *data, size_t lumplength, size_t selectpos)
{
	...
	if (canuseRW)
	{
		...
		free(olddata); // Free old memory, do not use Z_ because it isn't stored with Zone - NEWMEM
		...
		music[selectpos] = Mix_LoadMUS_RW(SDLRW); // New Mix_Chuck from RWops // SRB2CBTODO: Causes an error sometimes
		if (music[selectpos])
...
#else
	CONS_Printf("Error: No MIDI support\n");
	(void)len;
	(void)data;
#endif
	return false;
...
boolean I_StartDigSong(const char *musicname, int looping) // SRB2CBTODO: Pass MIDI's to SDL the same way as a digital song
{
#ifdef HAVE_MIXER
...
#ifdef FMODSOUND
	if (nofmodmusic)
		return false; // There is no music or FMOD is not loaded

	if ((!fmus || !fmod375->FSOUND_IsPlaying(fsoundchannel)) && (!mod || !fmod375->FMUSIC_IsPlaying(mod)))
		return false; // There is no FMOD music playing

	if (speed == 0.0f)
		return true; // The music speed can be changed
	if (speed > 250.0f)
		speed = 250.0f; // Limit the speed up to 250x

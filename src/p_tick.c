...
void P_RemoveThinkerDelayed(void *pthinker)
{
	...
	if (!thinker->references)
	{
		...
		Z_Free(thinker); // NEVER EVER TOUCH THIS! (otherwise lots of crashes ensue)
    }
}
 ...
 * Now, we keep a count of the number of
...
// load the "next" pointer prior to freeing the node.
//
...
void P_Ticker(void)
{
	...
	// SRB2CBTODO: Slow motion, per mobj/level special for cool effects later
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		if (playeringame[i] && players[i].mo)
		{
			...
			{
				JB_BotThink(i);
				P_BotThink(i); // P_BotThink works when the bot is alive, reborn or dead
#ifdef BOTWAYPOINTS
				JB_UpdateWaypoints();
#endif
				continue;
		...
		if ((leveltime % TICRATE == 0) && (leveltime > hidetime*TICRATE))
		{
			...
			for (i = 0; i < MAXPLAYERS; i++)
			{
				if (!(players[i].pflags & PF_TAGIT) && !(players[i].pflags & PF_TAGGED)
					...
					P_AddPlayerScore(&players[i], (D_NumPlayers() - spectators) / 2); // Points given is the number of participating players divided by two. // SRB2CBTODO: WHAT?!!
			}
		}
	}
	...
	if (countdowntimer)
	{
		...
		if (countdowntimer <= 0)
		{
			...
			for (i = 0; i < MAXPLAYERS; i++)
			{
				...
				P_DamageMobj(players[i].mo, NULL, NULL, 10000); // SRB2CBTODO: Make a better param to "kill" something
			}
		}
	}

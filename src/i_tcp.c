...
static void SOCK_Get(void)
{
	...
	if (c == ERRSOCKET)
	{
		...
		else if (errno == WSAECONNRESET) // 2k has some extra errors
		{
			...
			return;
			/// \todo // SRB2CBTODO: see if the D_QuitNetGame actually fixes it, or whether it crashes or something
			/// Alam_GBC: this WSAECONNRESET happends alot when talking to a masterlist server, i am guess when talking too much at a time
			/// Later, SIO_UDP_CONNRESET turned off should fix this
		}
#endif
	...
	if (j > 0)
	{
		...
		// check if it's a banned okayer so we can send a refusal later
		for (i = 0; i < numbans; i++)
		{
...
static SOCKET_TYPE UDP_Socket(void)
{
...
#ifdef _WIN32
	{
		ULONG falseval = false; // Alam_GBC: disable the new UDP connection reset behavior for Win2k and up
	...
	// SRB2CBTODO: Figure out this stuff!
	...
	if (M_CheckParm("-clientport"))
	{
		...
		sock_port_local = (USHORT)atoi(M_GetNextParm());
	}
	else
		...
		if (WSAresult != 0)
		{
			...
			switch (WSAresult)
			{
				...
				case WSAEPROCLIM:
					...
					break;
				case WSAEFAULT:
					WSError = "Strange Error! The WSAData is not a valid pointer? What kind of setup do you have?";
					break;
				default:
...
static signed char SOCK_NetMakeNode(const char *hostname)
{
	...
	char *portchar;
	USHORT portnum = htons(sock_port);
	...
	if (portchar)
		portnum = htons((USHORT)atoi(portchar));
	free(localhostname);
...
boolean I_InitTcpNetwork(void)
{
	...
	{
		if (M_IsNextParm())
			sock_port = (USHORT)atoi(M_GetNextParm());
		else
			sock_port = 0;
	}

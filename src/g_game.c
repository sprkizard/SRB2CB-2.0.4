...
boolean playercontinuing = false;
...
mobj_t *skyboxmobj = NULL;
mobj_t *skyboxcentermobj = NULL;
boolean useskybox = false;
boolean moveskybox = false;
...
// Mystic's funky SA ripoff emblem idea // SRB2CBTODO: Use this for something, like a hashed text file instead to load into the game?
...
int underwatertics = 32*TICRATE; // Characters can hold their breath for almost a minute
tic_t spacetimetics = 12*TICRATE;
...
/** Clips the console player's mouse aiming to the current view.
  ...
  */
short G_ClipAimingPitch(int *aiming) // This is only for when chasecam is off
{
	int limitangle;

	// SRB2CBTODO: Vphysics: support slopes for max angle limits
	// NOTE: The old renderer: software mode, doesn't have true perspective
	if (rendermode == render_soft)
	{
		// Limit software mode so it doesn't show a visible band
		// when looking too far up or down
		limitangle = ANGLE_60;
	}
	else
		limitangle = ANG90 - 1;
...
void G_BuildTiccmd(ticcmd_t *cmd, int realtics)
{
	...
	if (cv_analog.value && !cv_objectplace.value) // Analog
	{
		if (turnright)
	...
	if (strafe || (cv_analog.value && !cv_objectplace.value) || twodlevel
	...
	if ((gamekeydown[gamecontrol[gc_camleft][0]] ||
		gamekeydown[gamecontrol[gc_camleft][1]]) && (cv_devmode || cv_analog.value || cv_objectplace.value)) // SRB2CBTODO: allow this with analog off, but set the camera to tilt back when moving
		cmd->buttons |= BT_CAMLEFT;

	if ((gamekeydown[gamecontrol[gc_camright][0]] ||
		gamekeydown[gamecontrol[gc_camright][1]]) && (cv_devmode || cv_analog.value || cv_objectplace.value))
	...
	if (mouseaiming)
	{
		...
		// Look up/down, reverse adding/subtracting if
		// the screen is inverted
		if (players[consoleplayer].mo
			&& players[consoleplayer].mo->eflags & MFE_VERTICALFLIP
			&& !cv_chasecam.value ? !cv_invertmouse.value : cv_invertmouse.value)
	...
	if (strafe || (cv_analog.value && !cv_objectplace.value)
		|| players[consoleplayer].climbing
...
#ifdef AWESOME // // SRB2CBTODO: Since we really flip, also flip the turning
	if (players[consoleplayer].mo
		//&& players[consoleplayer].mo->eflags & MFE_VERTICALFLIP
		//&& !cv_chasecam.value
		&& viewroll > ANG90 // The viewroll should adjust to the player's camera, find a more specific way to check?
		// SRB2CBTODO: things like viewroll should be copied to the player struct for proper reference
		)
		// SRB2CBTODO: This should work in multiplayer mode too!
	...
	if ((cmd->forwardmove || cmd->sidemove || cmd->buttons) // SRB2CBTODO: Support singleplayer?
...
void G_BuildTiccmd2(ticcmd_t *cmd, int realtics)
{
	...
	// let movement keys cancel each other out
	if (cv_analog2.value && !cv_objectplace.value) // Analog
	{
		if (turnright)
			cmd->angleturn = (short)(cmd->angleturn - angleturn[tspeed]);
		if (turnleft)
	...
	if (strafe || (cv_analog2.value && !cv_objectplace.value) || twodlevel
	...
	if ((gamekeydown[gamecontrolbis[gc_camleft][0]] ||
		gamekeydown[gamecontrolbis[gc_camleft][1]]) && (cv_devmode || cv_analog2.value || cv_objectplace.value))
		cmd->buttons |= BT_CAMLEFT;

	if ((gamekeydown[gamecontrolbis[gc_camright][0]] ||
		gamekeydown[gamecontrolbis[gc_camright][1]]) && (cv_devmode || cv_analog2.value || cv_objectplace.value))
		cmd->buttons |= BT_CAMRIGHT;
	...
	if (strafe || (cv_analog2.value && !cv_objectplace.value)
		|| players[secondarydisplayplayer].climbing
...
static void Analog_OnChange(void)
{
	...
	if (netgame || !cv_chasecam.value) // SRB2CBTODO: WHY?
		cv_analog.value = 0;
}
...
void G_DoLoadLevel(boolean resetplayer)
{
	...
	if (!(cv_devmode || devparm || modifiedgame) && !(multiplayer || netgame))
		SetSavedSkin(0, players[0].skin, players[0].prefcolor);

#ifdef JTEBOTS
	JB_AddWaitingBots(consoleplayer); // SRB2CBTODO: May cause crash on reload?
#endif
}
...
boolean G_Responder(event_t *ev)
{
	...
	{
		if (!cv_devmode && gametype != GT_COOP && gametype != GT_RACE && gametype != GT_CTF && gametype != GT_MATCH && gametype != GT_TAG)
		{
			displayplayer = consoleplayer;
		}
		else
		{
			...
			// tell who's the view // SRB2CBTODO: Isn't this already done?
...
static inline void G_PlayerFinishLevel(int player)
{
	...
	p->mo->flags2 &= ~MF2_TRANSLUCENT; // cancel invisibility
	p->bonuscount = 0;
	p->starpostangle = 0;
	p->starposttime = 0;
	p->starpostx = 0;
...
#ifdef JTEBOTS
	p->bot = bot; // SRB2CBTODO: Possibly merge JTEBots with LXShadows?
#endif
...
//
static boolean G_CheckSpot(int playernum, mapthing_t *mthing) // SRB2CBTODO: Use this elsewhere!
{
	fixed_t x;
	fixed_t y;
	subsector_t *ss;
...
startdeath:
	...
	if (!playerstarts[0])
		I_Error("Not enough starts in this map!\n"); // SRB2CBTODO: Make it go to a console/menu only screen instead
...
#ifdef JTEBOTS // Bots should not reload a level
		&& !player->bot
#endif
		))
	{
		...
		if (mapheaderinfo[gamemap-1].noreload && !playercontinuing && !timeattacking)
		{
			int i;
			...
			G_DoLoadLevel(true); // SRB2CBTODO: Shouldn't reload EVERY thing
...
{
	//if (!multiplayer && !(nextmapgametype != -1))
	//	return TOL_SP; // SRB2CBTODO: FIX THIS
...
/** Select a random map with the given typeoflevel flags.
  ...
  * \author Graue <graue@oceanbase.org>
  */
static short RandMap(short tolflags, short pprevmap) // SRB2CBTODO: Make sure that prevmap is not checked if there is only one map!
{
	...
	for (ix = 0; ix < NUMMAPS; ix++)
		if ((mapheaderinfo[ix].typeoflevel & tolflags) == tolflags && (ix != pprevmap))
			okmaps[numokmaps++] = ix;

	if (numokmaps == 0)
		return 1; // None match, default to map1
...
void G_DoCompleted(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
		if (playeringame[i]
#ifdef JTEBOTS // Allow bots to finish levels? WARNING: May cause crashes?
		//	&& (!players[i].bot && gametype == GT_COOP)
#endif
			)
	...
	if (!token && !G_IsSpecialStage(gamemap))
	{
		...
		{
			tolflag = TOLFlag(nextmapgametype);
			CONS_Printf("next gametype is different..\n");
		}
		else
			tolflag = TOLFlag(gametype);
			...
			&& !(mapheaderinfo[nextmap].typeoflevel & tolflag))
		{
			...
			while (!(mapheaderinfo[cm].typeoflevel & tolflag))
			{
				...
				if (visitedmap[cm/8] & (1<<(cm%8))) // smells familiar
				{
					...
					CONS_Printf(PREFIX_WARN "Can't find a "
...
skipit:
	...
	if (G_IsSpecialStage(gamemap) && !gottoken)
		nextmap = lastmap; // Exiting from a special stage? Go back to the game.
...
void G_ModifyGame(void)
{
	// Devmode access to all commands without changing the modifying of the game,
	// the game is modified when switching on devmode,
	// unless the game has been compiled in debug
	if (cv_devmode)
		return;
...
// G_LoadGameSettings
//
// Sets a tad of default info we need. // SRB2CBTODO: Better method needed?
...
// Load a game only to check to see if its valid, true if it's bad
boolean G_CheckGame(unsigned int slot)
{
	size_t length;
	char vcheck[VERSIONSIZE];
	char savename[255];
	...
	if (!length)
	{
		M_StartMessage(va("Can't read save slot %u\nPress ESC/Enter\n", slot), NULL, MM_NOTHING);
		return true;
	}
...
void G_InitNew(boolean pultmode, const char *mapname, boolean resetplayer, boolean skipprecutscene)
{
	...
	if (resetplayer)
	{
		...
		if (!(cv_devmode || devparm) && !(multiplayer || netgame))
			SetSavedSkin(0, players[0].skin, players[0].prefcolor);
	}
	...
	// SRB2CB, always do this, it's very helpful
	{
		const int actnum = mapheaderinfo[gamemap-1].actnum;
		CONS_Printf("%s", G_BuildMapName(gamemap));

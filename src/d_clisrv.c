...
void SendNetXCmd(netxcmd_t id, const void *param, size_t nparam)
{
	...
	if (localtextcmd[0]+2+nparam > MAXTEXTCMD)
	{
		...
		if (cv_devmode) // If you're not in devmode, it just ain't gonna happen...
...
#ifdef JTEBOTS // SRB2CBTODO: Possible for a bot to not have an owner for things like Riders mode?
	if (!players[playernum].bot && server)
	{
		int i;

		// Remove the player's bots 'cause they're not around anymore.
		for (i = 0; i < MAXPLAYERS; i++)
			if (playeringame[i] && players[i].bot && players[i].bot->ownernum == playernum)
				COM_BufAddText(va("kick %d",i));
	}
#endif
...
#ifdef JTEBOTS
	INT32 i;
	i = buf[0] = nametonum(COM_Argv(1));

	if (!stricmp(COM_Argv(1),"allbots"))
	{
		for(i = 0; i < MAXPLAYERS; i++)
			if (players[i].bot)
			{
...
static void Got_KickCmd(UINT8 **p, INT32 playernum)
{
	...
	if (playernum != serverplayer && playernum != adminplayer
		...
		&& !(playeringame[pnum]
			 && players[pnum].bot
			 && players[pnum].bot->ownernum == playernum)
#endif
...
#ifdef JTEBOTS
			if (players[pnum].bot)
				CONS_Printf("(A CPU player) has been removed\n");
			else
#endif
...
#ifdef JTEBOTS
			if (nodetoplayer[n] == (signed char)n && playeringame[n] && players[n].bot)
				supposedtics[n] = nettics[n] = lasttictosend;
#endif
...
void TryRunTics(tic_t realtics) // This is the core function that handles server and game updating
{
	...
	GetPackets(); // This is where the tics start

	if (neededtic > gametic) // ZTODO: This is the gateway to uncapped framerate
	{
		if (advancedemo)
			D_StartTitle();
		else

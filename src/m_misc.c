...
void M_SaveConfig(char *filename)
{
	...
	// Print the header message, a total of 4 lines, seprated just for better code spacing
	const char header1[] = "// SRB2CB configuration file,\n// all values here are able to be edited.\n";
	const char header2[] = "// If a value changed in this file is not recognized by the game,\n";
	const char header3[] = "// the value will be changed to the default setting when the game reloads.\n";
	const char header4[] = "// UTF-8 characters (Japanese, Chinese, etc.) are currently unsupported\n";
...
{
	static char freename[13] = "shotXXXX.ext";
...
#ifdef HAVE_PNG
FUNCNORETURN void PNG_error(png_structp PNG, png_const_charp pngtext)
{
	I_Error("Error: libpng error at %p: %s\n", PNG, pngtext);
}

void PNG_warn(png_structp PNG, png_const_charp pngtext)
{
	CONS_Printf("WARNING: libpng warning at %p: %s\n", PNG, pngtext);
}
...
#define SRB2PNGTXT 8
	...
	char titletxt[] = "SRB2CB"VERSIONSTRING;
	png_charp authortxt = I_GetUserName();
	png_charp playertxt =  cv_playername.zstring;
	char desctxt[] = "SRB2CB Screenshot";
	char Movietxt[] = "SRB2CB Movie";
	size_t i;
	char interfacetxt[] =
#ifdef SDL
	"SDL";
...
static inline boolean M_PNGfind_acTL(void)
{
	    ...
	    )
	{
		...
		ftell(apng_FILE); // (curpos) then , [0-2^31] bytes for data, then 4 bytes for CRC
...
typedef struct
{
	...
	char filler[58];
	byte data; ///< Unbounded; used for all picture data.
} pcx_t;
...
static boolean WritePCXfile(const char *filename, const byte *data, int width, int height, const byte *palette)
{
	...
	pcx->palette_type = SHORT(1); // not a gray scale
...
failure:
	if (ret)
	{
		...
		// SRB2CB: Don't cover up the screenshot with console lines!
		// that stops a user from taking consecutive pictures!
		// Only display something if there's a problem
	}
	else
	{
		if (freename)
...
void M_PNGHeightWidth(const char *filename, int *w, int *h)
{
	png_structp png_ptr;
	png_infop png_info_ptr;
...
static /*FUNCTARGET("sse2")*/ void *sse_cpy(void * dest, const void * src, size_t n)
{
	...
	if (n >= MIN_LEN)
	{
		register ULONG int delta;
		/* Align destinition to MMREG_SIZE -boundary */
		delta = ((ULONG int)dest)&(SSE_MMREG_SIZE-1);
		if (delta)
		{
...
static FUNCTARGET("mmx") void *mmx2_cpy(void *dest, const void *src, size_t n)
{
	...
	if (n >= MIN_LEN)
	{
		register ULONG int delta;
		/* Align destinition to MMREG_SIZE -boundary */
		delta = ((ULONG int)dest)&(MMX_MMREG_SIZE-1);
		if (delta)
		{

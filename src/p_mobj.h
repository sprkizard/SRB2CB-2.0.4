...
typedef enum
{
	...
	// NOTE: Because something has no sector,
	// it can not be called on by its thinker,
	// do not use this on things that should be displayed
	MF_NOSECTOR         = 0x0008,
	// Don't use the blocklinks (inert but displayable)
	MF_NOBLOCKMAP       = 0x0010,
	// Not to be activated by sound, deaf monster.
	MF_AMBUSH           = 0x0020, // SRB2CBTODO: Different name, deaf?
	...
	// NOTE: The object will be inert and not automatically dismissed
	MF_NOTHINK          = 0x200000,
	...
	// The MF_TRANSSHIFT flag merely signals to use the proper drawing function for a mobj's color.
	// Since this got changed from containing the color in the flags itself,
	// some FREE FLAGS open up: 0x8000000 0x10000000 and 0x20000000
...
typedef enum
{
	...
	MF2_TWOSEVEN       =     0x00002000, // Special analog control in a level
	...
	MF2_TRANSLUCENT    =     0x01000000, // Preset semitransparency for an object
	MF2_STANDONME      =     0x02000000, // While not pushable, stand on me anyway.
	MF2_SUPERFIRE      =     0x04000000, // Firing something with Super Sonic-stopping properties.
										 // Or, if mobj has MF_MISSILE, this is the actual fire from it.
...
typedef enum
{
	...
	// (instant damage in lava/slime sectors to prevent jump cheat..) // SRB2CBTODO: Spikes need this?
	MFE_JUSTHITFLOOR      = 2,
	...
	MFE_JUSTSTEPPEDDOWN   = 16,  // SRB2CBTODO: Find a use!
	// Vertically flip sprite/allow upside-down physics
	MFE_VERTICALFLIP      = 32,
	// Just bounced off something
	MFE_JUSTBOUNCEDBACK   = 64,
	// Extra flags for any mobj
	MFE_EXTRAFLAG         = 128,
	MFE_EXTRAFLAG2        = 256
} mobjeflag_t;
...
typedef struct mobj_s
{
	...
	byte color; // Use with MF_TRANSLATION
...
#ifdef SPRITEROLL
	angle_t rollangle; // current roll angle, use for logic/drawing
	angle_t destrollangle; // angle that we want
	byte rollspeed;	// speed to rotate to destrollangle
#endif
	
#ifdef VPHYSICS
	angle_t pitchangle; // What? Oh YES!
	// roll = rollangle
	// yaw = angle
#endif
	
	v3float_t vector; // VPHYSICS: WHAT? YESH
...
void P_SecretMove(mobj_t* mo); // For LXShadow
...
void P_SpawnMobjHoop(fixed_t x, fixed_t y, fixed_t z, fixed_t radius, int number,
							mobjtype_t type, angle_t rotangle);
void P_SpawnPrecipitation(void);
void P_SpawnParaloop(fixed_t x, fixed_t y, fixed_t z, fixed_t radius, int number,
					 mobjtype_t type, angle_t rotangle, boolean spawncenter, boolean ghostit);
...
fixed_t P_ScaleMomentum(fixed_t momentum, USHORT scale);
void P_XYMovement(mobj_t *mo);
void P_EmeraldManager(void);

void P_Particles(mobj_t* source, mobjtype_t type, byte frequency, fixed_t speed, fixed_t zspeed,
				 int alive, boolean pgravity, boolean addmomentum, byte color, byte repeat, int particlesprite);
void P_SpiralParticles(mobj_t* source, fixed_t speed, fixed_t zspeed, int alive, boolean pgravity, byte color,
					   int particlesprite, mobjtype_t type);
void P_BalloonParticles(mobj_t* source, mobjtype_t type, byte frequency, fixed_t speed, fixed_t zspeed, int alive,
						boolean pgravity, boolean addmomentum, byte color, byte repeat);

#define MAXHUNTEMERALDS 64 // SRB2CBTODO: More emerald locations?
extern mapthing_t *huntemeralds[MAXHUNTEMERALDS];
// Because of P_Random, only up to 255 possible locations are allowed,
// this can be changed but 255 possible things to be found seems to be more then plenty
extern byte numhuntemeralds;
extern boolean runemeraldmanager;
extern int numstarposts; // SRB2CBTODO: THIS SHOULD BE BYTE/size_t!
#endif

                                ...
                                byte flipped)
{
	...
	// lumppat(lump patch) & lumpid (lump ID) are the same for SRB2 when unmodified, but different
...
static void R_RemoveSpriteLump(USHORT wad,            // graphics patch
                               ...
                               byte flipped)
{
	(void)wad; /// \todo: how do I remove sprites? // SRB2CBTODO:
	(void)lump;
	(void)lumpid;
	(void)frame;
	(void)rotation;
...
static boolean R_AddSingleSpriteDef(const char *sprname, spritedef_t *spritedef, USHORT wadnum, USHORT startlump, USHORT endlump)
{
	...
	for (l = startlump; l < endlump; l++)
	{
		if (memcmp(lumpinfo[l].name,sprname,4)==0)
		{
			...
			// SRB2CBTODO: FIXME : numspritelumps do not duplicate sprite replacements
			...
			if (++numspritelumps >= MAXSPRITELUMPS)
				I_Error("R_AddSingleSpriteDef: too many sprite replacements (numspritelumps)\n");

			++numadded;
		}
	}

	// Check if no frames were found for this sprite
	if (maxframe == (size_t)-1)
	{
		...
		// SRB2CBTODO: Is this commented code no longer needed?
	...
	// Some checks to ease catching errors with wads
...
void R_AddSpriteDefs(USHORT wadnum)
{
	...
	if (start == MAXSHORT)
		start = 0; // let's say S_START is lump 0
	else
	...
	CONS_Printf("%s added %hd frame%s in %lu sprite%s\n", wadname, end-start, (end-start > 1) ? "s" : "", (ULONG)addsprites, (addsprites > 1) ? "s" : "");
}
	...
	for (; column->topdelta != 0xff ;)
	{
		...
		if (dc_yl <= dc_yh && dc_yl < vid.height && dc_yh > 0)
		{
					 ...
					 )
			{
				...
				if (first)
				{
					CONS_Printf(PREFIX_WARN "avoiding a crash in %s %d\n", __FILE__, __LINE__);
					first = 0;
				}
			}
		}
...
static void R_DrawVisSprite(vissprite_t *vis)
{
	...
	// JTE: SOMEWHERE THIS ISN'T RESETTING PROPERLY. FIX IT OR LEAVE THIS HERE.
	colfunc = basecolfunc;
	...
	else if (vis->mobjflags & MF_TRANSLATION)
	{
		...
		// New colormap stuff for skins
		if (vis->mobj->player && vis->mobj->sprite == SPR_PLAY) // This thing is a player!
		{
			if (vis->mobj->player->skincolor)
				dc_translation = translationtables[vis->mobj->player->skin] - 256 + ((long)vis->mobj->color<<8);
			else
			{
				...
				if (firsttime)
				{
					CONS_Printf("MF_TRANSLATION: Error with color translation!\n");
					firsttime = 0;
				}
			}
		}
	...
	if (vis->mobj->scale > 400) // Software can't handle a scale greater than 400%
	{
		if (vis->mobjflags & MF_HIRES)
		{
			spryscale = (spryscale >> 1)*400/100;
			vis->scale = (vis->scale >> 1)*400/100;
			dc_iscale = (dc_iscale << 1)*100/400;
			vis->xiscale = (vis->xiscale << 1)*100/400;
		}
		else
		{
			spryscale = spryscale*400/100;
			vis->scale = vis->scale*400/100;
			dc_iscale = dc_iscale*100/400;
			vis->xiscale = vis->xiscale*100/400;
		}
		dc_hires = 1;
	}
	else if (vis->mobj->scale != 100)
	{
		if (vis->mobjflags & MF_HIRES)
		{
			spryscale = (spryscale >> 1)*vis->mobj->scale/100;
			vis->scale = (vis->scale >> 1)*vis->mobj->scale/100;
			dc_iscale = (dc_iscale << 1)*100/vis->mobj->scale;
			vis->xiscale = (vis->xiscale << 1)*100/vis->mobj->scale;
		}
		else
		{
	...
	else if (vis->mobjflags & MF_HIRES)
	{
		spryscale = spryscale >> 1;
		vis->scale = vis->scale >> 1;
		dc_iscale = dc_iscale << 1;
		vis->xiscale = vis->xiscale << 1;
		dc_hires = 1;
	}
	...
	for (dc_x = vis->x1; dc_x <= vis->x2; dc_x++, frac += vis->xiscale)
	{
		...
		if (texturecolumn < 0 || texturecolumn >= SHORT(patch->width))
			I_Error("Error: Software mode is unstable with sprite scaling\n R_DrawSpriteRange: bad texturecolumn");
	...
	if (vis->mobjflags & MF_HIRES) // SRB2CBTODO: Is this even needed?
	{
		spryscale = spryscale << 1;
		vis->scale = vis->scale << 1;
		dc_iscale = dc_iscale >> 1;
...
static void R_DrawPrecipitationVisSprite(vissprite_t *vis)
{
	...
	// R_InitSprites now sets a wad lump number
	patch = W_CacheLumpNum(vis->patch, PU_CACHE);
	if (!patch)
		return;
...
static void R_SplitSprite(vissprite_t *sprite, mobj_t *thing)
{
	...
	for (i = 1; i < sector->numlights; i++)
	{
		...
		if (!(sector->lightlist[i].caster->flags & FF_NOSHADE))
		{
			 ...
			 */
			if (!((thing->frame & (FF_FULLBRIGHT|FF_TRANSMASK) || thing->flags2 & MF2_TRANSLUCENT)
...
static void R_ProjectSprite(mobj_t *thing)
{
...
#else
	{
		CONS_Printf(PREFIX_WARN "Mobj of type %d with invalid sprite data (%d) detected and removed.\n", thing->type, thing->sprite);
		if (thing->player)
	...
	// thing->skin overrides spritedef currently used for skin
...
#else
	{
		CONS_Printf(PREFIX_WARN "Mobj of type %d with invalid sprite data (%ld) detected and removed.\n", thing->type, (thing->frame&FF_FRAMEMASK));
		if (thing->player)
			P_SetPlayerMobjState(thing, S_PLAY_STND);
		else
			P_SetMobjState(thing, S_DISS);
	...
	if (thing->scale > 400) // Software can't handle a scale greater than 400%
	{
		if (thing->flags & MF_HIRES)
			tx -= (spritecachedinfo[lump].offset/2)*400/100;
		else
			tx -= spritecachedinfo[lump].offset*400/100;
	}
	else if (thing->scale != 100)
	{
		if (thing->flags & MF_HIRES)
			tx -= (spritecachedinfo[lump].offset/2)*thing->scale/100;
		else
			tx -= spritecachedinfo[lump].offset*thing->scale/100;
	}
	...
	if (thing->scale > 400) // Scale >= 400%? Software can't handle that! Render it as 400% instead.
	{
		if (thing->flags & MF_HIRES)
			tx += (spritecachedinfo[lump].width/2)*400/100;
		else
			tx += spritecachedinfo[lump].width*400/100;
	}
	else if (thing->scale != 100)
	{
		if (thing->flags & MF_HIRES)
			tx += (spritecachedinfo[lump].width/2)*thing->scale/100;
		else
			tx += spritecachedinfo[lump].width*thing->scale/100;
	}
	else if (thing->flags & MF_HIRES)
	...
	if (thing->eflags & MFE_VERTICALFLIP)
	{
		if (thing->scale > 400) // Software can't handle a scale greater than 400%
		{
			if (thing->flags & MF_HIRES)
			{
				gzt = thing->z + thing->height + (((spritecachedinfo[lump].height - spritecachedinfo[lump].topoffset)/2)/2)*400/100;
				gz = gzt - (spritecachedinfo[lump].height/2)*400/100;
			}
			else
			{
				gzt = thing->z + thing->height + ((spritecachedinfo[lump].height - spritecachedinfo[lump].topoffset)/2)*400/100;
				gz = gzt - spritecachedinfo[lump].height*400/100;
			}
		}
		else if (thing->scale != 100)
		{
	...
	else
	{
		if (thing->scale > 400) // Scale > 400%? Software can't handle that! Render it as 400% instead.
		{
			if (thing->flags & MF_HIRES)
			{
				gzt = thing->z + (spritecachedinfo[lump].topoffset/2)*400/100;
				gz = gzt - (spritecachedinfo[lump].height/2)*400/100;
			}
			else
			{
				gzt = thing->z + spritecachedinfo[lump].topoffset*400/100;
				gz = gzt - spritecachedinfo[lump].height*400/100;
			}
		}
		else if (thing->scale != 100)
	...
	if (thing->flags2 & MF2_TRANSLUCENT) // Preset semitransparency
		vis->transmap = ((tr_trans80-1)<<FF_TRANSSHIFT) + transtables;
	else if (thing->frame & FF_TRANSMASK) // Custom transparency
...
static void R_ProjectPrecipitationSprite(precipmobj_t *thing)
{
	...
	// Disregard sprites that are out of view..
	gzt = thing->z + spritecachedinfo[lump].topoffset;
...
void R_AddSprites(sector_t *sec, int lightlevel)
{
	...
	// Special function for precipitation
	...
	for (rover = vsprsortedhead.prev; rover != &vsprsortedhead; rover = rover->prev)
	{
		...
		for (r2 = nodehead.next; r2 != &nodehead; r2 = r2->next)
		{
			if (r2->plane)
			{
				...
				{
					// NOTE: Because a visplane's shape and scale is not directly
					// bound to any single linedef, a simple poll of it's frontscale is
					// not adequate. The game must check the entire frontscale array for any
...
static void R_DrawSprite(vissprite_t *spr)
{
	...
	// All clipping has been performed, now draw the sprite
	...
	for (x = spr->x1; x <= spr->x2; x++)
	{
		...
		if (cliptop[x] == -2)
			// Now clips against console bottom
...
// Special drawer for precipitation sprites
...
void SetSavedSkin(int playernum, int skinnum, int skincolor)
{
	...
	snprintf(val, sizeof val, "%li", (ULONG)players[playernum].skincolor);
	val[sizeof val - 1] = '\0';

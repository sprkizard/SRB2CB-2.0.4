...
*/
extern boolean keyboard_started;
...
/**	\brief  Called by D_SRB2Loop.
 
	Returns current time in 1/(TICRATE*scale) second tics
 */
tic_t I_GetScaledTime(float scale);
...
typedef struct JoyFF_s
{
	...
	ULONG Phase; ///< Position in the cycle of the periodic effect at which playback begins, in the range from 0 through 35,999
	ULONG Period; ///< Period of the effect, in microseconds.
} JoyFF_t;
...
/**	\brief	The I_CreateDirectory function

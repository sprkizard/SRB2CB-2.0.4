...
// haleyjd 02/16/06
//
// SRB2CBTODO: Add NEWTICRATERATIO support here
//
//----------------------------------------------------------------------------
...
//
void Polyobj_GetInfo(short tag, int *polyID, int *mirrorID, USHORT *exparg)
{
...
static void Polyobj_addVertex(polyobj_t *po, vertex_t *v)
{
	...
	for (i = 0; i < po->numVertices; ++i)
	{
		if (po->vertices[i] == v)
		{
			// SRB2CBTODO: FIX THIS: SAME VERTEX ADDED TWICE!!!!!
			//CONS_Printf("Polyobj_addVertex: Same vertex added twice!\n");
			return;
		}
	}
...
static void Polyobj_addSeg(polyobj_t *po, seg_t *seg)
{
	...
	// reduce problems.
	// SRB2CBTODO: This v2 error may no longer apply to SRB2's polyobjects,
	// because they cannot even be an incomplete polygon, check to remove
...
newseg:
	...
	po->isBad = true;
	CONS_Printf("Error: Polyobject %d is not closed\n", po->id);
}
...
static void Polyobj_spawnPolyObj(int num, mobj_t *spawnSpot, int id)
{
	...
	// TODO: // SRB2CBTODO: support customized thrust?
	po->thrust = FRACUNIT;
	po->flags = 0;
	...
	for (i = 0; i < numsegs; ++i)
	{
		...
		if (polyID == po->id)
		{
			...
			//if (!(seg->linedef->flags & ML_EFFECT4)) // SRB2CBTODO: Find a use for this
			//	po->flags |= POF_UNUSED1;

			// Unclips plane collision // SRB2CBTODO:
			if (!(seg->linedef->flags & ML_BOUNCY))
				po->flags |= POF_RENDERBOTTOM;
			
			if (!(seg->linedef->flags & ML_TFERLINE))
				po->flags |= POF_RENDERTOP;
			...
			if (po->parent == po->id) // do not allow a self-reference
				po->parent = -1;
			// SRB2CBTODO: sound sequence is in args[2]
			break;
		}
	}

	if (cv_devmode == 2)
		CONS_Printf("PO ID: %d; Num verts: %d\n", po->id, (int)po->numVertices);
...
static void Polyobj_attachToSubsec(polyobj_t *po)
{
	...
	numVertices = po->numVertices*FRACUNIT;
...
static void Polyobj_linkToBlockmap(polyobj_t *po)
{
	...
	blockbox[BOXRIGHT]  = (unsigned int)(blockbox[BOXRIGHT]  - bmaporgx) >> MAPBLOCKSHIFT;
	blockbox[BOXLEFT]   = (unsigned int)(blockbox[BOXLEFT]   - bmaporgx) >> MAPBLOCKSHIFT;
	blockbox[BOXTOP]    = (unsigned int)(blockbox[BOXTOP]    - bmaporgy) >> MAPBLOCKSHIFT;
	blockbox[BOXBOTTOM] = (unsigned int)(blockbox[BOXBOTTOM] - bmaporgy) >> MAPBLOCKSHIFT;
...
static void Polyobj_pushThing(polyobj_t *po, line_t *line, mobj_t *mo)
{
	...
	// If an object doesn't fit at desired location, possibly hurt it
	if (po->damage && (mo->flags & MF_SHOOTABLE))
	{
		P_CheckPosition(mo, mo->x + momx, mo->y + momy);
		mo->floorz = tmfloorz;
		mo->ceilingz = tmceilingz; // SRB2CBTODO: ZMOVE
	}
}
...
static int Polyobj_clipThings(polyobj_t *po, line_t *line)
{
	...
	linebox[BOXLEFT]   = (unsigned int)(line->bbox[BOXLEFT]   - bmaporgx - MAXRADIUS) >> MAPBLOCKSHIFT;
	linebox[BOXRIGHT]  = (unsigned int)(line->bbox[BOXRIGHT]  - bmaporgx + MAXRADIUS) >> MAPBLOCKSHIFT;
	linebox[BOXBOTTOM] = (unsigned int)(line->bbox[BOXBOTTOM] - bmaporgy - MAXRADIUS) >> MAPBLOCKSHIFT;
	linebox[BOXTOP]    = (unsigned int)(line->bbox[BOXTOP]    - bmaporgy + MAXRADIUS) >> MAPBLOCKSHIFT;
		...
		for (x = linebox[BOXLEFT]; x <= linebox[BOXRIGHT]; ++x)
		{
			if (!(x < 0 || y < 0 || x >= bmapwidth || y >= bmapheight))
			{
				...
				for (; mo; mo = mo->bnext)
				{
					...
					if (mo->player && (po->lines[0]->backsector->flags & SF_TRIGGERSPECIAL_INSIDE) && !(po->flags & POF_NOSPECIALS))
...
static boolean Polyobj_moveXY(polyobj_t *po, fixed_t x, fixed_t y)
{
	...
	// Lastly, update the plane texture coordinates,
	// subtract the x, add the y
	// SRB2CBTODO: Mathematical reason for subtracting one, and adding one?
	po->lines[0]->backsector->floor_yoffs += y;
	po->lines[0]->backsector->floor_xoffs -= x;
	po->lines[0]->backsector->ceiling_yoffs += y;
	po->lines[0]->backsector->ceiling_xoffs -= x;
...
// It is, of course, just a vector-matrix multiplication. // SRB2CBTODO: Convert vertex_t and the rest of the code with matrix conformance!!!
//
...
static boolean Polyobj_rotate(polyobj_t *po, angle_t delta)
{
	...
	{
		// Update seg angles (used only by software renderer)
	...
	// SRB2CBTODO: Lastly, update the floor coordinates, also,
	// find out how to properly add angle coordinates
	// SRB2CBTODO: find out how to use a rotated floorpic for OpenGL
	po->lines[0]->backsector->floorpic_angle = po->angle;
	po->lines[0]->backsector->ceilingpic_angle = po->angle; // POLYOBJECTS_PLANES
...
//
// SRB2CBTODO: Polyobjects don't always work on the first jump NOTE: Not here
...
void T_PolyDoorSwing(polyswingdoor_t *th)
{
	...
	else if (th->closing && th->distance != th->initDistance)
	{
		...
		// SRB2TODO: sound sequence start event
	}
}

static int Polyobj_AngSpeed(int speed)
{
	return (speed*ANGLE_1)>>3; // no FixedAngle()
}
...
int EV_DoPolyObjWaypoint(polywaypointdata_t *pwdata)
{
	...
	// We don't deal with the mirror stuff here, we'll
...
int EV_DoPolyObjFlag(line_t *pfdata)
{
	...
	if (po->numVertices & 1)
	{
		CONS_Printf("EV_DoPolyFlag: Polyobject has an odd number of vertices!\n");
		return 0;
	}

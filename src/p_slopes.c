...
void P_SpawnSlope_Line(int linenum)
{
	...
	if(frontfloor || frontceil)
	{
		...
		if(frontfloor)
		{
			...
			fslope->xydirection = R_PointToAngle2(FLOAT_TO_FIXED(A.x), FLOAT_TO_FIXED(A.y), FLOAT_TO_FIXED(B.x), FLOAT_TO_FIXED(B.y))/(ANG45/45);
...
//
// The line with the slope copy special will affect the sector(s) next to it
// and the slope is copied from a sloped SECTOR with a tag
// TODO: Do the back sector
void P_CopySectorSlope(line_t *line)
{
   ...
   for(i = -1; (i = P_FindSectorFromLineTag(line, i)) >= 0;)
   {
      ...
      if((special == 394 || special == 396) && !line->frontsector->f_slope && srcsec->f_slope)
      {
         line->frontsector->f_slope = P_CopySlope(srcsec->f_slope);
         //CONS_Printf("Copied floor slope to line with tag %i frontsector\n", line->tag);
      }
      if((special == 395 || special == 396) && !line->frontsector->c_slope && srcsec->c_slope)
      {
         line->frontsector->c_slope = P_CopySlope(srcsec->c_slope);
         //CONS_Printf("Copied ceiling slope to line with tag %i frontsector\n", line->tag);
      }
   }

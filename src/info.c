...
state_t states[NUMSTATES] =
{
	...
	{SPR_EGGM,  8,  15, {A_BossFireShot},  MT_ROCKET, 1, S_EGGMOBILE_LATK9},  // S_EGGMOBILE_LATK8
	...
	{SPR_EGGM, 18,  15, {A_BossFireShot},  MT_ROCKET, 0, S_EGGMOBILE_RATK9},  // S_EGGMOBILE_RATK8
...
#ifdef PARTICLES
	{SPR_STPT, 0, 2, {A_EmitStarParticle}, 1, 220, S_STARPOST3},   // S_STARPOST2
#else
	...
	// DSZ Seaweed
	{SPR_SEWE, 0, 4, {NULL}, 0, 0, S_SEAWEED2}, // S_SEAWEED1
	{SPR_SEWE, 1, 4, {NULL}, 0, 0, S_SEAWEED3}, // S_SEAWEED2
	{SPR_SEWE, 2, 4, {NULL}, 0, 0, S_SEAWEED4}, // S_SEAWEED3
	{SPR_SEWE, 3, 4, {NULL}, 0, 0, S_SEAWEED5}, // S_SEAWEED4
	{SPR_SEWE, 4, 4, {NULL}, 0, 0, S_SEAWEED6}, // S_SEAWEED5
	{SPR_SEWE, 5, 4, {NULL}, 0, 0, S_SEAWEED1}, // S_SEAWEED6
...
#ifdef PARTICLES
	{SPR_SPRK, 1, 1, {A_EmitLightParticle}, 40, 14, S_SPRK3},  // S_SPRK2
#else
	{SPR_SPRK, 1, 1, {NULL}, 0, 0, S_SPRK3},  // S_SPRK2
#endif
	{SPR_SPRK, 2, 1, {NULL}, 0, 0, S_SPRK4},  // S_SPRK3
#ifdef PARTICLES
	{SPR_SPRK, 3, 1, {A_EmitLightParticle}, 150, 14, S_SPRK5},  // S_SPRK4
#else
	{SPR_SPRK, 3, 1, {NULL}, 0, 0, S_SPRK5},  // S_SPRK4
#endif
	{SPR_SPRK, 0, 1, {NULL}, 0, 0, S_SPRK6},  // S_SPRK5
#ifdef PARTICLES
	{SPR_SPRK, 1, 1, {A_EmitLightParticle}, 20, 14, S_SPRK7},  // S_SPRK6
#else
...
{           // MT_BLUECRAWLA
	...
	{           // MT_FLOORSPIKE
		...
		0,              // damage
		sfx_None,       // activesound
		MF_NOGRAVITY|MF_SCENERY|MF_NOCLIPHEIGHT,  // flags
		S_NULL          // raisestate
	},
	...
	{           // MT_THOK
		...
		0,              // damage
		sfx_None,       // activesound
		MF_NOBLOCKMAP|MF_NOCLIP|MF_NOGRAVITY|MF_SCENERY|MF_NOCLIPHEIGHT, // flags
		S_NULL          // raisestate
	},
...
{
	byte *tData;
	tData = Z_Calloc(1024, PU_STATIC, NULL);
	byte *w = tData;
	byte *q = (void *)p;
	byte *r = (void *)p;
	byte *s = (void *)p;
...
#if 0// NEWTICRATERATIO != 1 // SRB2CBTODO: This is strange, do a real change
...
// SRB2CBTODO: If not moved over to external thing table, switch all objects to this
void P_InitExtraObjects(void)
{
	size_t thing;

	////////////// Define extra object sprites ////////////////

	thing = SPR_YSDS;
	strcpy(sprnames[thing],"YSDS");

	thing = SPR_RSDS;
	strcpy(sprnames[thing],"RSDS");

	thing = SPR_CBUM;
	strcpy(sprnames[thing],"CBUM");

	thing = SPR_BLON;
	strcpy(sprnames[thing],"BLON");

	thing = SPR_HANG;
	strcpy(sprnames[thing],"HANG");

	thing = SPR_SKAT;
	strcpy(sprnames[thing],"SKAT");

	// Bubble Shield
	thing = SPR_BBTV;
	strcpy(sprnames[thing],"BBTV");

	// Flame Shield
	thing = SPR_FLTV;
	strcpy(sprnames[thing],"FLTV");

	// Lightning Shield
	thing = SPR_LTTV;
	strcpy(sprnames[thing],"LTTV");


	/////// Sound Effects ////////////

	thing = sfx_balloonpop;
	S_sfx[thing] = S_sfx[sfx_crumbl]; // SFX after this should have "none"
	S_sfx[thing].name = "balpop";
	S_sfx[thing].singularity = false;
	S_sfx[thing].priority = 64;
	S_sfx[thing].pitch = SF_NOINTERRUPT|SF_X8AWAYSOUND;


	////////////// Define extra object states ////////////////
	thing = S_LIGHTPARTICLE;
    states[thing] = states[S_PARTICLE];
    states[thing].sprite = SPR_NSPK;
    states[thing].tics = -1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_NULL;
    states[thing].frame = 0;

#ifdef SRB2K
	thing = S_BOUNCETV1;
	states[thing] = states[S_WHTV1];
	states[thing].frame = 0;
	states[thing].nextstate = S_BOUNCETV2;
	states[thing].sprite = SPR_BBTV; // SRB2KTODO: custom sprite

	thing = S_BOUNCETV2;
	states[thing] = states[S_WHTV2];
	states[thing].frame = 0;
	states[thing].nextstate = S_BOUNCETV1;

	thing = S_BOUNCETV3;
	states[thing] = states[S_WHTV3];
	states[thing].frame = 1;
	states[thing].nextstate = S_BOUNCETV4;
	states[thing].sprite = SPR_BBTV; // SRB2KTODO: custom sprite
	...
	states[thing].action.acp1 = (actionf_p1)A_BubbleShield;
	states[thing].var1 = 0;
	states[thing].var2 = 0;
	...
	thing = S_REDFIRETV1;
	states[thing] = states[S_BKTV1];
	states[thing].nextstate = S_REDFIRETV2;
	states[thing].sprite = SPR_FLTV; // SRB2KTODO: custom sprite

	thing = S_REDFIRETV2;
	states[thing] = states[S_BKTV2];
	states[thing].nextstate = S_REDFIRETV1;

	thing = S_REDFIRETV3;
	states[thing] = states[S_BKTV3];
	states[thing].nextstate = S_REDFIRETV4;
	states[thing].sprite = SPR_FLTV; // SRB2CBTODO: custom sprite

	thing = S_REDFIRETV4;
	states[thing] = states[S_BKTV4];
	states[thing].action.acp1 = (actionf_p1)A_FlameShield;
	states[thing].sprite = SPR_FLTV; // SRB2KTODO: custom sprite
	...
	thing = S_ELECTRICTV1;
	states[thing] = states[S_YLTV];
	states[thing].nextstate = S_ELECTRICTV2;
	states[thing].sprite = SPR_LTTV; // SRB2KTODO: custom sprite

	thing = S_ELECTRICTV2;
	states[thing] = states[S_YLTV1];
	states[thing].nextstate = S_ELECTRICTV1;

	thing = S_ELECTRICTV3;
	states[thing] = states[S_YLTV2];
	states[thing].nextstate = S_ELECTRICTV4;
	states[thing].sprite = SPR_LTTV; // SRB2KTODO: custom sprite

	thing = S_ELECTRICTV4;
	states[thing] = states[S_YLTV3];
	states[thing].action.acp1 = (actionf_p1)A_LightningShield;
	states[thing].sprite = SPR_LTTV; // SRB2KTODO: custom sprite

	//////////

	thing = S_TELEPORTER; // Just for easier identification
	states[thing] = states[S_INVISIBLE];

	//////////

	thing = S_SKYBOXVIEW; // Set the level's skybox viewpoint
    states[thing].sprite = SPR_DISS;
    states[thing].tics = -1;
    states[thing].action.acp1 = (actionf_p1)A_SetSkyBoxMobj;
    states[thing].nextstate = S_NULL;
    states[thing].frame = 0;

	thing = S_SKYBOXMAPCENTER; // Set the level's skybox relative viewpoint on the map
    states[thing].sprite = SPR_DISS;
    states[thing].tics = -1;
    states[thing].action.acp1 = (actionf_p1)A_SetSkyBoxCenterMobj;
    states[thing].nextstate = S_NULL;
    states[thing].frame = 0;
    ...
    thing = S_YELLOWSIDE;
    states[thing] = states[S_YELLOWSIDE];
    states[thing].sprite = SPR_YSDS;
    states[thing].tics = -1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_NULL;
    states[thing].frame = 0;
    ...
    thing = S_YELLOWSIDE2;
    states[thing] = states[S_YELLOWSIDE2];
    states[thing].sprite = SPR_YSDS;
    states[thing].tics = 4;
    states[thing].action.acp1 = (actionf_p1)A_Pain;
    states[thing].nextstate = S_YELLOWSIDE3;
    states[thing].frame = 4;

    thing = S_YELLOWSIDE3;
    states[thing] = states[S_YELLOWSIDE3];
    states[thing].sprite = SPR_YSDS;
    states[thing].tics = 1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_YELLOWSIDE4;
    states[thing].frame = 3;

    thing = S_YELLOWSIDE4;
    states[thing] = states[S_YELLOWSIDE4];
    states[thing].sprite = SPR_YSDS;
    states[thing].tics = 1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_YELLOWSIDE5;
    states[thing].frame = 2;

    thing = S_YELLOWSIDE5;
    states[thing] = states[S_YELLOWSIDE5];
    states[thing].sprite = SPR_YSDS;
    states[thing].tics = 1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_YELLOWSIDE;
    states[thing].frame = 1;


    thing = S_REDSIDE;
    states[thing] = states[S_REDSIDE];
    states[thing].sprite = SPR_RSDS;
    states[thing].tics = -1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_NULL;
    states[thing].frame = 0;
    ...
    thing = S_REDSIDE2;
    states[thing] = states[S_REDSIDE2];
    states[thing].sprite = SPR_RSDS;
    states[thing].tics = 4;
    states[thing].action.acp1 = (actionf_p1)A_Pain;
    states[thing].nextstate = S_REDSIDE3;
    states[thing].frame = 4;

    thing = S_REDSIDE3;
    states[thing] = states[S_REDSIDE3];
    states[thing].sprite = SPR_RSDS;
    states[thing].tics = 1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_REDSIDE4;
    states[thing].frame = 3;

    thing = S_REDSIDE4;
    states[thing] = states[S_REDSIDE4];
    states[thing].sprite = SPR_RSDS;
    states[thing].tics = 1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_REDSIDE5;
    states[thing].frame = 2;
    ...
    states[thing].tics = 1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_REDSIDE;
    states[thing].frame = 1;

    thing = S_CBUMPER;
    states[thing] = states[S_CBUMPER];
    states[thing].sprite = SPR_CBUM;
    states[thing].tics = -1;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_NULL;
    states[thing].frame = 0;
    ...
    thing = S_CBUMPER2;
    states[thing] = states[S_CBUMPER2];
    states[thing].sprite = SPR_CBUM;
    states[thing].tics = 5;
    states[thing].action.acp1 = (actionf_p1)A_Pain;
    states[thing].nextstate = S_CBUMPER;
    states[thing].frame = 0;

    thing = S_BALLOON;
    states[thing] = states[S_BALLOON];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 10;
    states[thing].action.acp1 = (actionf_p1)A_SetBalloonColor;
    states[thing].nextstate = S_BALLOON2;
    states[thing].frame = 0;

    thing = S_BALLOON2;
    states[thing] = states[S_BALLOON2];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 10;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_BALLOON3;
    states[thing].frame = 1;

    thing = S_BALLOON3;
    states[thing] = states[S_BALLOON3];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 10;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_BALLOON;
    states[thing].frame = 2;

    thing = S_BALLOON4;
    states[thing] = states[S_BALLOON4];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 7;
    states[thing].action.acp1 = (actionf_p1)A_Pain;
    states[thing].nextstate = S_BALLOON5;
    states[thing].frame = 3;

    thing = S_BALLOON5;
    states[thing] = states[S_BALLOON5];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 7;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_DISS;
	...
	// Baloon that respawns

    thing = S_BALLOONR;
    states[thing] = states[S_BALLOONR];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 10;
    states[thing].action.acp1 = (actionf_p1)A_SetBalloonColor;
    states[thing].nextstate = S_BALLOONR2;
    states[thing].frame = 0;

    thing = S_BALLOONR2;
    states[thing] = states[S_BALLOONR2];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 10;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_BALLOONR3;
    states[thing].frame = 1;

    thing = S_BALLOONR3;
    states[thing] = states[S_BALLOONR3];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 10;
    states[thing].action.acp1 = (actionf_p1)NULL;
    states[thing].nextstate = S_BALLOON;
    ...
    thing = S_BALLOONR4;
    states[thing] = states[S_BALLOONR4];
    states[thing].sprite = SPR_BLON;
    states[thing].tics = 7;
    states[thing].action.acp1 = (actionf_p1)A_Pain;
    states[thing].nextstate = S_BALLOONR5;
    states[thing].frame = 3;

    thing = S_BALLOONR5;
    states[thing] = states[S_BALLOONR5];
    states[thing].sprite = SPR_BLON;
	...
	states[thing] = states[S_PARTICLEFOUNTAIN];

	thing = S_HANGGLIDER;
	states[thing].tics = -1;
	states[thing].frame = 0;
	states[thing].nextstate = S_HANGGLIDER;
	states[thing].action.acp1 = (actionf_p1)NULL;
	states[thing].sprite = SPR_HANG; // SRB2CBTODO: Custom sprite

	thing = S_SKATEBOARD;
	states[thing].tics = -1;
	states[thing].frame = 0;
	states[thing].nextstate = S_SKATEBOARD;
	states[thing].action.acp1 = (actionf_p1)NULL;
	states[thing].sprite = SPR_SKAT; // SRB2CBTODO: Custom sprite


	////////////// Object Types ////////////////

	thing = MT_BOUNCEORB;
	mobjinfo[thing] = mobjinfo[MT_WHITEORB];
	mobjinfo[thing].spawnstate = S_SORB1; // SRB2KTODO: Custom sprite
	mobjinfo[thing].speed = pw_bubbleshield;

	thing = MT_REDFIREORB;
	mobjinfo[thing] = mobjinfo[MT_BLACKORB];
	mobjinfo[thing].spawnstate = S_SORB1; // SRB2KTODO: Custom sprite
	mobjinfo[thing].speed = pw_flameshield;

	thing = MT_ELECTRICORB;
	mobjinfo[thing] = mobjinfo[MT_YELLOWORB];
	mobjinfo[thing].spawnstate = S_SORB1; // SRB2KTODO: Custom sprite
	mobjinfo[thing].speed = pw_lightningshield;

	thing = MT_BOUNCETV;
	mobjinfo[thing] = mobjinfo[MT_WHITETV];
	// SRB2KTODO: Custom sprites
	mobjinfo[thing].doomednum = 440;
	mobjinfo[thing].spawnstate = S_BOUNCETV1;
	mobjinfo[thing].painstate = S_BOUNCETV1;
	mobjinfo[thing].deathstate = S_BOUNCETV3;
	mobjinfo[thing].damage = MT_BOUNCESHIELDICO;

	thing = MT_REDFIRETV;
	mobjinfo[thing] = mobjinfo[MT_BLACKTV];
	// SRB2KTODO: Custom sprites
	mobjinfo[thing].doomednum = 441;
	mobjinfo[thing].spawnstate = S_REDFIRETV1;
	mobjinfo[thing].painstate = S_REDFIRETV1;
	mobjinfo[thing].deathstate = S_REDFIRETV3;
	mobjinfo[thing].damage = MT_REDFIRESHIELDICO;


	thing = MT_ELECTRICTV;
	mobjinfo[thing] = mobjinfo[MT_YELLOWTV];
	// SRB2KTODO: Custom sprites
	mobjinfo[thing].doomednum = 442;
	mobjinfo[thing].spawnstate = S_ELECTRICTV1;
	mobjinfo[thing].painstate = S_ELECTRICTV1;
	mobjinfo[thing].deathstate = S_ELECTRICTV3;
	mobjinfo[thing].damage = MT_ELECTRICSHIELDICO;


	thing = MT_BOUNCESHIELDICO;
	mobjinfo[thing] = mobjinfo[MT_WSHIELDICO];
	mobjinfo[thing].spawnstate = S_BOUNCETV3; // SRB2KTODO: Custom sprite
	mobjinfo[thing].seesound = sfx_shield;

	thing = MT_REDFIRESHIELDICO;
	mobjinfo[thing] = mobjinfo[MT_BSHIELDICO];
	mobjinfo[thing].spawnstate = S_REDFIRETV3; // SRB2KTODO: Custom sprite
	mobjinfo[thing].seesound = sfx_shield;

	thing = MT_ELECTRICSHIELDICO;
	mobjinfo[thing] = mobjinfo[MT_YSHIELDICO];
	mobjinfo[thing].spawnstate = S_ELECTRICTV3; // SRB2KTODO: Custom sprite
	mobjinfo[thing].seesound = sfx_shield;
#endif

	// SRB2CB: - sideways springs

	thing = MT_YELLOWSIDE;
	mobjinfo[thing].doomednum = 570;
	mobjinfo[thing].spawnstate = S_YELLOWSIDE;
	mobjinfo[thing].spawnhealth = 1000;
	mobjinfo[thing].seestate = S_YELLOWSIDE2;
	...
	mobjinfo[thing].painchance = 0; // Don't change the player's states
	...
	mobjinfo[thing].speed = 1; // Just enough height so it doesn't mess up
	mobjinfo[thing].radius = 20*FRACUNIT;
	mobjinfo[thing].height = 0*FRACUNIT;
	mobjinfo[thing].mass = 100;
	mobjinfo[thing].damage = 40*FRACUNIT;
	mobjinfo[thing].activesound = sfx_None;
	mobjinfo[thing].flags = MF_SOLID|MF_SPRING|MF_NOGRAVITY;
	mobjinfo[thing].raisestate = S_NULL;

	thing = MT_REDSIDE;
	mobjinfo[thing].doomednum = 571;
	mobjinfo[thing].spawnstate = S_REDSIDE;
	mobjinfo[thing].spawnhealth = 1000;
	mobjinfo[thing].seestate = S_REDSIDE2;
	mobjinfo[thing].seesound = sfx_None;
	mobjinfo[thing].reactiontime = 8;
	mobjinfo[thing].attacksound = sfx_None;
	mobjinfo[thing].painstate = S_NULL;
	...
	mobjinfo[thing].height = 0*FRACUNIT;
	mobjinfo[thing].mass = 100;
	mobjinfo[thing].damage = 60*FRACUNIT;
	mobjinfo[thing].activesound = sfx_None;
	mobjinfo[thing].flags = MF_SOLID|MF_SPRING|MF_NOGRAVITY;
	mobjinfo[thing].raisestate = S_NULL;

	thing = MT_CBUMPER;
	mobjinfo[thing].doomednum = 572;
	mobjinfo[thing].spawnstate = S_CBUMPER;
	mobjinfo[thing].spawnhealth = 1000;
	mobjinfo[thing].seestate = S_CBUMPER2;
	mobjinfo[thing].seesound = sfx_None;
	mobjinfo[thing].reactiontime = 8;
	mobjinfo[thing].attacksound = sfx_None;
	mobjinfo[thing].painstate = S_NULL;
	...
	mobjinfo[thing].xdeathstate = S_NULL;
	mobjinfo[thing].deathsound = sfx_None;
	mobjinfo[thing].speed = 10*FRACUNIT; // Just enough height so it doesn't mess up
	mobjinfo[thing].radius = 20*FRACUNIT;
	mobjinfo[thing].height = 10*FRACUNIT;
	mobjinfo[thing].mass = 100;
	mobjinfo[thing].damage = 25*FRACUNIT;
	mobjinfo[thing].activesound = sfx_None;
	mobjinfo[thing].flags = MF_SOLID|MF_SPRING|MF_NOGRAVITY;
	mobjinfo[thing].raisestate = S_NULL;
	...
	// Particles NEED these flags, otherwise, crashes
	// (because of collision when there shouldn't be, etc.)
	mobjinfo[thing].flags = MF_NOBLOCKMAP|MF_NOCLIP|MF_SCENERY;

	thing = MT_SPARTICLE; // Spring+Particles = SPARTACLE!
	mobjinfo[thing].doomednum = -1;
	mobjinfo[thing].spawnstate = S_SPARTICLE;
	mobjinfo[thing].spawnhealth = 1000;
	mobjinfo[thing].seestate = S_SPARTICLE4;
	mobjinfo[thing].seesound = sfx_None;
	mobjinfo[thing].reactiontime = 8;
	mobjinfo[thing].attacksound = sfx_None;
	mobjinfo[thing].painstate = S_NULL;
	...
	thing = MT_PARTICLEFOUNTAIN;
	mobjinfo[thing].doomednum = 585;
	mobjinfo[thing].spawnstate = S_PARTICLEFOUNTAIN;
	mobjinfo[thing].spawnhealth = 1000;
	...
	mobjinfo[thing].radius = 8;
	mobjinfo[thing].height = 8;
	mobjinfo[thing].mass = 10;
	mobjinfo[thing].damage = 0;
	mobjinfo[thing].activesound = sfx_None;
	mobjinfo[thing].flags = MF_NOBLOCKMAP|MF_NOCLIP|MF_SCENERY;
	mobjinfo[thing].raisestate = S_NULL;
	...
	mobjinfo[thing] = mobjinfo[MT_PARTICLEFOUNTAIN];
	mobjinfo[thing].doomednum = 586;
	mobjinfo[thing].spawnstate = S_SPIRALFOUNTAIN;

	thing = MT_FSPIRALFOUNTAIN;
	mobjinfo[thing] = mobjinfo[MT_PARTICLEFOUNTAIN];
	mobjinfo[thing].doomednum = 587;
	mobjinfo[thing].spawnstate = S_SPIRALFOUNTAIN;

	thing = MT_FPARTICLEFOUNTAIN;
	mobjinfo[thing] = mobjinfo[MT_PARTICLEFOUNTAIN];
	mobjinfo[thing].doomednum = 6975;
	mobjinfo[thing].spawnstate = S_PARTICLEFOUNTAIN;
#endif

	thing = MT_BALLOON;
	mobjinfo[thing].doomednum = 573;
	mobjinfo[thing].spawnstate = S_BALLOON;
	mobjinfo[thing].spawnhealth = 1000;
	mobjinfo[thing].seestate = S_BALLOON4;
	mobjinfo[thing].seesound = sfx_None;
	mobjinfo[thing].reactiontime = 8;
	mobjinfo[thing].attacksound = sfx_None;
	mobjinfo[thing].painstate = S_NULL;
	mobjinfo[thing].painchance = 0; // Don't change the player's states
	mobjinfo[thing].painsound = sfx_balloonpop;
	mobjinfo[thing].meleestate = S_NULL;
	mobjinfo[thing].missilestate = S_NULL;
	mobjinfo[thing].deathstate = S_NULL;
	mobjinfo[thing].xdeathstate = S_NULL;
	...
	mobjinfo[thing].damage = 0;
	mobjinfo[thing].activesound = sfx_None;
	mobjinfo[thing].flags = MF_SOLID|MF_SPRING|MF_NOGRAVITY|MF_RUNSPAWNFUNC;
	mobjinfo[thing].raisestate = S_NULL;

	// Same as a normal balloon, but respawns
	thing = MT_BALLOONR;
	mobjinfo[thing].doomednum = 574;
	mobjinfo[thing].spawnstate = S_BALLOONR;
	mobjinfo[thing].spawnhealth = 1000;
	mobjinfo[thing].seestate = S_BALLOONR4;
	mobjinfo[thing].seesound = sfx_None;
	mobjinfo[thing].reactiontime = 8;
	mobjinfo[thing].attacksound = sfx_None;
	mobjinfo[thing].painstate = S_NULL;
	...
	mobjinfo[thing].xdeathstate = S_NULL;
	mobjinfo[thing].deathsound = sfx_None;
	mobjinfo[thing].speed = 15*FRACUNIT; // Just enough height so it doesn't mess up
	mobjinfo[thing].radius = 20*FRACUNIT;
	mobjinfo[thing].height = 10*FRACUNIT;
	mobjinfo[thing].mass = 100;
	mobjinfo[thing].damage = 0;
	...
	thing = MT_BALLOONFOUNTAIN;
	mobjinfo[thing].doomednum = 575;
	mobjinfo[thing].spawnstate = S_PARTICLEFOUNTAIN;
	mobjinfo[thing].spawnhealth = 1000;
	mobjinfo[thing].seestate = S_NULL;
	mobjinfo[thing].seesound = sfx_None;
	...
	mobjinfo[thing].damage = 0;
	mobjinfo[thing].activesound = sfx_None;
	mobjinfo[thing].flags = MF_NOBLOCKMAP|MF_NOCLIP|MF_SCENERY|MF_NOGRAVITY|MF_NOCLIPHEIGHT;
	mobjinfo[thing].raisestate = S_NULL;

	thing = MT_BALLOONFOUNTAIN2;
	mobjinfo[thing] = mobjinfo[MT_BALLOONFOUNTAIN];
	mobjinfo[thing].doomednum = 576;

	thing = MT_HANGGLIDER;
	mobjinfo[thing] = mobjinfo[MT_BLACKEGGMAN_MISSILE];
	mobjinfo[thing].doomednum = 580;
	mobjinfo[thing].spawnstate = S_HANGGLIDER;
	mobjinfo[thing].flags = MF_NOGRAVITY|MF_SLIDEME;
	mobjinfo[thing].speed = 12*FRACUNIT;
	mobjinfo[thing].radius = 40*FRACUNIT;
	mobjinfo[thing].height = 32*FRACUNIT;
	// SRB2CBTODO: Once you latch on, play a sound

	thing = MT_SKATEBOARD;
	mobjinfo[thing] = mobjinfo[MT_HANGGLIDER];
	mobjinfo[thing].doomednum = 582;
	mobjinfo[thing].spawnstate = S_SKATEBOARD;
	mobjinfo[thing].flags = MF_SLIDEME;
	mobjinfo[thing].reactiontime = 26*FRACUNIT; // Reaction time is the max speed to add to the speed
	mobjinfo[thing].speed = 20*FRACUNIT;
	mobjinfo[thing].radius = 40*FRACUNIT;
	mobjinfo[thing].height = 1*FRACUNIT; // Skateboards are thin

	// SRB2CBTODO: Mobj types can't be over a certain number(around > 4999?), fix this limit!
	thing = MT_XTELEPORT;
	mobjinfo[thing] = mobjinfo[MT_TELEPORTMAN];
	mobjinfo[thing].doomednum = 780;
	mobjinfo[thing].spawnstate = S_TELEPORTER;
	mobjinfo[thing].flags = MF_NOGRAVITY; // This object MUST have a blockmap though

	thing = MT_YTELEPORT;
	mobjinfo[thing] = mobjinfo[MT_XTELEPORT];
	mobjinfo[thing].doomednum = 781;

	thing = MT_ZTELEPORT;
	mobjinfo[thing] = mobjinfo[MT_XTELEPORT];
	mobjinfo[thing].doomednum = 782;

	thing = MT_XYTELEPORT;
	mobjinfo[thing] = mobjinfo[MT_XTELEPORT];
	mobjinfo[thing].doomednum = 783;

	thing = MT_XYZTELEPORT;
	mobjinfo[thing] = mobjinfo[MT_XTELEPORT];
	mobjinfo[thing].doomednum = 784;

	thing = MT_ZXTELEPORT;
	mobjinfo[thing] = mobjinfo[MT_XTELEPORT];
	mobjinfo[thing].doomednum = 785;

	thing = MT_ZYTELEPORT;
	mobjinfo[thing] = mobjinfo[MT_XTELEPORT];
	mobjinfo[thing].doomednum = 786;

	thing = MT_SKYBOXVIEW; // SRB2CBTODO: This should also allow for dynamic skybox changing
	mobjinfo[thing] = mobjinfo[MT_XTELEPORT];
	mobjinfo[thing].doomednum = 790;
	mobjinfo[thing].flags = MF_RUNSPAWNFUNC|MF_NOGRAVITY|MF_NOCLIPHEIGHT|MF_NOCLIPTHING;
	mobjinfo[thing].spawnstate = S_SKYBOXVIEW;

	thing = MT_SKYBOXMAPCENTER; // SRB2CBTODO: This should also allow for dynamic skybox changing
	mobjinfo[thing] = mobjinfo[MT_SKYBOXVIEW];
	mobjinfo[thing].doomednum = 791;
	mobjinfo[thing].flags = MF_RUNSPAWNFUNC|MF_NOGRAVITY|MF_NOCLIPHEIGHT|MF_NOCLIPTHING;
	mobjinfo[thing].spawnstate = S_SKYBOXMAPCENTER;

	thing = MT_SLOPEFLOORLINE;
	mobjinfo[thing] = mobjinfo[MT_XTELEPORT];
	mobjinfo[thing].doomednum = 9500;
	thing = MT_SLOPECEILINGLINE;
	mobjinfo[thing] = mobjinfo[MT_SLOPEFLOORLINE];
	mobjinfo[thing].doomednum = 9501;
	thing = MT_SETFLOORSLOPE;
	mobjinfo[thing] = mobjinfo[MT_SLOPEFLOORLINE];
	mobjinfo[thing].doomednum = 9502;
	thing = MT_SETCEILINGSLOPE;
	mobjinfo[thing] = mobjinfo[MT_SLOPEFLOORLINE];
	mobjinfo[thing].doomednum = 9503;
	thing = MT_COPYFLOORSLOPE;
	mobjinfo[thing] = mobjinfo[MT_SLOPEFLOORLINE];
	mobjinfo[thing].doomednum = 9510;
	thing = MT_COPYCEILINGSLOPE;
	mobjinfo[thing] = mobjinfo[MT_SLOPEFLOORLINE];
	mobjinfo[thing].doomednum = 9511;
	thing = MT_VERTEXFLOORSLOPE;
	mobjinfo[thing] = mobjinfo[MT_SLOPEFLOORLINE];
	mobjinfo[thing].doomednum = 1504;
	thing = MT_VERTEXCEILINGSLOPE;
	mobjinfo[thing] = mobjinfo[MT_SLOPEFLOORLINE];
	mobjinfo[thing].doomednum = 1505;
	...
	mobjinfo[thing].xdeathstate = S_NULL;
	mobjinfo[thing].deathsound = sfx_None;
	mobjinfo[thing].speed = 10*FRACUNIT; // Distance between each burst to check for other players
	mobjinfo[thing].radius = 1*FRACUNIT;
	mobjinfo[thing].height = 32*FRACUNIT; // how high is the line to the player?
	mobjinfo[thing].mass = 1000; // does nothing
	mobjinfo[thing].damage = 0;
	mobjinfo[thing].activesound = sfx_None;
    ...
    // LXShadow's netcode rewrite
    // A virtual player is an absolutely ESSENTIAL object for netplay,
    // instead of trying to sync EVERYTHING in the game,
    // just the player's essential actions and events are recorded
    // the current status of the level is handle in p_saveg.c
	thing = MT_VIRTUALPLAYER;
	mobjinfo[thing].doomednum = -1;
	mobjinfo[thing].spawnstate = S_PLAY_STND;
	mobjinfo[thing].spawnhealth = 1;
	mobjinfo[thing].seestate = S_PLAY_RUN1;
	mobjinfo[thing].seesound = sfx_None;
	mobjinfo[thing].reactiontime = 99;
	mobjinfo[thing].attacksound = sfx_thok;
	mobjinfo[thing].painstate = S_PLAY_PAIN;
	mobjinfo[thing].painchance = MT_THOK; // Don't change the player's states
	mobjinfo[thing].painsound = sfx_None;
	mobjinfo[thing].meleestate = S_NULL;
	mobjinfo[thing].missilestate = S_PLAY_ATK1;
	mobjinfo[thing].deathstate = S_PLAY_DIE1;
	mobjinfo[thing].xdeathstate = S_PLAY_DIE1;
	mobjinfo[thing].deathsound = sfx_None;
	mobjinfo[thing].speed = 1; // Distance between each burst to check for other players
	mobjinfo[thing].radius = 16*FRACUNIT;
	mobjinfo[thing].height = 56*FRACUNIT;
	mobjinfo[thing].mass = 100; // LXShadow changed this
	mobjinfo[thing].damage = MT_THOK;
	mobjinfo[thing].activesound = sfx_None;
	mobjinfo[thing].flags = MF_TRANSLATION|MF_NOTHINK|MF_SHOOTABLE, // flags  Removed MF_SOLID

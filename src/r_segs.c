...
static fixed_t worldtop, worldbottom, worldhigh, worldlow; // changed from int for easy HW conversion
...
void R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2)
{
	...
	// Unused for maps, really only used for testing
	...
	for (times = 0; times < repeats; times++)
	{
		...
		for (dc_x = x1; dc_x <= x2; dc_x++)
		{
			...
			if (maskedtexturecol[dc_x] != MAXSHORT)
			{
				...
				if (curline->dontrenderme && curline->polyseg
					&& ((curline->polyseg->flags & POF_RENDERTOP) || (curline->polyseg->flags & POF_RENDERBOTTOM))
					)
				{
...
void R_RenderThickSideRange(drawseg_t *ds, int x1, int x2, ffloor_t *pfloor)
{
	...
	if (pfloor->master->flags & ML_TFERLINE) // SRB2CBTODO: OpenGL support? Also, better method to do this than line #?
	{
	...
	if (pfloor->flags & FF_TRANSLUCENT)
	{
		...
		if (pfloor->alpha < 12)
			return; // Don't even draw it when less then 12
	...
	//     are not stored per-column with post info anymore in SRB2
	if (textures[texnum]->patchcount == 1)
		colfunc_2s = R_DrawMaskedColumn;                    // Render the usual 2sided single-patch packed texture
	else
	{
	...
	for (dc_x = x1; dc_x <= x2; dc_x++)
	{
		if (maskedtexturecol[dc_x] != MAXSHORT)
		{
			...
			if (((signed int)dc_texturemid > 0 && (spryscale>>FRACBITS > MAXINT / (signed int)dc_texturemid))
				|| ((signed int)dc_texturemid < 0 && (spryscale) && (signed int)(dc_texturemid)>>FRACBITS < (MININT / spryscale)))
			{
				spryscale += rw_scalestep;
				continue;
			}
...
void R_StoreWallRange(const int start, const int stop)
{
	...
	temp.x = v1->x - viewx;
	temp.y = v1->y - viewy;
	t1.x   = (temp.x * viewcos) - (temp.y * viewsin);
	t1.y   = (temp.y * viewcos) + (temp.x * viewsin);
	temp.x = v2->x - viewx;
	temp.y = v2->y - viewy;
	t2.x   = (temp.x * viewcos) - (temp.y * viewsin);
	t2.y   = (temp.y * viewcos) + (temp.x * viewsin);
	...
	hyp = R_PointToDist (curline->v1->x, curline->v1->y); // SRB2CBTODO: The wall range of OpenGL should be like this?
...
#undef ADJUST
	...
	if (!backsector)
	{
		...
		else
		{
			...
			if(frontsector->f_slope)
			{
				...
				z1 = P_GetZAtf(frontsector->f_slope, FIXED_TO_FLOAT(linedef->v1->x), FIXED_TO_FLOAT(linedef->v1->y));
				z2 = P_GetZAtf(frontsector->f_slope, FIXED_TO_FLOAT(linedef->v2->x), FIXED_TO_FLOAT(linedef->v2->y));
				zstep = (z2 - z1) / linedef->len;
				
				z1 += lclip1 * zstep;
				z2 -= (linedef->len - lclip2) * zstep;
				linedef->bottom = centery - ((z1 - viewz) * i1) - 1.0f;
				linedef->bottom2 = centery - ((z2 - viewz) * i2) - 1.0f;
			}
			else
			{      
				linedef->bottom = centery - ((frontsector->floorheight - viewz) * i1) - 1.0f;
				linedef->bottom2 = centery - ((frontsector->floorheight - viewz) * i2) - 1.0f;
			}
			
			linedef->bottomstep = (linedef->bottom2 - linedef->bottom) * pstep;
#else
	...
	else
	{
		...
		worldhigh = backsector->ceilingheight - viewz; // SSLOPE
...
#ifdef SESLOPE
		// ESLOPE Modify worldtop and world high to render walls that would have otherwise been invisible
		if (frontsector->f_slope)
		{
			if (frontsector->f_slope->lowz < backsector->floorheight)
				worldbottom = frontsector->f_slope->lowz - viewz;
			
			if (backsector->f_slope)
			{
				if (frontsector->f_slope->highz < backsector->f_slope->highz)
					worldlow = backsector->f_slope->highz - viewz;
			}
		}
		if (backsector->f_slope) // For lines that were invisible before
		{
			if (backsector->f_slope->highz > backsector->floorheight)
				worldlow = backsector->f_slope->highz - viewz;
			
			if (frontsector->f_slope)
			{
				if (backsector->f_slope->highz > backsector->f_slope->highz)
					worldlow = frontsector->f_slope->highz - viewz;
			}
		}
		
		if (backsector->f_slope)
		{
			if (backsector->f_slope->lowz < frontsector->floorheight)
				worldbottom = backsector->f_slope->lowz - viewz;
		}
#endif
...
//					continue;
	...
	if (backsector)
	{
		...
		// This affects the vertical height of a the bottom texture
		if (worldlow > worldbottom)
		{
#ifdef SESLOPE
			seg_t *line = curline;
			fixed_t lclip2 = line->len;
			fixed_t lclip1 = 0.0f;
			if(backsector->f_slope)
			{
				fixed_t z1, z2, zstep;
				
				z1 = (P_GetZAt(backsector->f_slope, line->v1->x, line->v1->y) - viewz) >> 4;
				z2 = (P_GetZAt(backsector->f_slope, line->v2->x, line->v2->y) - viewz) >> 4;
				zstep = (z2 - z1) / line->len;
				
				z1 += lclip1 * zstep;
				z2 -= (line->len - lclip2) * zstep;
				pixlow = (centeryfrac>>4) - (FixedMul(z1, rw_scale));
				pixlowstep = - FixedMul(z2, rw_scalestep);
			}
			else
#endif
			{ 
				...
				// This controls one vertex
				pixlow = ((centeryfrac>>4) - FixedMul(((backsector->floorheight - viewz)>>4), rw_scale)); // with this too
				// This controls the other
				pixlowstep = -FixedMul (rw_scalestep, worldlow);//- FixedMul(((backsector->floorheight - viewz)>>4), rw_scalestep); // try div/4
				
				/*pixlow = (centeryfrac>>4) - FixedMul (worldlow, rw_scale); // with this too
				// This controls the other
				pixlowstep = -FixedMul (rw_scalestep, worldlow); // try div/4*/
...
#ifdef POLYOBJECTS_PLANESS // Software
			if (curline->polyseg && frontsector
				&& ((curline->polyseg->flags & POF_RENDERTOP) || (curline->polyseg->flags & POF_RENDERBOTTOM))
				)
			{
					...
					(viewz < backsector->floorheight))
				{
					...
					if (ffloor[i].plane->minx > ds_p->x1) // SORTING ds_p = drawsegs

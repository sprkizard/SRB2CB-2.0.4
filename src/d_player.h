...
typedef enum
{
	...
	// For linedef exec 427 - Disable player control
	pw_nocontrol,

	// Is the player in goop?
	pw_ingoop,
#ifdef HUDFADE
	pw_stilltime, // SRB2CBTODO: For when the player isn't moving for the hud
	pw_laststill,
	pw_movingtime,
#endif
...
typedef struct player_s
{
...
#ifdef WALLRUN
	long wallrunning; // Running on the wall
#endif
	...
	// SRB2CBTODO: New stuff in the player struct, must be netgame sent
#ifdef SRB2K
	boolean dobounce; // For bubble shield
#endif

	// These values must be stored in the player struct,
	// because these need to act like constant switches for a player's spin state
	boolean usedspin;
	boolean unspinready;

#ifdef JTEBOTS
	struct bot_s* bot; // Storage if the player is a bot
#endif
	...
	fixed_t itemspeed; // For contolling how fast an object the player is using moves

	// SRB2CBTODO: Custom angle stuff for 2D
	angle_t twodcamangle;
	fixed_t twodcamdist;
	// NEWCB!
	fixed_t twodspeed;
	int fovadd;

	byte grade; // SRB2CBTODO: Grading system, 0 - 100
	//#ifdef REALV
	// This vector stors all movement data that used to be handled by momxyz,
	// this allows for real 3D movement and can be changed relative to a plane
	v3float_t controlvec;

	// The final movement vector after other vector collisions
	// and plane relations have been calculated in one frame
	v3float_t finalvec;
	//#endif

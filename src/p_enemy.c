...
void A_SetBalloonColor(mobj_t *actor);
...
#ifdef PARTICLES
void A_EmitParticle(mobj_t *actor);
void A_EmitLightParticle(mobj_t *actor);
void A_EmitStarParticle(mobj_t *actor);
#endif
...
void A_LightningShield(mobj_t *actor);
#endif
// For p_enemy.c
...
void A_SetSkyBoxMobj(mobj_t *actor);
void A_SetSkyBoxCenterMobj(mobj_t *actor);

typedef fixed_t TVector[4];
typedef fixed_t TMatrix[4][4];

static TVector *VectorMatrixMultiply(TVector v, TMatrix m) // SRB2CBTODO: Replace this with standard code?
{
	static TVector ret;
...
// Most enemies are spawned unaware of all players, but some can be made preaware.
//
...
static boolean P_CheckMeleeRange(mobj_t *actor)
{
	...
	// check height now, so that crawlas cant attack
...
/** Checks for water in a sector.
  ...
  * \sa ::MT_SKIM
  */
static boolean P_WaterInSector(mobj_t *mobj, fixed_t x, fixed_t y) // SRB2CBTODO: Use this for sector watertop and such!
{
	sector_t *sector;
	fixed_t height = -1;
...
// How close a player needs to be to a ring to attract it
...
static boolean P_LookForShield(mobj_t *actor)
{
	...
	sector = actor->subsector->sector; // SRB2CBFIX: Never read!
...
void A_HoodThink(mobj_t *actor)
{
	...
	{
		if ((actor->flags & MFE_VERTICALFLIP && actor->momz < 0)
			|| (!(actor->flags & MFE_VERTICALFLIP) && actor->momz > 0))
	...
	if (P_AproxDistance(actor->x-actor->target->x, actor->y-actor->target->y) > 64*FRACUNIT)
	{
		...
		if (actor && actor->spawnpoint && (actor->spawnpoint->options & MTF_AMBUSH)) // Don't jump
...
void A_SnailerThink(mobj_t *actor)
{
	...
	if (actor->target->z > actor->z)
		P_SetObjectAbsMomZ(actor, actor->info->speed, true);
...
void A_LobShot(mobj_t *actor)
{
	...
	hitspot = P_SpawnMobj(actor->target->x&(64*FRACUNIT-1), actor->target->y&(64*FRACUNIT-1), actor->target->subsector->sector->floorheight, MT_DISS); // SRB2CBTODO: P_GetFloorz and P_GetCeilingZ
...
void A_MonitorPop(mobj_t *actor)
{
	...
	remains = P_SpawnMobj(actor->x, actor->y, actor->eflags & MFE_VERTICALFLIP ? actor->z + actor->height - mobjinfo[actor->info->speed].height
						  : actor->z, actor->info->speed);  // SRB2CBTODO: Are these boxes weird?
	...
	if (item != 0)
	{
		...
		if (actor->eflags & MFE_VERTICALFLIP) // SRB2CBTODO: Strange?
...
void A_CustomPower(mobj_t *actor)
{
	...
	if (spawnshield) // SRB2CBTODO: Workaround for a bug?
...
//
// Description: Awards the player a Sonic 3 style fire shield.
//
// var1 = unused
// var2 = unused
//
void A_FlameShield(mobj_t *actor)
{
	...
	{
		player->powers[pw_flameshield] = true;
		P_SpawnShieldOrb(player);
	}
...
//
// Description: Awards the player a Sonic 3 style bounce shield.
//
// var1 = unused
// var2 = unused
//
void A_BubbleShield(mobj_t *actor)
{
	...
	{
		player->powers[pw_bubbleshield] = true;
		P_SpawnShieldOrb(player);
	}
...
void A_LightningShield(mobj_t *actor)
{
	...
	{
		player->powers[pw_lightningshield] = true;
		P_SpawnShieldOrb(player);
	}
...
// Function: A_SetSkyBoxMobj
//
// Description: Sets the level's skybox to use the view of the mobj this action is called from
//
// var1 = unused
// var2 = unused
//
...
// Function: A_SetSkyBoxCenterMobj
//
// Description: Sets the level's skybox to use the view of the mobj this action is called from,
// but relative to the viewer
//
// var1 = unused
// var2 = unused
//
...
void A_ScoreRise(mobj_t *actor)
{
	P_SetObjectMomZ(actor, actor->info->speed, false, false); // make logo rise!
}
...
void A_ParticleSpawn(mobj_t *actor)
{
	...
	spawn->momz = speed;
	actor->tics = actor->spawnpoint->extrainfo*NEWTICRATERATIO + 1;
}
...
void A_BunnyHop(mobj_t *actor)
{
	...
	if (P_IsObjectOnGround(actor))
	{
		P_SetObjectMomZ(actor, locvar1*FRACUNIT, false, false); // make it hop!
...
void A_BubbleRise(mobj_t *actor)
{
	...
	{
		P_SetObjectMomZ(actor, FixedDiv(6*FRACUNIT, 5*FRACUNIT), false, false); // make bubbles rise!
	}
	else
	{
		P_SetObjectMomZ(actor, locvar2, true, false); // make bubbles rise!
...
void A_AttractChase(mobj_t *actor)
{
	...
	// spilled rings flicker before disappearing, if they aren't collected
#ifdef RINGSCALE
	if (!(actor->eflags & MFE_EXTRAFLAG2))
#endif
	{
		if (leveltime & 1*NEWTICRATERATIO && actor->type == (mobjtype_t)actor->info->reactiontime && actor->fuse && actor->fuse < 2*TICRATE)
			actor->flags2 |= MF2_DONTDRAW;
		else
			actor->flags2 &= ~MF2_DONTDRAW;
	}
	...
	if (actor->tracer && actor->tracer->player
		&& (!actor->tracer->player->powers[pw_ringshield]
#ifdef SRB2K
		|| !actor->tracer->player->powers[pw_lightningshield]
#endif
			)
...
void A_FishJump(mobj_t *actor)
{
	...
	if (P_IsObjectOnGround(actor)
		|| ((actor->eflags & MFE_VERTICALFLIP) && actor->z >= actor->watertop - (64 << FRACBITS))
		|| (!(actor->eflags & MFE_VERTICALFLIP) && actor->z <= actor->watertop - (64 << FRACBITS)))
	{
		fixed_t jumpval;
...
void A_ThrownRing(mobj_t *actor)
{
	...
	if (leveltime % (TICRATE/7) == 0)
	{
		...
		if (ring)
		{
			P_SetTarget(&ring->target, actor); // Kalaron: Gotta set the actor's target across all clients
			ring->flags |= MF_TRANSLATION;
	...
	if (leveltime & 1*NEWTICRATERATIO && actor->fuse > 0 && actor->fuse < 2*TICRATE)
		actor->flags2 |= MF2_DONTDRAW;
	else
		actor->flags2 &= ~MF2_DONTDRAW;
	...
	if (actor->tracer)
	{
		...
		// This is for the homing rings, not the regular rings
		if (actor->tracer && (actor->tracer->health)
			&& actor->tracer->player && (actor->tracer->player->powers[pw_ringshield]
#ifdef SRB2K
			|| actor->tracer->player->powers[pw_lightningshield]
#endif
			))// Already found someone to follow.
		{
	...
	actor->lastlook %= MAXPLAYERS; // SRB2CBTODO: What us this?
...
void A_GrenadeRing(mobj_t *actor)
{
	...
	// SRB2CBTODO: Make the grenade beep faster as it comes closer to exploding
	if (leveltime % 35 == 0)
		S_StartSound(actor, actor->info->activesound);

	// Use blockmap to check for nearby shootables
	yh = (unsigned int)(actor->y + explodedist - bmaporgy)>>MAPBLOCKSHIFT;
	yl = (unsigned int)(actor->y - explodedist - bmaporgy)>>MAPBLOCKSHIFT;
	xh = (unsigned int)(actor->x + explodedist - bmaporgx)>>MAPBLOCKSHIFT;
	xl = (unsigned int)(actor->x - explodedist - bmaporgx)>>MAPBLOCKSHIFT;
...
void A_SetSolidSteam(mobj_t *actor)
{
	...
	P_SetObjectMomZ (actor, 1, true, true);
}
...
void A_JetChase(mobj_t *actor)
{
	...
	if (ultimatemode)
	{
		if (actor->z < (actor->target->z + actor->target->height + (64<<FRACBITS)))
			P_SetObjectAbsMomZ(actor, FRACUNIT/2, true);
		else
			P_SetObjectAbsMomZ(actor, -FRACUNIT/2, true);
	}
	else
...
void A_JetbThink(mobj_t *actor)
{
	...
	if (nextsector->ceilingheight < actor->height)
		P_SetObjectAbsMomZ(actor, -5*FRACUNIT, true);
...
void A_MinusDigging(mobj_t *actor)
{
	...
	// Play digging sound
	if (!(leveltime & 15*NEWTICRATERATIO))
		S_StartSound(actor, actor->info->activesound);
...
void A_DetonChase(mobj_t *actor)
{
	...
	if (actor->reactiontime == -42)
	{
		...
		// Variable re-use
		xyspeed = (P_AproxDistance(actor->tracer->x - actor->x,
								   P_AproxDistance(actor->tracer->y - actor->y, actor->tracer->z - actor->z))>>(FRACBITS+6));
...
void A_CapeChase(mobj_t *actor)
{
	...
	if (locvar1 & FRACUNIT) // SRB2CBTODO: Why isn't & 65535 FRACTUNIT instead?
		chaser = actor->tracer;
	else
		chaser = actor->target;
	...
	{
		if (chaser && cv_devmode)
			CONS_Printf("A_CapeChase the object chased (object type %d) has no health, cape removed!\n", chaser->type);
...
// var2 = unused
//
void A_MaceRotate(mobj_t *actor) // SRB2CBTODO: Find a way to interpoliate swinging stuff
{
	TVector v;
	TVector *res;
...
*/
	...
	// SRB2CBTODO: This vector thing can be just a bit too blocky, interpoliate!
	actor->x += v[0];
	actor->y += v[1];
	actor->z += v[2];
	...
	if ((leveltime & 63*NEWTICRATERATIO) && (actor->type == MT_BIGMACE || actor->type == MT_SMALLMACE) && actor->target->type == MT_MACEPOINT)
		S_StartSound(actor, actor->info->activesound);
}

// Function: A_SnowBall
//
// Description: Moves an object like A_MoveAbsolute in its current direction,
// using its Speed variable for the speed. It also sets a timer for the object to disappear.
//
...
void A_CrawlaCommanderThink(mobj_t *actor)
{
	...
	// Pogo Mode
	if (!actor->fuse && actor->health == 1 && P_IsObjectOnGround(actor))
	{
...
void A_RingExplode(mobj_t *actor)
{
	...
	// NOTE: This code would be very CPU-intense if it was much higher
	for (d = 0; d < 16; d++)
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		...
		if (mo2->flags & MF_SHOOTABLE)
		{
			...
			// SRB2CBTODO: Bombs can repel you in the game, so you can use your own bombs to get around,
			// requires lots of skill though, but very fun in the game
			// SRB2CBTODO: Make bots smart enough to use this some day

			angle_t jumpangle;

			// Repel away from the explosion
			jumpangle = R_PointToAngle2(actor->x, actor->y, mo2->x, mo2->y);

			// Horizontal momentum
			P_InstaThrust(mo2, jumpangle, 10*FRACUNIT);

			mo2->momz += 9*FRACUNIT;

            /* LXShadow: ...
                         So *this* is what broke the grenades! I'll bet the bombs were broken too. Fixed.*/
			// Don't damage a player if he thew this bomb
			if (!(actor->target && mo2->player && (mo2->player == actor->target->player) && actor->type == MT_THROWNEXPLOSION))
...
void A_OldRingExplode(mobj_t *actor) {
	...
	{
		mo->flags |= MF_TRANSLATION;
		if (gametype != GT_CTF) // LXShadow: Reverted this code, only for consistency and that I don't think it's actually being used. (A_*Old*RingExplode)
            mo->color = actor->target->color; //copy color
		else if (actor->target->player->ctfteam == 2)
			mo->color = 8;
	}
...
void A_MixUp(mobj_t *actor)
{
	...
	else if (numplayers == 2) // Special case -- simple swap
	{
		...
		// SRB2CB, you mixed the positions, so mix the scales as well
		USHORT scale1 = players[one].mo->scale;
		USHORT scale2 = players[two].mo->scale;
		players[one].mo->scale = scale2;
		players[two].mo->scale = scale1;
	...
	else
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
				...
				&& players[i].mo && players[i].mo->health > 0 && !players[i].exiting)
			{
				...
				// SRB2CB:
				players[i].mo->scale = players[teleportfrom].mo->scale;
...
void A_RecyclePowers(mobj_t *actor)
{
	...
	else if (numplayers == 2) //simple swap is all that's needed
	{
		...
		// SRB2 2.0 weapons need to be swapped too
...
void A_Boss2Chase(mobj_t *actor)
{
	...
	actor->target->angle += FixedAngle(FixedMul(actor->watertop,(actor->info->spawnhealth*(FRACUNIT/4)*3)/speedvar));
...
void A_Boss2Pogo(mobj_t *actor)
{
	if ((!(actor->eflags & MFE_VERTICALFLIP) && (actor->z <= actor->floorz + 8*FRACUNIT) && actor->momz <= 0)
		|| ((actor->eflags & MFE_VERTICALFLIP) && (actor->z >= actor->ceilingz - 8*FRACUNIT) && actor->momz >= 0))
	{
		P_SetMobjState(actor, actor->info->raisestate);
		// Pogo Mode
	}
	else if ((((actor->eflags & MFE_VERTICALFLIP) && actor->momz > 0) || (!(actor->eflags & MFE_VERTICALFLIP) && actor->momz < 0))
				 && actor->reactiontime)
	{
		const fixed_t ns = 3*FRACUNIT;
		mobj_t *goop;
		...
		angle_t fa;
		byte i;
		// Spray some goop in all 8 directions!
		for (i = 0; i < 8; i++)
		{
			actor->movedir++;
			actor->movedir %= NUMDIRS;
			...
			goop->momx = FixedMul(FINESINE(fa),ns);
			goop->momy = FixedMul(FINECOSINE(fa),ns);
			P_SetObjectMomZ(goop, 4*FRACUNIT, false, false);
...
void A_BuzzFly(mobj_t *actor)
{
	...
	if (!actor->target || !(actor->target->flags & MF_SHOOTABLE))
	{
		...
		// Kalaron: Fixed so momx gets initialized too
		actor->momx = 0;
		actor->momy = 0;
		actor->momz = 0;
		P_SetMobjState(actor, actor->info->spawnstate);
...
void A_GuardChase(mobj_t *actor)
{
	...
	{
		if (leveltime && leveltime % 2 == 0) // SRB2CB: Eggguards, don't ALWAYS turn around
		{
			P_NewChaseDir(actor);
...
void A_SpawnObjectRelative(mobj_t *actor)
{
	...
	x = (signed short)(locvar1>>16);
	y = (signed short)(locvar1&65535); // SRB2CBTODO: This should be FRACUNIT and FRACBITS
	z = (signed short)(locvar2>>16);
	type = (mobjtype_t)(locvar2&65535);
...
void A_FindTarget(mobj_t *actor)
{
	...
	if (cv_devmode == 2) // SRB2CBTODO: Change devmode into detailed, specific suite of stuff
...
// Function: A_SetBalloonColor
//
// Description: Simple function just to setup a balloon's color
//
// var1 = unused
// var2 = unused
//
void A_SetBalloonColor(mobj_t *actor)
{
	...
	{
		byte bcolor;
		bcolor = actor->angle/(ANG45/45); // Use the ballon's angle to determine
...
void A_ZThrust(mobj_t *actor)
{
	...
	P_SetObjectMomZ(actor, locvar1*FRACUNIT, !(locvar2 >> 16), false);
}
...
#ifdef PARTICLES
// Particle scripting addtion
// Function: A_EmitParticle
//
...
void A_EmitParticle(mobj_t *actor)
{
	int locvar1 = var1;
	int locvar2 = var2;
	P_Particles(actor, MT_PARTICLE, 0+locvar1, FRACUNIT, FRACUNIT/2, TICRATE, false, false, 0+locvar2, 1, 0);
}
// Particle scripting addtion
// Function: A_EmitLightParticle
//
// Description: Emits shiny particles
//
// var1 = frequency
// var2 = color
//
void A_EmitLightParticle(mobj_t *actor)
{
	int locvar1 = var1;
	int locvar2 = var2;
	P_Particles(actor, MT_LIGHTPARTICLE, 0+locvar1, FRACUNIT, FRACUNIT/2, TICRATE, false, false, 0+locvar2, 1, 0);
}
// Function: A_EmitStarParticle
//
// Description: Emits particles, just for sparkles
//
// var1 = gravity on/off
// 0 = gravity
// 1 = no gravity
...
void A_EmitStarParticle(mobj_t *actor)
{
	...
	int locvar2 = var2;
	if (locvar1 == 0)
		P_Particles(actor, MT_SPARK, 0+locvar2, FRACUNIT, FRACUNIT/2, TICRATE, true, false, 0, 1, 0);
	if (locvar1 == 1)
		P_Particles(actor, MT_SPARK, 0+locvar2, FRACUNIT, FRACUNIT/2, TICRATE, false, false, 0, 1, 0);
}
#endif

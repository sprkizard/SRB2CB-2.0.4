...
void R_LoadTextures(void)
{
	...
	// NOTE: The game works by memory, so if the same file is added multiple times,
	// the amount of textures will also increase
	long totaltexturesadded = 0;
	...
	for (i = 0, w = 0; w < numwadfiles; w++)
	{
...
#ifdef HAVE_PNG
				if (W_LumpIsPng(W_GetNumForName(texture->name)) == true) // Make sure to read data correctly if it's a png file
				{
					int w =0, h=0;
					const char *filename = textures[i]->name;
		...
		totaltexturesadded += texend - texstart;
		
	}
	
	CONS_Printf("Added %i TX_ textures\n", (int)(abs(lasttexamount - totaltexturesadded)));
...
// Ok, so the game actually doesn't USE F_START or F_END to load flats,
// the game just searches for the name of a flat on the fly!

// So this makes the job easier, just check to see if the found lump is a PNG
// and if so, flag it for special PNG rendering in hardware mode
int R_GetFlatNumForName(const char *name)
{
	...
	if (lump == LUMPERROR)
	{
		...
		lump = W_CheckNumForName("FLOOR1_2"); // Default flat texture
	}
...
//
long R_CheckTextureNumForName(const char *name, USHORT sidenum) // ZTODO: This is called too many times, no?
{
	size_t i;

...
static boolean P_CrossSubsector(size_t num, register los_t *los)
{
	...
	if (num >= numsubsectors)
		I_Error("P_CrossSubsector: ss %i with numss = %i", (int)num, (int)numsubsectors);
#endif
	...
	for (count = subsectors[num].numlines; --count >= 0; seg++)  // check lines
	{
		...
		// no wall to block sight with? // ESLOPE
...
// Uses REJECT.
//
boolean P_CheckSight(mobj_t *t1, mobj_t *t2) // SRB2CBTODO: Make this work when FOFs are bewtween places(add FOFs to the blockmap list?)
{
	const sector_t *s1 = t1->subsector->sector;
	const sector_t *s2 = t2->subsector->sector;
	//int pnum = (s1-sectors)*numsectors + (s2-sectors); // Crashy
	los_t los;
	...
	// SRB2CBTODO: Make a dynamic reject system for optimal stuff

	// SRB2CBTODO: Crashy and confusing!
	...
	// SRB2CBTODO: This does not work if an FOF is in between the start and end points

...
static short itemOn = 1; // menu item skull is on // SRB2CBTODO: This is not a "skull"
static short MenuAnimCounter = 10; // skull animation counter
...
static const char *ALREADYPLAYING = "End the current game? (Y/N)\n"; // SRB2CBTODO: Make translate-ready!
...
static void M_ConfirmSpectate(void);
...
static menuitem_t MainMenu[] =
{
	...
	{IT_CALL    | IT_STRING, NULL, "Extras"           , M_SecretsMenu,         84},
	...
	{IT_CALL    | IT_STRING, NULL, "Exit Game"        , M_QuitSRB2,           116},
};
...
static void M_ConnectMenu(INT32 choice)
{
	...
	if (modifiedgame)
	{
		M_StartMessage("The game has been modified\nwith netgame files/cheats/wads/etc.\nPlease restart SRB2 before\nconnecting.", NULL, MM_NOTHING);
		return;
	}
...
static void M_ChangeLevel(void)
{
	char mapname[6];
...
static menuitem_t ServerMenu[] =
{
	...
	{IT_STRING|IT_CVAR,              NULL, "Level",                 &cv_nextmap,        75},
...
static inline void M_StartSplitServerMenu(void)
{
	...
	M_SetupNextMenu(&Serverdef); // SRB2CBTODO: Not important, but Serverdef when NONET is defined
}
...
static menuitem_t SetupMultiPlayerMenu[] =
{
	...
	{IT_KEYHANDLER | IT_STRING,   NULL, "Your player", M_HandleSetupMultiPlayer,  96},
...
static void M_DrawConnectIPMenu(void)
{
	...
	if (itemOn == 0 &&
	    MenuAnimCounter < 4)   //blink cursor
...
static void M_DrawSetupMultiPlayerMenu(void)
{
	...
	// draw text cursor for name
	if (!itemOn && MenuAnimCounter < 4) // blink cursor
...
#ifdef PLAYERMD2SKIN // TODO (Turtle Man#1#): if the skin has a model, render the model insted of the sprite.
	md2_t *md2 = NULL;
	if (cv_grmd2.value && R_SkinAvailable(setupm_cvskin->string) != -1)
	{
		md2 = &md2_playermodels[R_SkinAvailable(setupm_cvskin->string)];
		md2->skin = R_SkinAvailable(setupm_cvskin->string);
	}
#endif
	...
	if (md2)
	{
		GLPatch_t *sprgpatch; // sprite patch converted to hardware
		GLPatch_t *gpatch;
		FSurfaceInfo Surf;
		FTransform p;
		int frame;
		...
		// Sprite Gfx.
		sprgpatch = (GLPatch_t *)patch;

		// Color map Sprite Gfx.
		HWR_GetMappedPatch(sprgpatch, colormap);
		...
		GL_DrawPolygon(&Surf, NULL, 4, blend|PF_Modulated|PF_Clip|PF_MD2);
		//

		if (!md2->model)
		{
#ifdef WADMOD
			md2->model = md2_readModel(md2->lumpname);
#else
...
#ifdef WADTEX2
			MD2_LoadTexture(md2, sprframe->lumppat[0]);
#else
		MD2_LoadTexture(md2);
#endif
			...
			GL_SetTexture(&sprgpatch->mipmap);

		//FIXME: this is not yet correct
		frame = (multi_state->frame & FF_FRAMEMASK) % md2->model->header.numFrames;

#if 1
		p.x = 0.0f; // What should this be?
		p.y = -(cv_numlaps.value*1.0f); // What should this be?
		p.z = -(cv_pointlimit.value*1.0f); // What should this be?
		p.angley = -90.0f;
		p.anglex = 0.0f;
#else
		p.x = -5.0f; // What should this be?
		p.y = 160.0f-md2->offset; // What should this be?
		p.z = ZCLIP_PLANE*20; // ZCLIP_PLANE-1; // What should this be?
		p.angley = -90.0f;
		p.anglex = 0.0f;
#endif
		GL_DrawMD2Menu(md2->model->glCommandBuffer, &md2->model->frames[frame], &p, md2->scale);
	}
	else
#endif
	// draw player sprite
...
static void M_DrawSetupChoosePlayerMenu(void)
{
	...
	// Black BG
	V_DrawFadeScreen(); // Kalaron: Cooler and far more elegant :D
...
static void M_NewGame(void)
{
	...
	CV_SetValue(&cv_newgametype, GT_COOP);
...
// Chose the player you want to use
static void M_ChoosePlayer(INT32 choice)
{
	int skinnum;
...
{IT_STRING  | IT_CVAR, NULL, "Control per key", &cv_controlperkey, 100},
};
...
{IT_STRING  | IT_CVAR, NULL, "Analog Control", &cv_useranalog,  60},
...
{IT_STRING  | IT_CVAR, NULL, "Analog Control", &cv_useranalog2,  60},
...
{IT_STRING | IT_CVAR, NULL, "Sound Test",         &cv_soundtest,       25},
...
static void M_SecretsMenu(INT32 choice)
{
	...
	// SRB2CBTODO: Use this method for other things, it's very simple

	// Disable all the menu choices
	(void)choice;
	for (i = unlockchecklist+1; i < secrets_end; i++) // from the start of the secret's list to the end
...
static menuitem_t VideoOptionsMenu[] =
{
...
#endif
#ifdef HWRENDER
	{IT_CALL|IT_WHITESTRING, NULL, "OpenGL Options...",  M_OpenGLOption,    20},
#endif
	...
	{IT_STRING | IT_CVAR,    NULL, "FPS Meter",           &cv_showfps,       90},
};
...
{
{IT_STRING | IT_CVAR, NULL, "Bot AI",                &cv_botai,        0},
...
#ifdef SHARDS_MODE
{IT_STRING | IT_SUBMENU, NULL, "Shards options...",          &ShardsOptionsDef,       70},
#endif
};
...
#ifdef SHARDS_MODE
//===========================================================================
//                       Shards Mode OPTIONS MENU
//===========================================================================

static menuitem_t ShardsOptionsMenu[] =
{
{IT_STRING | IT_CVAR, NULL, "Number of Jewel Shards",          &cv_jewelshards,   10},
};

menu_t ShardsOptionsDef =
{
"M_OPTTTL",
"OPTIONS",
sizeof (ShardsOptionsMenu)/sizeof (menuitem_t),
&GametypeOptionsDef,
ShardsOptionsMenu,
M_DrawGenericMenu,
60, 40,
...
static void M_ToggleMIDI(void) // SRB2CBTODO: This SHOULDN'T make the entire system quiet on Windows!!!
{
	if (nomidimusic)
	{
		nomidimusic = false;
...
static void M_DrawVideoMode(void)
{
	...
	if (MenuAnimCounter < 4) // use the Skull anim counter to blink the cursor
	{
		i = 41 - 10 + ((vidm_current / vidm_column_size+1)*7*13);
...
static void M_DrawLoad(void)
{
	...
	for (i = MAXSAVEGAMES + saveMenuLocation - 2 + offset, j = 0;i <= MAXSAVEGAMES + saveMenuLocation + 2 + offset; i++, j++)
	{
		...
		if (savegameinfo[i%MAXSAVEGAMES].lives == -42)
			V_DrawString(LoadDef.x-6,LoadDef.y + LINEHEIGHT * j - 1 + ymod, rendermode == render_opengl ? V_TRANSLUCENT : 0,"NO DATA");
	...
	V_DrawCenteredString(BASEVIDWIDTH/2, 40, 0, "Backspace to delete, Shift for no save");
	M_DrawGameStats();
...
static void M_LoadSelect(INT32 choice)
{
	// Make sure to check that the file trying to be accessed is ok before quiting any games
	if (FIL_ReadFileOK(va(savegamename, saveSlotSelected)))
		if (G_CheckGame((unsigned int)choice))
			return;
...
static void M_HandleLoadSave(INT32 choice)
{
	...
	switch (choice)
	{
		...
		case KEY_BACKSPACE:
			...
			if (savegameinfo[saveMenuLocation].lives != -42) // SRB2CBTODO: Better way to check if no data?
...
void M_EndGame(void)
{
	...
	M_StartMessage(va("%s%s", text[ENDGAME], text[PRESSCANCEL]), M_EndGameResponse, MM_YESNO);
}
...
void M_QuitResponse(int ch)
{
	...
	// If you're in a netgame or in debug, you won't get the pretty fade effect :P
	//if (!(netgame || devparm))
	{
		if (quitsounds[(gametic>>2)&7])
			S_StartSound(NULL, quitsounds[(gametic>>2)&7]); // SRB2CBTODO: Need sounds?

		{
			// Fade to black first
			if (rendermode != render_none)
			{
				// Capture the current screen for wipe storage
				F_WipeStartScreen();
				...
				// Capture the now black screen
				F_WipeEndScreen(0, 0, vid.width, vid.height);

				F_RunWipe(TICRATE); // Now fade from the current screen to black
			}
		}
	}

	// F_RunWipe delays I_Quit till it's done
	I_Quit();
}

static void M_QuitSRB2(void)
{
	M_StartMessage(va("%s\n%s%s", text[QUITMSG], text[PRESSQUIT], text[PRESSCANCEL]), M_QuitResponse, MM_YESNO);
}
...
void M_DrawTextBox(int x, int y, int width, int boxlines)
{
	...
	if (rendermode == render_soft)
		...
		V_DrawTranslucentPatch(cx, cy, 0, W_CachePatchNum(viewborderlump[BRDR_TL], PU_CACHE));
	cy += boff;
	...
	for (n = 0; n < boxlines; n++)
	{
		if (rendermode == render_soft)
			...
			V_DrawTranslucentPatch(cx, cy, V_WRAPY, p);
		cy += step;
	}
	if (rendermode == render_soft)
		V_DrawScaledPatch(cx, cy, 0, W_CachePatchNum(viewborderlump[BRDR_BL], PU_CACHE));
	else
		V_DrawTranslucentPatch(cx, cy, 0, W_CachePatchNum(viewborderlump[BRDR_BL], PU_CACHE));

	// draw middle
	V_DrawFlatFill(x + boff, y + boff, width*step, boxlines*step, st_borderpatchnum, 0);
	...
	while (width > 0)
	{
		if (rendermode == render_soft)
			...
			V_DrawTranslucentPatch(cx, cy, V_WRAPX, W_CachePatchNum(viewborderlump[BRDR_T], PU_CACHE));
		if (rendermode == render_soft)
			V_DrawScaledPatch(cx, y + boff + boxlines*step, V_WRAPX, W_CachePatchNum(viewborderlump[BRDR_B], PU_CACHE));
		else
			V_DrawTranslucentPatch(cx, y + boff + boxlines*step, V_WRAPX, W_CachePatchNum(viewborderlump[BRDR_B], PU_CACHE));
		width--;
		cx += step;
	}
	...
	if (rendermode == render_soft)
		...
		V_DrawTranslucentPatch(cx, cy, 0, W_CachePatchNum(viewborderlump[BRDR_TR], PU_CACHE));
	cy += boff;
	...
	if (rendermode == render_soft)
		...
		V_DrawTranslucentPatch(cx, cy, 0, W_CachePatchNum(viewborderlump[BRDR_BR], PU_CACHE));
}


//
//  Textbox, with flags.
//  I'm not going to sift through the code and modify all the old M_DrawTextBox lines
//  to give them flags (too lazy, too many ifdefs, etc.) so this will have to do.
//
void M_DrawTextBoxWithFlags(int x, int y, int width, int boxlines, int flags)
{
	patch_t *p;
	int cx, cy, n;
	int step, boff;
					...
					menumessagetype_t itemtype)
{
	...
	// compute max length and the numbers of lines
...
{
	int x;
	//added : 02-02-98 : centre on 320(BASEVIDWIDTH), because V_DrawString centers on vid.width...
...
boolean M_Responder(event_t *ev)
{
	...
	{
		if ((shiftdown || capslock) && ch >= 32 && ch <= 127)
			ch = shiftxform[ch];
		routine(ch);
		return true;
	}
...
#ifdef SHARDS_MODE
	if (!(gametype == GT_MATCH || gametype == GT_TAG || gametype == GT_CTF || gametype == GT_SHARDS) || splitscreen)
#else
...
void M_Ticker(void)
{
	...
	if (--MenuAnimCounter <= 0)
		MenuAnimCounter = 8 * NEWTICRATERATIO;
...
{
	{IT_SUBMENU|IT_WHITESTRING, NULL, "Texture Filter...", &OGL_TexDef,          10},
	{IT_STRING|IT_CVAR,         NULL, "Low-End Hardware Mode",&cv_grcompat,      30},
#ifdef _WINDOWS
	{IT_STRING|IT_CVAR,         NULL, "Fullscreen",      &cv_fullscreen,       40},
#endif
	{IT_STRING|IT_CVAR,         NULL, "Color Quality",    &cv_scr_depth,        50},

	{IT_STRING | IT_CVAR,       NULL, "Polygon Count",   &cv_polycount,     60},
	//	{IT_STRING|IT_CVAR|IT_CV_SLIDER,
	//	NULL, "Translucent HUD", &cv_grtranslucenthud, 60},
	{IT_SUBMENU|IT_WHITESTRING, NULL, "Fog...",          &OGL_FogDef,          70},
	{IT_SUBMENU|IT_WHITESTRING, NULL, "Gamma...",        &OGL_ColorDef,        80},
...
static menuitem_t OGL_TexMenu[] =
{
	{IT_STRING|IT_CVAR,  NULL, "Texture Filter",  &cv_grfiltermode,  20},
	{IT_STRING|IT_CVAR,  NULL, "Anisotropic",     &cv_granisotropicmode,30},
};

static menuitem_t OGL_DevMenu[] =
{
	{IT_STRING | IT_CVAR, NULL, "3D Objects (MD2)", &cv_grmd2, 20},
};
...
menu_t OGL_TexDef =
{
	"M_OPTTTL",
	"OPTIONS",
	sizeof (OGL_TexMenu)/sizeof (menuitem_t),
	&OpenGLOptionDef,
	OGL_TexMenu,
	M_DrawGenericMenu,
	60, 40,
...
static void M_OGL_DrawFogMenu(void)
{
	...
	M_DrawGenericMenu(); // use generic drawer for cursor, items and title
	/*
	 // SRB2CBTODO: Find a use for this OpenGL hex menu thingy
	 ...
	 if (itemOn == FOG_COLOR_ITEM && MenuAnimCounter < 4)
...
static void M_OpenGLOption(INT32 choice)
{
	...
	if (rendermode != render_soft)
		...
		M_StartMessage("Restart in OpenGL mode to change these options\n", NULL, MM_NOTHING);
}

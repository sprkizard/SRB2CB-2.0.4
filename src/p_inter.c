...
void P_ForceFeed(const player_t *player, int attack, int fade, tic_t duration, int period)
{
	...
	if (!player)
		return;
	Basicfeed.Duration = (ULONG)(duration * (100L/TICRATE));
...
static boolean P_CanPickupItem(player_t *player, boolean weapon)
{
	if (!player)
	{
		CONS_Printf("P_CanPickupItem: NULL player");
		return false;
	}

	(void)weapon; //unused // SRB2CBTODO: WHY?!

	if (player->powers[pw_flashing] > (flashingtics/4)*3
...
//
// Pickup sparkle for numerous objects.
//
static mobj_t *P_PickupSpark(mobj_t *mo, mobjtype_t type)
{
	mobj_t *spark;

	spark = P_SpawnMobj(mo->x, mo->y, mo->z, type);
	if (mariomode)
		spark->momz = mo->momz;
#if 0 // SRB2CBTODO: Support thing scaling not having collision issues with it
	spark->scale = 1;
	spark->destscale = mo->scale;
#endif
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher, boolean heightcheck)
{
	...
	// Dead thing touching, dead things are assumed to be non solid
	if (toucher->health <= 0)
		return;
	...
	else if ((special->flags & MF_ENEMY) && !(special->flags & MF_MISSILE))
	{
		...
		if (special->type == MT_GSNAPPER
			&& !(toucher->z - toucher->momz > special->z + special->height
				 || (toucher->z + toucher->height) - toucher->momz < special->z))
		{
			// Can only hit snapper from above
			P_DamageMobj(toucher, special, special, 1);
		}
		if (special->type == MT_SHARP
			&& ((special->state >= &states[special->info->xdeathstate]
				 && special->state <= &states[special->info->raisestate])
			|| (special->z < toucher->z - toucher->height/8)))
		{
			P_DamageMobj(toucher, special, special, 1);
		}

		// SRB2CB: With things more than 1.5x your height, need you to ram them at high speeds in order hurt them
		else if (!((player->speed > FIXEDSCALE(40, toucher->scale))
				   && (toucher->player->pflags & PF_SPINNING))
				 && !(player->speed > FIXEDSCALE(45, toucher->scale))
				 && (toucher->scale < (USHORT)((float)special->scale*0.51f)))
		{
			if (((toucher->z <= special->z + special->height && toucher->z + toucher->height >= special->z)
				&& (((toucher->player->pflags & PF_JUMPED)
					 || (toucher->player->pflags & PF_SPINNING)) && !P_IsObjectOnGround(player->mo)))
				|| (toucher->player->pflags & PF_SPINNING))
			{
				if (!(player->mo->eflags & MFE_JUSTBOUNCEDBACK))
				{
					if (!(toucher->player->pflags & PF_SPINNING)) // Don't bounce up when spinning
					{
						// Instead of hurting, just bounce off harmlessly.
						if (toucher->eflags & MFE_VERTICALFLIP)
						{
							if (toucher->momz > -2*FRACUNIT)
								toucher->momz = -2*FRACUNIT; // go at least two fracunits
							if (toucher->momz > 0)
								toucher->momz = -toucher->momz;
						}
						else
				...
				P_DamageMobj(toucher, special, special, 1);
				 ...
				 // Are you touching the side of it?
				 && (toucher->z <= special->z + special->height
							&& toucher->z + toucher->height >= special->z)
		         && (((toucher->player->pflags & PF_NIGHTSMODE)
					  && (toucher->player->pflags & PF_DRILLING))
					 || (toucher->player->pflags & PF_JUMPED)
					 || (toucher->player->pflags & PF_SPINNING)
					 // If the object is less than one-third your size, auto kill it
		             || (toucher->scale >= (USHORT)((float)special->scale*1.33f))
					 // Do you possess the ability to subdue the object?
		             || toucher->player->powers[pw_invulnerability]
					 || toucher->player->powers[pw_super]))
		{
			// SRB2CBTODO: Vertical flip check
			if (((toucher->momz < 0 && !(toucher->eflags & MFE_VERTICALFLIP))
				 || (toucher->momz > 0 && (toucher->eflags & MFE_VERTICALFLIP)))
				// Don't bounce back up when you're more than 2/3 the object's size
				&& !(toucher->scale > (USHORT)((float)special->scale*1.65f)))
				toucher->momz = -toucher->momz;
		 ...
		 && toucher->z < special->z
		 && toucher->player && toucher->player->charability == CA_FLY
		 // Tails can shred stuff with his propeller.
		 && (toucher->player->powers[pw_tailsfly]
		 || toucher->state == &states[S_PLAY_SPC1]
		 || toucher->state == &states[S_PLAY_SPC2]
		 || toucher->state == &states[S_PLAY_SPC3]
		 || toucher->state == &states[S_PLAY_SPC4]))
		{
			if (toucher->momz < 0)
				toucher->momz = -toucher->momz/2;
	...
	{
	// Identify by object type, not sprite!
	switch (special->type)
	{
		...
		case MT_EGGSHIELD:
			{
				...
				else if ((toucher->z <= special->z + special->height // Are you touching the side of it?
						  && toucher->z + toucher->height >= special->z)
						&& (((toucher->player->pflags & PF_NIGHTSMODE)
							 && (toucher->player->pflags & PF_DRILLING))
							|| (toucher->player->pflags & PF_JUMPED)
		...
		case MT_EMBLEM: // Secret emblem thingy
			{
				...
				if (timeattacking || demoplayback)
					return;
				P_SetMobjState(special, S_DISS);
				P_PickupSpark(special, MT_SPARK);
		...
		case MT_HOOPCOLLIDE:
			...
			// Move to the first sprite in the hoop
			for (; special->hprev != NULL; special = special->hprev)
				;
			i = 0;
			...
			return;
		case MT_NIGHTSDRONE:
			// After-mare bonus time/emerald reward in special stages.
			...
			// Initial transformation. Don't allow second chances in special stages!
		...
		case MT_STARPOST:
			...
			player->starpostangle = special->angle; // SRB2CBTODO: Is this wrong?
...
#ifdef HUDFADE
			// Make sure to set the hud back to full alpha
			player->powers[pw_stilltime] = 248;
			player->powers[pw_laststill] = 0;
			player->powers[pw_movingtime] = 0;
#endif
		...
		case MT_EMERHUNT:
			...
			if (special == hunt1)
				hunt1 = NULL;
			else if (special == hunt2)
				hunt2 = NULL;
			else if (special == hunt3)
				hunt3 = NULL;
			...
			for (i = 0; i < MAXPLAYERS; i++)
			{
				...
				if (players[i].emeraldhunt > 0)
					playeremercount += players[i].emeraldhunt;
			}
			if (playeremercount >= 3) // SRB2CBTODO: Turn this into a custom map header
			{
				for (i = 0; i < MAXPLAYERS; i++)
				{
					if (!playeringame[i])
		...
		case MT_AXE:
			{
				...
				junk.tag = 649; // SRB2CBTODO: hard coded tags...some better way to do this?
		...
		case MT_FLINGCOIN:
			...
			P_PickupSpark(special, MT_COINSPARKLE);
		...
		case MT_RING:
		case MT_FLINGRING:
#ifdef RINGSCALE // SRB2CBTODO: CLEAN up my own mess
#ifdef RINGSCALE
			if (!(special->eflags & MFE_EXTRAFLAG2) && ((special->eflags & MFE_EXTRAFLAG) || (!(special->eflags & MFE_EXTRAFLAG) && !(special->eflags & MFE_EXTRAFLAG2))))
#endif
			{
				if (!(P_CanPickupItem(toucher->player, false)))
					return;

				P_PickupSpark(special, MT_SPARK); // SRB2CBTODO: This has an issue when used with scaling! FIX!

#ifdef RINGSCALE
				// If this ring has been flagged to have been attracted by a player's shield,
				// make sure that there is not a continous addtion to the player score
				if (special->tracer && (special->eflags & MFE_EXTRAFLAG))
					P_GivePlayerRings(player, 1, (special->type == MT_FLINGRING));
				else if (!(special->eflags & MFE_EXTRAFLAG2) && !(special->eflags & MFE_EXTRAFLAG)
						 && !(toucher->player->powers[pw_lightningshield] || toucher->player->powers[pw_ringshield]))
#endif
					P_GivePlayerRings(player, 1, (special->type == MT_FLINGRING));
...
#ifdef HUDFADE
				// Make sure to set the hud back to full alpha // SRB2CBTODO: Make this a function
				player->powers[pw_stilltime] = 248;
				player->powers[pw_laststill] = 0;
				player->powers[pw_movingtime] = 0;
#endif
...
#ifdef RINGSCALE
				if (!(special->eflags & MFE_EXTRAFLAG))
				{
					special->destscale = 1;
					special->fuse = (1*TICRATE)/2; // Make sure the ring dissapears!
				}

				special->eflags |= MFE_EXTRAFLAG2; // Flag that this mobj has been touched already
#endif
			}
#else
			if (!(P_CanPickupItem(toucher->player, false)))
		...
		case MT_FLINGBALL:
			...
			for (i = 0; i < 8; i++)
				P_SpawnParaloop(special->x, special->y, special->z + special->height,
								192*FRACUNIT, 8, MT_BLUEBALLSPARK, i*(ANGLE_45/2), true, true);
		...
		case MT_EXPLODEPICKUP:
			...
			temp = P_SpawnMobj(special->x, special->y, special->z, special->type); // SRB2CBTODO:! ERROR: ERROR: issue with this
			temp->flags &= ~MF_SPECIAL;
			temp->health = 0;
			temp->momz = FRACUNIT;
			temp->fuse = special->info->damage;
...
#ifdef RINGSCALE // SRB2CBTODO: Support thing scaling not having collision issues with it
	switch (special->type)
	{

		case MT_RING:
		case MT_FLINGRING:
			// Play the sound first and dissmiss the mobj after it is done scaling down
			if (special->fuse == (1*TICRATE)/2)
			{
				// player->mo so players can hear others pick up rings
				if (sound != sfx_None)
					S_StartSound(player->mo, sound);
			}
			// Unstable
			if ((special->eflags & MFE_EXTRAFLAG) || (!(special->eflags & MFE_EXTRAFLAG) && (special->scale == special->destscale || special->scale < 2)))
			{
				P_SetMobjState(special, S_DISS);
				P_UnsetThingPosition(special);
				special->flags &= ~MF_NOBLOCKMAP;
		...
		default:
			...
			// player->mo so you could hear others pick up rings
			if (sound != sfx_None)
				S_StartSound(player->mo, sound);
			//return;
			break;
...
#ifdef JTEBOTS
	if (gametype == GT_COOP && target->player->bot)
		return;
#endif
...
void P_CheckSurvivors(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++) // Find the number of taggers, survivors and spectators.
	{
		if (playeringame[i])
		{
...
void P_KillMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source)
{
	...
	// A dead target is no longer shootable
	...
	if (target->type != MT_PLAYER && !(target->flags & MF_MONITOR))
		target->flags |= MF_NOGRAVITY; // Don't drop

	// Let EVERYONE know what happened to a player!
	...
	if (source && source->player)
	{
		...
		// Award Score to the player
		{
			...
			// SRB2CBTODO: Scale the score, the bigger your enemy is than you,
			// the more points you get, reverse if the enemy is smaller
			score = score/source->scale*100;
	...
	if (target->player)
	{
...
#ifdef HUDFADE
		// Make sure to set the hud back to full alpha
		target->player->powers[pw_stilltime] = 248;
		target->player->powers[pw_laststill] = 0;
		target->player->powers[pw_movingtime] = 0;
#endif

		// SRB2CBTODO: Make a little before life amount and after obvious with an animation

		// Coop // SRB2CBTODO: Per level option for no lives
		if ((gametype == GT_COOP && !G_IsSpecialStage(gamemap))
			|| gametype == GT_RACE)
		{
			if (!timeattacking) // You don't need to lose a life in timeattack, ever, infinate tries
			{
				target->player->lives -= 1; // Lose a life

				if (target->player->lives <= 0)
				{
					if (P_IsLocalPlayer(target->player))
					{
						S_StopMusic(); // Stop the Music!
						S_ChangeMusic(mus_gmover, false); // Music changes when dead
					}
				}
			}
		}
		...
		{
			// Don't die in automap view, // SRB2CBTODO: Fix automap
			// switch view prior to dying
			if (automapactive)
				AM_Stop();

			// Re-center view for next life...
			localaiming = 0;
		}
		...
		if (gametype == GT_TAG && !(target->player->pflags & PF_TAGIT)
			&& (!source || !source->player) && !(target->player->spectator))
		{
	...
	if (mariomode // Don't show birds, etc. in Mario Mode
#ifdef CHAOSISNOTDEADYET
	...
	if (target->flags & MF_ENEMY)
	{
		...
		// Animals can be different sizes too! :D
		mo->destscale = target->scale;
		P_SetScale(mo,mo->destscale);
#ifdef PARTICLES
		P_Particles(target, MT_PARTICLE, 255, 1*FRACUNIT, 1*FRACUNIT, TICRATE+5, false, false, 13, 3, 0);
		P_Particles(target, MT_PARTICLE, 255, 1*FRACUNIT, 1*FRACUNIT, TICRATE+5, false, false, 13, 3, 0);
#endif
	}
}
...
static inline boolean P_TagDamage(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
	...
	// Check for a shield
	if (target->player->powers[pw_forceshield] || target->player->powers[pw_jumpshield]
		|| target->player->powers[pw_ringshield] || target->player->powers[pw_bombshield]
		|| target->player->powers[pw_watershield]
#ifdef SRB2K
		|| target->player->powers[pw_lightningshield]
		|| target->player->powers[pw_flameshield]
		|| target->player->powers[pw_bubbleshield]
#endif
		)
	{
		...
		target->player->powers[pw_lightningshield] =
		target->player->powers[pw_flameshield] =
		target->player->powers[pw_bubbleshield] = false;
#endif
	...
	if (inflictor && ((inflictor->flags & MF_MISSILE) || inflictor->player)
		&& target->player->powers[pw_super] && ALL7EMERALDS(target->player->powers[pw_emeralds]))
	{
...
static inline boolean P_PlayerHitsPlayer(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
	...
	if (source == target) // You can't kill yourself...
		return false;
...
static void P_KillPlayer(player_t *player, mobj_t *source, int damage)
{
	...
	if (cv_grmd2.value && md2_playermodels[player->skin].notfound == false)
		P_RollMobjRelative(player->mo, ANG180, 6, false);
#endif
	...
	player->powers[pw_flameshield] =
	player->powers[pw_bubbleshield] = false;
#endif
	player->mo->momx = player->mo->momy = player->mo->momz = 0;
	...
	if (player->powers[pw_underwater] != 1) // Don't jump up when drowning
		P_SetObjectMomZ(player->mo, 18*FRACUNIT, false, false);
	else
		P_SetObjectMomZ(player->mo, 1, true, false);
	...
	else if (source && (source->type == MT_DISS
						|| source->type == MT_FLOORSPIKE
						|| source->type == MT_CEILINGSPIKE) && source->threshold == 43) // Spikes
	...
	// Don't score points against super players
	if (source && source->player && !player->powers[pw_super])
	{
		// Award no points when players shoot each other when cv_friendlyfire is on.
		if (!((gametype == GT_CTF || (gametype == GT_MATCH && cv_matchtype.value))
			  && (source->player->ctfteam == player->ctfteam)))
...
boolean P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
	...
	if (target->flags & MF_MONITOR
		&& ((!source || !source->player) || (inflictor && !inflictor->player)))
		return false;
	...
	if (target->type == MT_REDRINGBOX && !(source && source->player && source->player->ctfteam == 1))
		return false;

	if (target->type == MT_BLUERINGBOX && !(source && source->player && source->player->ctfteam == 2))
		return false;
	...
	if (player) // Player is the target
	{
		...
		if ((target->player->pflags & PF_NIGHTSMODE) 
#ifdef FREEFLY
			&& !mapheaderinfo[gamemap-1].freefly
#endif
			&& source == target)
			return false;

		if (!(target->player->pflags & PF_NIGHTSMODE)
			&& !(target->player->pflags & PF_NIGHTSFALL) && (maptol & TOL_NIGHTS))
			return false;
		...
		if (damage == 10000)
		{
			...
			|| (player->powers[pw_super] && !(ALL7EMERALDS(player->powers[pw_emeralds])
											  && inflictor && ((inflictor->flags & MF_MISSILE) || inflictor->player)))))
		{
		...
		else if (damage < 10000 && !player->powers[pw_super]
				 && (player->powers[pw_forceshield] || player->powers[pw_jumpshield]
					 || player->powers[pw_ringshield] || player->powers[pw_bombshield] || player->powers[pw_watershield]
#ifdef SRB2K
		...
		if (inflictor && ((inflictor->flags & MF_MISSILE) || inflictor->player)
			&& player->powers[pw_super] && ALL7EMERALDS(player->powers[pw_emeralds]))
		{
			if (player->powers[pw_forceshield] || player->powers[pw_jumpshield]
				|| player->powers[pw_ringshield] || player->powers[pw_bombshield] || player->powers[pw_watershield]
#ifdef SRB2K
				|| player->powers[pw_lightningshield] || player->powers[pw_flameshield] || player->powers[pw_bubbleshield]
#endif
	...
	// do the damage
	if (player && player->powers[pw_super] && ALL7EMERALDS(player->powers[pw_emeralds])
		&& inflictor && ((inflictor->flags & MF_MISSILE) || inflictor->player))
	{
...
#ifdef JTEBOTS
	// Bots dun spill rings, silly.
	if (gametype == GT_COOP && player->bot)
	{
		player->pflags &= ~PF_NIGHTSFALL;
		if (gametype == GT_COOP) // make sure you can spill rings in other gametypes!
			return;
	}
#endif

	if (num_rings > 64 && !(player->pflags & PF_NIGHTSFALL)) // SRB2CBTODO: Custom stuff for falling
		num_rings = 64;
	...
	if (player->ringweapons)
	{
		...
		for (i = 0; i < num_weapons; i++)
		{
			...
			if (player->pflags & PF_NIGHTSFALL)
			{
				...
				P_SetObjectMomZ(mo, 8*FRACUNIT, false, false);
			...
			else
			{
				if (i > 15)
				{
					...
					if (i & 1)
						P_SetObjectMomZ(mo, 4*FRACUNIT, true, false);
				}
				else
				{
					...
					P_SetObjectMomZ(mo, 3*FRACUNIT, false, false);

					if (i & 1)
						P_SetObjectMomZ(mo, 3*FRACUNIT, true, false);
				}
			}
		}
	}
	...
	for (i = 0; i < num_rings; i++)
	{
		...
		else
		{
			if (i > 15)
			{
				...
				P_SetObjectMomZ(mo, (maptol & TOL_ERZ3) ? (4*FRACUNIT) >> 2 : 4*FRACUNIT, false, false);

				if (i & 1)
					P_SetObjectMomZ(mo, (maptol & TOL_ERZ3) ? (4*FRACUNIT) >> 2 : 4*FRACUNIT, true, false);
			}
			else
			{
				ns = FIXEDSCALE(2 * FRACUNIT, mo->scale);
				...
				P_SetObjectMomZ(mo, (maptol & TOL_ERZ3) ? (3*FRACUNIT) >> 2 : 3*FRACUNIT, false, false);

				if (i & 1)
					P_SetObjectMomZ(mo, (maptol & TOL_ERZ3) ? (3*FRACUNIT) >> 2 : 3*FRACUNIT, true, false);
			}
		}
	}
	...
	if (player->powers[pw_emeralds])
	{
		...
		for (i = 0; i < num_stones; i++)
		{
			...
			if (toss)
			{
				...
				if (!(player->mo->eflags & MFE_VERTICALFLIP)) // SRB2CBCHECK
			...
			if (player->mo->eflags & MFE_VERTICALFLIP)  // SRB2CBCHECK was * MFE_V
				mo->momz = -mo->momz;

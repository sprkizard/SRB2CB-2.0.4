...
//
// Gets the name of the first button bound to a game control.
// Returns "(unbound)" if none is found. // XSRB2
//
const char *G_GamekeyToString(int gamecontrolnum)
{
	I_Assert(gamecontrolnum < num_gamecontrols);
	
	if (gamecontrol[gamecontrolnum][0] != KEY_NULL)
		return G_KeynumToString(gamecontrol[gamecontrolnum][0]);
	else if (gamecontrol[gamecontrolnum][1] != KEY_NULL)
		return G_KeynumToString(gamecontrol[gamecontrolnum][1]);
	else
		return "(unbound)";
}

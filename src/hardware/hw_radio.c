...
/**
 * Imported from the Doomsday Engine: rend_fakeradio.c: Faked Radiosity Lighting
 *
 * Perhaps the most distinctive characteristic of radiosity lighting
 * is that the corners of a room are slightly darker than the rest of
...
static void scanEdges(shadowcorner_t topCorners[2],
                      ...
                      const seg_t* line, boolean backSide);
...
void Rend_RadioRegister(void)
{
   // C_VAR_INT("rend-fakeradio", &rendFakeRadio, 0, 0, 2);
	...
	// SRB2CBTODO: For the console redesign,
	//make it so setting up variables is easy like this
	// and can be pased to a global var? (maybe keep the cv_ stuff)
 ...
 * given linedef.
 */
void Rend_RadioUpdateLinedef(seg_t* line, boolean backSide)
{
    side_t*          s;
	
    if(!rendFakeRadio || // Disabled? // SRB2CBTODO: levelFullBright
       !line)
        return;
    ...
    // on this sidedef?
	if (line->backsector)
		s = &sides[line->linedef->sidenum[1]];
	else // front sector
		s = &sides[line->linedef->sidenum[0]];
	
    //sideDefs[backSide? BACK : FRONT];
    //if(s->fakeRadioUpdateCount != frameCount) // SRB2CBTODO: Uncapped framerate
    {   // Not yet. Calculate now.
        uint                i;
		
        for(i = 0; i < 2; ++i)
        {
            s->spans[i].length = line->len; // FRADIO correct floating?
            s->spans[i].shift = 0;
        }
		
        scanEdges(s->topCorners, s->bottomCorners, s->sideCorners,
                  s->spans, line, backSide);
       // s->fakeRadioUpdateCount = frameCount; // Mark as done. // FRADIO: Marker for uncapped fps, the process is done
    }
}
 ...
 */
static void setRendpolyColor(rcolor_t* rcolors, uint num, float darkness) // FRADIO: Convert this to OpenGLness
{
    uint                i;
...
static void scanNeighbor(boolean scanTop, const seg_t* line, uint side,
                         edge_t* edge, boolean toLeft)
{
#define SEP             (10)
	
    seg_t*          iter;
    lineowner_t*        own; // FRADIO: Oh man, make a whole system checking what line a vertex belongs to, that's CRAZY!
    angle_t          diff = 0;
    float               lengthDelta = 0, gap = 0;
    float               iFFloor, iFCeil;
    float               iBFloor, iBCeil;
    int                 scanSecSide = side;
    sector_t*           startSector = side ? line->backsector : line->frontsector; // FRADIO: Line side or side on sector?
    sector_t*           scanSector;
    ...
    fFloor = side ? line->backsector->floorheight/FRACUNIT : line->frontsector->floorheight/FRACUNIT; // line->L_sector(side)->SP_floorvisheight; is float!!! FRADIO
    fCeil = side ? line->backsector->ceilingheight/FRACUNIT : line->frontsector->ceilingheight/FRACUNIT;//line->L_sector(side)->SP_ceilvisheight i float!;
    ...
    do
    {
        ...
        scanSecSide = (iter->frontsector == startSector);
        ...
        if(!stopScan)
        {
            iFFloor = iter->frontsector->floorheight/FRACUNIT;
            iFCeil  = iter->frontsector->ceilingheight/FRACUNIT;
			
            if(iter->backsector)
            {
                iBFloor = iter->backsector->floorheight/FRACUNIT;
                iBCeil  = iter->backsector->ceilingheight/FRACUNIT;
            }
            else
                iBFloor = iBCeil = 0;
        }
        ...
        if(!stopScan)
        {
            ...
            {
                if(iter->backsector &&
                   ((side == 0 && iter->backsector == line->frontsector &&
					 iFCeil >= fCeil) ||
					(side == 1 && iter->backsector == line->backsector &&
					 iFCeil >= fCeil) ||
                    (side == 0 && closed == false && iter->backsector != line->frontsector &&
					 iBCeil >= fCeil &&
					 isSectorOpen(iter->backsector))))
                {
                    gap += iter->len; // Should we just mark it done instead?
                }
                else
                {
                    edge->len += iter->len + gap;
                    gap = 0;
                }
            }
            else
        ...
        else
        {
            ...
            if(!(diff >= ANGLE_180 - SEP && diff <= ANGLE_180 + SEP))
                stopScan = true; // no.
            else if(scanSector)
            {
                ...
                else
                {
                    ...
                    if(scanTop)
                    {
                        if(scanSector->ceilingheight/FRACUNIT != fCeil &&
                           scanSector->floorheight/FRACUNIT <
						   startSector->ceilingheight/FRACUNIT)
                            stopScan = true;
                    }
                    else
                    {
                        if(scanSector->floorheight/FRACUNIT != fFloor &&
                           scanSector->ceilingheight/FRACUNIT >
						   startSector->floorheight/FRACUNIT)
                            stopScan = true;
                    }
                }
            }
        ...
        if(!stopScan)
        {
            ...
            if(scanSector && line->L_side(side^1) &&
			   ...
			   ((scanTop && scanSector->ceilingheight/FRACUNIT ==
				 startSector->ceilingheight/FRACUNIT) ||
				(!scanTop && scanSector->floorheight/FRACUNIT ==
				 startSector->floorheight/FRACUNIT)))
            {
    ...
    if(edge->sector) // the back sector of the coalignable neighbor.
    {
		...
		R_FindLineNeighbor(edge->sector, edge->line,
						   edge->line->vo[(edge->line->backsector && edge->line->backsector == edge->sector)^!toLeft],
						   !toLeft, &edge->diff);
    }
                          ...
                          edgespan_t spans[2], boolean toLeft)
{
    ...
    fFloor = line->L_sector(side)->floorheight/FRACUNIT;
    fCeil = line->L_sector(side)->ceilingheight/FRACUNIT;
    ...
    for(i = 0; i < 2; ++i) // 0=bottom, 1=top
    {
        ...
        {
            if(edge->diff > ANGLE_180)
            {   // The corner between the walls faces outwards.
                corner->corner = -1;
            }
            else if(edge->diff == ANGLE_180)
            {   // Perfectly coaligned? Great.
                corner->corner = 0;
            }
            else if(edge->diff < ANGLE_45 / 5)
            {   // The difference is too small, there won't be a shadow.
                corner->corner = 0;
            }
            // 90 degrees is the largest effective difference.
            else if(edge->diff > ANGLE_90)
            {
                corner->corner = (float) ANGLE_90 / edge->diff;
            }
            else
            {
                corner->corner = (float) edge->diff / ANGLE_90;
            }
        }
        else
        ...
        if(edge->sector)
        {
            ...
            if(i == 0) // Floor.
            {
                corner->pOffset = corner->proximity->floorheight/FRACUNIT - fFloor;
                corner->pHeight = corner->proximity->floorheight/FRACUNIT;
            }
            else // Ceiling.
            {
                corner->pOffset = corner->proximity->ceilingheight/FRACUNIT - fCeil;
                corner->pHeight = corner->proximity->ceilingheight/FRACUNIT;
            }
        }
        else
        {
                      ...
                      const seg_t* line, boolean backSide)
{
    ...
    for(i = 0; i < 2; ++i)
    {
        ...
        if(other && other != line)
        {
            ...
            else if(diff == ANGLE_180)
            {
                sideCorners[i].corner = 0;
            }
            else if(diff < ANGLE_45 / 5)
            {   // The difference is too small, there won't be a shadow.
                sideCorners[i].corner = 0;
            }
            else if(diff > ANGLE_90)
            {   // 90 degrees is the largest effective difference.
                sideCorners[i].corner = (float) ANGLE_90 / diff;
            }
            else
            {
                sideCorners[i].corner = (float) diff / ANGLE_90;
            }
        }
        else
            sideCorners[i].corner = 0;
...
// Very important for real lighting
/**
                               ...
                               const edgespan_t* spans)
{
    ...
    //p->texture = LST_RADIO_OO; // FRADIO: OK, the core thing about this radiosity is that images are used to make the shadows
    ...
    {   // At least one corner faces outwards
        ...
        {   // Both corners face outwards
            p->texture = LST_RADIO_OO;//CC;   // THIS IS VERY IMPORTANT, check this
        }
                                ...
                                const rendsegradio_params_t* p)
{
    ...
    fFloor = &p->frontSec->floorheight/FRACUNIT;
    fCeil = &p->frontSec->ceilingheight/FRACUNIT;
    if(p->backSec)
    {
        bFloor = &p->backSec->floorheight/FRACUNIT;
        bCeil  = &p->backSec->ceilingheight/FRACUNIT;
    }
    else
        bFloor = bCeil = NULL;
...
static uint radioEdgeHackType(seg_t *line, sector_t *front,
                              ...
                              boolean isCeiling, float fz, float bz)
{
    ...
    if(front->floorheight/FRACUNIT >= back->ceilingheight/FRACUNIT)
    {
 ...
 * rendering lists.
 */
static void radioAddShadowEdge(const seg_t* line, byte side,
    ...
    for(link = subsector->shadows; link != NULL; link = link->next)
    {
        ...
        for(pln = 0; pln < subsector->sector->planeCount; ++pln)
        {
            ...
            for(i = 0; i < 2; ++i)
            {
                ...
                if(neighbor != line && !neighbor->backsector &&
                   neighbor->buildData.windowEffect &&
                   neighbor->frontsector != subsector->sector)
                {   // A one-way window, open side.
                    sideOpen[i] = 1;
                }
                else if(!(neighbor == line || !neighbor->backsector))
                {
                    sector_t*           othersec;
                    byte                otherSide;
                    ...
                    if(LINE_SELFREF(neighbor) ||
                       ((R_IsSkySurface(&othersec->SP_planesurface(pln)) ||
                        ...
                        othersec->floorheight/FRACUNIT >= othersec->ceilingheight/FRACUNIT))
                    {
                        sideOpen[i] = 1;
                    }
                    ...
                    {   // Its a normal neighbor.
                        if(neighbor->L_sector(otherSide) != line->L_sector(side) &&
                           !((plane->type == PLN_FLOOR &&
							  othersec->ceilingheight/FRACUNIT <= plane->visHeight) ||
                             (plane->type == PLN_CEILING &&
							  othersec->floorheight >= plane->visHeight)))
                        {
                            front = line->L_sector(side);
...
void Rend_DrawShadowOffsetVerts(void)
{
    ...
    for(i = 0; i < numLineDefs; ++i)
    {
        ...
        for(k = 0; k < 2; ++k)
        {
            ...
            {
                pos[VZ] = vo->lineDef->frontsector->floorheight/FRACUNIT;

...
int HW3S_I_StartSound(const void *origin_p, source3D_data_t *source_parm, channel_type_t c_type, sfxenum_t sfx_id, int volume, int pitch, int sep)
{
	...
	if (splitscreen && rendersplit) listenmobj2 = players[secondarydisplayplayer].mo;
	...
	if ((splitscreen && rendersplit) && listenmobj2) // Copy the sound for the split player
	{
...
static void HW3S_UpdateListener(mobj_t *listener)
{
	...
	if (camera.chase)
	{
		...
		data.f_angle = ANGLE_TO_DOUBLE(camera.angle);
		data.h_angle = ANGLE_TO_DOUBLE(camera.aiming);
	...
	else
	{
		...
		data.f_angle = ANGLE_TO_DOUBLE(listener->angle);
		data.h_angle = ANGLE_TO_DOUBLE(listener->player->aiming);
...
static void HW3S_UpdateListener2(mobj_t *listener)
{
	...
	if (camera2.chase)
	{
		...
		data.f_angle = ANGLE_TO_DOUBLE(camera2.angle);
		data.h_angle = ANGLE_TO_DOUBLE(camera2.aiming);
...
void HW3S_UpdateSources(void)
{
	...
	if (splitscreen && rendersplit) listener2 = players[secondarydisplayplayer].mo;
...
void HW3S_Shutdown(void) // Only used for SDL
{
	HW3DS.pfnShutdown();
}

...
//-----------------------------------------------------------------------------
/// \file
/// \brief Windows specific part of the OpenGL API for SRB2CB
///
///	TODO:
...
#ifdef DEBUG_TO_FILE // SRB2CBTODO: ogllog is no longer needed!
...
//                  : video modes in Gem Engine OpenGL are of the same BPP, thus the
//                  : PixelFormat is set only once.
//                  : Setting the pixel format more than once on the same window
//                  : doesn't work. (Ultimately for different pixel formats, the game
...
int SetupPixelFormat(int WantColorBits, int WantStencilBits, int WantDepthBits)
{
	...
	if (iLastPFD)
	{
		CONS_LogPrintf("WARNING : SetPixelFormat() called twice, not supported by all drivers!\n");
	}
	...
	CONS_LogPrintf("SetupPixelFormat() - %d ColorBits - %d StencilBits - %d DepthBits\n",
				   WantColorBits, WantStencilBits, WantDepthBits);
	
	nPixelFormat = ChoosePixelFormat(hDC, &pfd);
	
	if (nPixelFormat == 0)
		CONS_LogPrintf("ChoosePixelFormat() FAILED\n");
	
	if (SetPixelFormat(hDC, nPixelFormat, &pfd) == 0)
	{
		CONS_LogPrintf("SetPixelFormat() FAILED\n");
		return 0;
	}
...
static int WINAPI SetRes(viddef_t *lvid, vmode_t *pcurrentmode)
{
	...
	pcurrentmode = NULL;
	CONS_LogPrintf ("SetMode(): %dx%d %d bits (%s)\n",
	...
	
	{
		...
		else if (res == 1)
		{
			...
			if (!hGLRC)
			{
				CONS_LogPrintf("pwglCreateContext() FAILED\n");
				return 0;
			}
			if (!wglMakeCurrent(hDC, hGLRC))
			{
				CONS_LogPrintf("wglMakeCurrent() FAILED\n");
				return 0;
			}
		}
	}
	...
	renderer = (LPCSTR)glGetString(GL_RENDERER);
	CONS_LogPrintf("Vendor     : %s\n", glGetString(GL_VENDOR));
	CONS_LogPrintf("Renderer   : %s\n", renderer);
	CONS_LogPrintf("Version    : %s\n", glGetString(GL_VERSION));
	CONS_LogPrintf("Extensions : %s\n", gl_extensions);
...
void GetModeList(vmode_t** pvidmodes, int *numvidmodes)
{
	...
	// SRB2CBTODO: More and higher resolutions!
	int res[][2] = {
		{ 320, 200},
		{ 320, 240},
		{ 400, 300},
	...
	CONS_LogPrintf("GetModeList()\n");
	
	bpphdc = GetDC(NULL); // Get the current bpp
...
void GL_Shutdown(void)
{
	...
	CONS_LogPrintf("GL_Shutdown()\n");
	nb_centiemes = ((clock()-my_clock)*100)/CLOCKS_PER_SEC;
	CONS_LogPrintf("Nb frames: %li;  Nb sec: %2.2f  ->  %2.1f fps\n",
	...
	CONS_LogPrintf("GL_Shutdown(DONE)\n");
}

// -----------------+
// GL_FinishUpdate     : Swap front and back buffers
// -----------------+
void GL_FinishUpdate(int waitvbl)
{
#ifdef USE_WGL_SWAP
...
#ifdef DEBUG_TO_FILE
	if ((++nb_frames)==2)  // Do not start clocking on the first frame
		my_clock = clock();
#endif
...
#endif
	// When the pallete is changed, all the textures must be reloaded
	GL_Flush();
}

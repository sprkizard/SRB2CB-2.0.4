...
fixed_t P_AproxDistance(fixed_t dx, fixed_t dy)
{
	...
	dy = abs(dy);
	if (dx < dy)
		return dx + dy - (dx>>1); // Why would this - (>>1) be needed?
	return dx + dy - (dy>>1);
}
...
int P_PointOnLineSide(fixed_t x, fixed_t y, line_t *line)
{
	...
	// SRB2CB: WALLSPLATS, check or crash!
	if (!line)
		return 0;
...
void P_CameraLineOpening(line_t *linedef)
{
	...
	// If you can see through it, why can't the camera move through it too?
	
	// Check the front
	if (front->heightsec >= 0) // SRB2CBTODO: != -1 instead?
	{
#ifdef ESLOPE
...
#ifdef ESLOPE
		if (sectors[back->heightsec].c_slope) // SRB2CBTODO: ESLOPE (sectors[front->heightsec].f_slope)
	...
	{
#ifdef ESLOPE
		if (back->f_slope) // SRB2CBTODO: ESLOPE (sectors[front->heightsec].f_slope)
			backfloor = P_GetZAt(back->f_slope, camera.x, camera.y);
		else
#endif
		backfloor = back->floorheight;
		
#ifdef ESLOPE
		if (back->c_slope) // SRB2CBTODO: ESLOPE (sectors[front->heightsec].f_slope)
			backceiling = P_GetZAt(back->c_slope, camera.x, camera.y);
		else
#endif
		...
		if (front->ffloors || back->ffloors)
		{
			...
			if (front->ffloors)
				for (rover = front->ffloors; rover; rover = rover->next)
				{
					...
					delta1 = abs(camera.z - (bottomheight + ((topheight - bottomheight)/2)));
...
// SRB2CBTODO: ESLOPE support for vertex P_GetZ etc, and P_CameraLineOpening
void P_LineOpening(line_t *linedef)
{
...
#ifdef POLYOBJECTS // P_LineOpening 3D POLYOBJECTS
	if (linedef->polyobj && (linedef->polyobj->flags & POF_TESTHEIGHT)) // SRB2CBTODO: Correct?
	{
		front = linedef->polyobj->lines[0]->frontsector;
		back = linedef->polyobj->lines[0]->frontsector;
	}
	else
#endif
	{
	...
	if (tmthing)
	{
		...
		if (front->ceilingheight < back->ceilingheight) // ESLOPE: FIX THIS VPHYSICS
		{
#ifdef ESLOPE
			if (front->c_slope)
		...
		if (front->floorheight > back->floorheight) // ESLOPE: FIX THIS VPHYSICS
		{
#ifdef ESLOPE
			if (front->f_slope)
...
#ifdef ESLOPE
			if (back->f_slope)
				lowfloor = P_GetZAt(back->f_slope, tmthing->x, tmthing->y);
			else
#endif
...
#ifdef ESLOPE
			if (front->f_slope)
				lowfloor = P_GetZAt(front->f_slope, tmthing->x, tmthing->y);
			else
#endif
...
#ifdef POLYOBJECTS // Lineopening stuff for 3D polys
			// Treat polyobj's backsector like a 3D Floor
			// SRB2CBTODO: Polyobjects have an issue when standing on them!
			// (but, this has nothing to do with this function)
			if (linedef->polyobj && (linedef->polyobj->flags & POF_TESTHEIGHT))
			{
				// SRB2CBTODO: linedef->polyobj->lines[0]->backsector, not linedef->backsector(or does it matter?)
				sector_t *polysec = linedef->polyobj->lines[0]->backsector;
	...
	// This tells the game if a linedef is 'open' as in visible
	
#ifdef ESLOPE // Special handling - make sure to 'open' any linedefs with slopes!
	// get the lowest plane to be the opentop
	if (front->c_slope || back->c_slope)
	{
		if (front->c_slope && back->c_slope)
		{
			if (P_GetZAt(front->c_slope, tmthing->x, tmthing->y) < P_GetZAt(back->c_slope, tmthing->x, tmthing->y))
			{
				opentop = P_GetZAt(front->c_slope, tmthing->x, tmthing->y);
			}
			else
		...
		else if (front->c_slope && !back->c_slope)
		{
			if (P_GetZAt(front->c_slope, tmthing->x, tmthing->y) < back->floorheight)
			{
				opentop = P_GetZAt(front->c_slope, tmthing->x, tmthing->y);
			}
			else
			{
				opentop = back->floorheight;
			}
		}
		else if (!front->c_slope && back->c_slope)
		{
			if (front->floorheight < P_GetZAt(back->c_slope, tmthing->x, tmthing->y))
			{
				opentop = front->floorheight;
			}
			else
			{
				opentop = P_GetZAt(back->c_slope, tmthing->x, tmthing->y);
	...
	if (front->f_slope || back->f_slope)
	{
		if (front->f_slope && back->f_slope)
		{
			if (front->f_slope->highz > back->f_slope->highz)
			{
				openbottom = front->f_slope->highz;
				lowfloor = back->f_slope->highz;
			}
			else
			{
				openbottom = back->f_slope->highz;
				lowfloor = front->f_slope->highz;
			}
		}
		else if (front->f_slope && !back->f_slope)
		{
			if (front->f_slope->highz > back->floorheight)
			{
				openbottom = front->f_slope->highz;
				lowfloor = back->floorheight;
			}
		...
		{
			if (front->floorheight > back->f_slope->highz)
			{
				openbottom = front->floorheight;
				lowfloor = back->f_slope->highz;
			}
...
{
	// Make sure that there is a thing to set
	if (!thing)
	{
...
{                                                      // link into subsector
	...
	if (!thing)
	{
		I_Error("P_SetThingPosition: Tried to set a thing that doesn't exist!\n");
		return;
	}

	if (thing->subsector && thing->player && (thing->z <= thing->floorz))
	...
	{
		// inert things don't need to be in blockmap
		const int blockx = (unsigned int)(thing->x - bmaporgx)>>MAPBLOCKSHIFT;
		const int blocky = (unsigned int)(thing->y - bmaporgy)>>MAPBLOCKSHIFT;
...
boolean P_BlockLinesIterator(int x, int y, boolean (*func)(line_t *))
{
...
#ifdef POLYOBJECTS // Blocklinesiterator
	...
	// First index is really empty, so +1 it. // SRB2CBTODO: Where else in the code does this happen?
...
//
boolean P_BlockThingsIterator(int x, int y, boolean (*func)(mobj_t *)) // TODO: WHAT IS THIS?!!!
{
	mobj_t *mobj = NULL;
	...
	{
		if (!func(mobj)) // TODO: WHAT IS THIS?!!!!
			return false;
	}
	return true;
}
...
// Allocate data for line and object intercepts
...
static boolean PIT_AddLineIntercepts(line_t *ld)
{
	...
	dl.x = trace.x;
	dl.y = trace.y;
	dl.dx = (trace.x+trace.dx) - trace.x;
	dl.dy = (trace.y+trace.dy) - trace.y;
...
// for all lines.
//
// This function allows for things such as stopping objects from gaining speed while touching walls
//
	...
	int flags, traverser_t trav)
{
	...
	px1 -= bmaporgx;
	py1 -= bmaporgy;
	xt1 = (unsigned int)px1>>MAPBLOCKSHIFT;
	yt1 = (unsigned int)py1>>MAPBLOCKSHIFT;

	px2 -= bmaporgx;
	py2 -= bmaporgy;
	xt2 = (unsigned int)px2>>MAPBLOCKSHIFT;
	yt2 = (unsigned int)py2>>MAPBLOCKSHIFT;

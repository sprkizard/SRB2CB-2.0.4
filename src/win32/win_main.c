...
static inline VOID OpenTextConsole(VOID)
{
	...
	if (M_CheckParm("-detachconsole"))
	{
		if (FreeConsole())
		{
			CONS_Printf("A console was lost, it probably was Mingw's Bash\n");
			console = TRUE; //lets get back a console
		}
		else
		{
			CONS_Printf("A console was not lost\n");
	...
	if (dedicated || console)
	{
		if (AllocConsole()) // Let get the real console HANDLEs, because Mingw's Bash is bad!
		{
			...
			CONS_Printf("Starting up SRB2's Console Window..\n");
		}
		else
		{
			CONS_Printf("There's a console already?\n");
			I_ShowLastError(FALSE);
			return;
		}
	}
...
static int WINAPI HandledWinMain(HINSTANCE hInstance)
{
	...
	if ((hWndMain = OpenMainWindow(hInstance, va("SRB2CB "VERSIONSTRING))) == INVALID_HANDLE_VALUE)
	{

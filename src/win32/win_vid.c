...
//-----------------------------------------------------------------------------
/// \file
/// \brief The win32 video driver for SRB2
...
#ifdef HWRENDER // The rendermode is opengl
...
{
	// I_UpdateNoBlit is only used in SDL builds
}
...
static void VID_Init(void)
{
	...
	if (rendermode != render_soft)
	{
        ...
        if (rendermode == render_opengl)
			GetModeList(&pvidmodes, &numvidmodes);
		else
		{
			switch (rendermode)
			{

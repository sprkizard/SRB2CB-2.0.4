...
typedef enum
{
	//RFLAGPOINT = 0x01, // SRB2CBTODO: We can use these for something now!
	//BFLAGPOINT = 0x02,
	CAPSULE    = 0x04,
	AWAYVIEW   = 0x08,
	FIRSTAXIS  = 0x10,
...
static void P_NetArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
...
#ifdef JTEBOTS
		WRITEBYTE(save_p, bots[i].ownernum);
		WRITEBYTE(save_p, bots[i].springmove);
		WRITESHORT(save_p, bots[i].targettimer);
#endif
		WRITELONG(save_p, players[i].speed);
		WRITEBYTE(save_p, players[i].jumping);
...
#ifdef WALLRUN
		WRITELONG(save_p, players[i].wallrunning);
#endif
		...
		// SRB2CBTODO: Proper save stuff // NEWCB
		WRITEANGLE(save_p, players[i].twodcamangle);
		WRITEFIXED(save_p, players[i].twodcamdist);
		
		WRITEFIXED(save_p, players[i].twodspeed);
        WRITELONG(save_p, players[i].fovadd);
...
#ifdef JTEBOTS
		if (flags & BOTTARGET)
			WRITEULONG(save_p, bots[i].target);
#endif
...
static void P_NetUnArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		players[i].jumping = READBYTE(save_p); // Jump counter
...
#ifdef WALLRUN
		players[i].wallrunning = READLONG(save_p); // Climbing on the wall
#endif
...
#ifdef JTEBOTS
		bots[i].ownernum = READBYTE(save_p);
		bots[i].springmove = READBYTE(save_p);
		bots[i].targettimer = READSHORT(save_p);
#endif
		...
		// SRB2CBTODO: New netgame stuff
		players[i].twodcamangle = READANGLE(save_p);
		players[i].twodcamdist = READFIXED(save_p);
		
		// NEWCB
		players[i].twodspeed = READFIXED(save_p);
        players[i].fovadd = READLONG(save_p);
...
#ifdef JTEBOTS
		if (flags & BOTTARGET)
			bots[i].target = (mobj_t *)(size_t)READULONG(save_p);
#endif
...
#define SD_FLOORSCAL 0x80
#define SD_CEILSCAL  0x100
...
static void P_NetArchiveWorld(void)
{
	...
	for (i = 0; i < numsectors; i++, ss++, ms++)
	{
		...
		if (diff)
		{
			...
			if (diff2 & SD_FLOORSCAL)
				WRITELONG(put, ss->floor_scale);
			if (diff2 & SD_CEILSCAL)
				WRITELONG(put, ss->ceiling_scale);
		}
	}
	...
	for (i = 0; i < numlines; i++, mld++, li++)
	{
		...
		if (li->sidenum[0] != 0xffff)
		{
			...
			if (si->textureoffset != SHORT(msd[li->sidenum[0]].textureoffset)<<FRACBITS)
				diff |= LD_S1TEXOFF;
			// Some textures are colormaps. Don't worry about invalid textures.
...
static void P_NetUnArchiveWorld(void)
{
	...
	for (;;)
	{
		...
		if (diff2 & SD_FLOORSCAL)
			sectors[i].floor_scale  = READLONG(get);
		if (diff2 & SD_CEILSCAL)
			sectors[i].ceiling_scale = READLONG(get);
	}
...
#ifdef SPRITEROLL
	MD_ROLLANGLE   = 0x80000000,
#endif
#ifdef VPHYSICSTODO // OSNAP, ran out of space
	MD_PITCHANGLE  = 0x100000000
#endif
} mobj_diff_t;
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		if (th->function.acp1 == (actionf_p1)P_MobjThinker)
		{
			...
			if (mobj->height != mobj->info->height)
				diff |= MD_HEIGHT;
			if (mobj->flags != mobj->info->flags || mobj->flags & MF_TRANSLATION) // SRB2CBTODO:?
				diff |= MD_FLAGS;
...
#endif
#ifdef VPHYSICSTODO // Need more space
			if (mobj->pitchangle)
				diff |= MD_PITCHANGLE;
#endif
...
// SRB2CBTODO:?
...
#ifdef SPRITEROLL
			if (diff & MD_ROLLANGLE)
			{
				WRITEANGLE(save_p, mobj->rollangle);
				WRITEANGLE(save_p, mobj->destrollangle);
				WRITEBYTE(save_p, mobj->rollspeed);
			}
#endif
#ifdef VPHYSICSTODO // Need more space
			if (diff & MD_PITCHANGLE)
				WRITEANGLE(save_p, mobj->pitchangle);
#endif
		...
		else if (th->function.acp1 == (actionf_p1)T_TimedThinker) // TIMEDTODO
		{
			SaveSpecialLevelThinker(th, tc_timed);
			continue;
		}
...
static void P_NetUnArchiveThinkers(void)
{
	...
	// The removed mobjs shouldn't come back
	iquetail = iquehead = 0;
	P_InitThinkers();
		...
		switch (tclass)
		{
			case tc_mobj:
				...
				if (diff & MD_SPAWNPOINT)
				{
					...
					if (mapthings[spawnpointnum].type == MT_HOOP) // NiGHTS Hoop special case
					{
...
// SRB2CBTODO:?
				...
				if (diff & MD_ROLLANGLE)
				{
					...
					mobj->destrollangle = READANGLE(save_p);
					mobj->rollspeed = READBYTE(save_p);
				}
#endif
#ifdef VPHYSICSTODO // Need more space
				if (diff & MD_PITCHANGLE)
					mobj->pitchangle = READANGLE(save_p);
#endif
			...
			case tc_timed: // TIMEDTODO
				LoadSpecialLevelThinker(0, (actionf_p1)T_TimedThinker);
				break;
			...
			/// \todo // SRB2CBTODO: rewrite all the that uses an elevator_t but isn't an elevator
			/// \note working on it!
			...
			case tc_laserflash:
				{
					...
					laser->sec = &sectors[(size_t)laser->sec];
					laser->ffloor = &(laser->sector->ffloors)[(size_t)laser->ffloor];
					laser->sourceline = &lines[(size_t)laser->sourceline];
...
static void P_ArchiveMisc(void)
{
	...
	WRITEUSHORT(save_p, emeralds+357); // SRB2CBTODO: What is this?

...
// SRB2CBTODO: This file need to be like, totally rewritten
...
boolean SendRequestFile(void)
{
	...
	// prepare to download
	I_CreateDirectory(downloaddir, 0755);
...
int CL_CheckFiles(void)
{
	...
	// Check the main data.wad file first
...
void SendFile(int node, char *filename, char fileid)
{
	...
	{
		// too big
		// don't inform client (client's awful)
...
void FiletxTicker(void)
{
	...
	found:
		...
		if (ram)
			...
			I_Error("FiletxTicker: can't read %u byte on %s at %lu because %s", (int)size, f->filename, transfer[i].position, strerror(ferror(transfer[i].currentfile)));
		p->position = transfer[i].position;
		// Add a flag so the receiver knows the totalsize
...
void Got_Filetxpak(void)
{
	...
	{
		// use a special tric to know when file is finished (not always used)
